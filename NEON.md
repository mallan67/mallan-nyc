...
> **⚠️ UPDATE 2026-06-03 (post cold-waterfall repoint):** The line above calling `morning-bread-68708332` "the production DB project" is **no longer accurate.** After the 2026-06-02 cross-project rescue, production DATA is served by **`hidden-mountain-87248164` / `ep-cold-waterfall-adno3ao2`** ("neon-green-school"), and the bare `DATABASE_URL` / `DATABASE_URL_UNPOOLED` / `ASSISTANT_DATABASE_URL` that Prisma reads were repointed there. `morning-bread` / `royal-dawn` is stale / do-not-serve. Do **not** "fix" `NEON_PROJECT_ID` toward `morning-bread`. See the 2026-06-03 row in §10 of this file.
