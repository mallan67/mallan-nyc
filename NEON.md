...
| 2026-06-01 | **Tier 2 stabilization — PR-close preview-branch cleanup workflow (draft PR, HELD).** Added `.github/workflows/cleanup-neon-preview-branch.yml` (official `neondatabase/delete-branch-action@v3` + a production-isolation guard step) to delete `preview/<head_ref>` on PR close, complementing the daily `neon-branch-prune` cron. Uses dedicated preview-only creds `vars.NEON_PREVIEW_PROJECT_ID` (= `hidden-mountain-87248164`) + `secrets.NEON_PREVIEW_API_KEY`; hard-pinned to the preview project and refuses the legacy do-not-serve project (`morning-bread-68708332`), protected branch names, and suspicious refs. **HELD** until Maya adds the two GitHub config items + confirms a read-only key test. See `docs/support/vercel-neon-false-branch-limit-status-2026-06-03.md`. | (draft PR) |
| 2026-06-03 | **Production DB ownership corrected after the 2026-06-02 cross-project repoint.** Canonical production data + the bare `DATABASE_URL` / `DATABASE_URL_UNPOOLED` / `ASSISTANT_DATABASE_URL` now serve **`hidden-mountain-87248164` ("neon-green-school") · `ep-cold-waterfall-adno3ao2` · branch `main` (`br-crimson-frog-adr7g9gt`)**. The old **`morning-bread-68708332` · `ep-royal-dawn-ad6eh8t2` (`br-old-tree-admdlb9z`) is stale / do-not-serve.** `cold-waterfall` and `royal-dawn` are endpoints in **two DIFFERENT Neon projects**, not two endpoints on one `main` branch — §11's "Known mismatch" framing and `docs/architecture/NEON-VERCEL-OWNERSHIP-MAP.md` §5/§8 were inverted and are now annotated as superseded. `rotate-db-keys` **schedule disabled** (PR #321; manual `workflow_dispatch` only) — keep disabled until retargeted to cold-waterfall + a fail-closed host guard (`docs/rotate-db-keys-host-guard-patch-2026-06-02.md`). Tracked Trestle token/diagnostic dumps removed from `main` (PR #322). **Do not treat `morning-bread` as production in future docs/scripts.** | (current) |
...
The `NEON_PROJECT_ID` env var on Vercel Production currently points at `morning-bread-68708332` (the legacy / stale project — see the 2026-06-03 update below), NOT at `hidden-mountain-87248164` (the integration's preview-branching project). The cron has therefore been running daily against a project that has no accumulating preview branches — operationally a no-op. Vercel's own auto-cleanup has been doing the work all along.

This is acknowledged but NOT fixed in this commit. Tracked separately. See `docs/support/vercel-neon-false-branch-limit-status-2026-06-03.md`.

> **⚠️ UPDATE 2026-06-03 (post cold-waterfall repoint):** The line above calling `morning-bread-68708332` "the production DB project" is **no longer accurate.** After the 2026-06-02 cross-project rescue, production DATA is served by **`hidden-mountain-87248164` / `ep-cold-waterfall-adno3ao2`** ("neon-green-school"), and the bare `DATABASE_URL` + `DATABASE_URL_UNPOOLED` that Prisma reads (datasource `prisma/schema.prisma:16-17`) were repointed there (the separate `ASSISTANT_DATABASE_URL` Production var was also repointed but is **not** a Prisma read path / currently unused in code). `morning-bread` / `royal-dawn` is stale / do-not-serve. Do **not** "fix" `NEON_PROJECT_ID` toward `morning-bread`. See the 2026-06-03 row in §10 of this file.
