...
- Returning /listing/{listing-id} as the public URL for address-displayable listings.
...
Verify final status.
Return publicUrl only when eligible.
```
...
```text
- publicUrl must use the canonical route only.
...
Canonical server/public status: controls public display, IDX, Featured, Exclusives, public URL.
```
...
10. Confirm the public URL panel shows canonical URL only.
