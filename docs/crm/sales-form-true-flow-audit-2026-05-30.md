...
|---|---|---|---|---|
| F1 | First-showing date | **C/F + K** | Form saves `FirstShowingDate`; the Coming-Soon / activation compliance gate reads **`ActivationDate`** (`[id]/route.ts:132`, `[id]/status/route.ts:148`). The agent's first-showing date never drives activation, and `FirstShowingDate` is not in Cotality. | `SALE-FORM-REDESIGN.html:7326` emits `data.FirstShowingDate`; no `ActivationDate` emit; no FIELD_MAP restore. |
| F2 | Building tax lot | **K** | Stored/restored as `BuildingTaxLot`; Cotality field is **`TaxLot`**. Public/display readers keyed on Cotality `TaxLot` miss it. | collect `:7163`, FIELD_MAP `:8754`. |
| F3 | Possession | **K** | Saved as `PossessionDate`; Cotality field is **`Possession`**. | collect `:7333`. |
| F4 | Bathrooms total | **K (low)** | `BathroomsTotal` in `features`; Cotality is **`BathroomsTotalInteger`**. Display uses the `bathrooms_full/half` columns so impact is low, but the feature key is non-Cotality. | collect `:7023`, PATCH featureKeys `:320`. |
| F5 | Permissions | **K (low)** | Form/PATCH use `Permissions` (plural); Cotality enum is **`Permission`** (singular). Internal derivation only (drives `owner_opt_out`/`participant_only`); no public leak, but non-Cotality name. | collect `:7075`, PATCH `:265`. |
| F6 | Syndicate YN | **K (drop)** | `SyndicateYN` emitted alongside the real `SyndicateTo`; `SyndicateYN` is not in Cotality and is redundant. | collect `:7083`,`:7088`. |
