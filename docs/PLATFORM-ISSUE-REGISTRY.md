...
| OPS-015 | **db-keepalive redundancy** — at `*/15` it cannot prevent 5-min compute suspends (`NEON.md:52,321`); the DB is already kept warm ~24/7 by idx-sync (`*/10`) + media-sync (`*/15`) + a 60s external uptime monitor (`docs/audits/cron-write-frequency-audit-2026-06-12.md:34,127`); the cron's main production output is error-dashboard noise (its 500s = OPS-002, masking real DB incidents). Track for retirement / re-purpose decision — **not fixed now** (cron config is Maya-held) | **P3** | Decision needed (Maya) | Maya (decision) + Claude (implement) | Prior audits + NEON.md read 2026-07-01 |
