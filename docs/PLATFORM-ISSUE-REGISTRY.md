...
| OPS-016 | **NEON.md ↔ live Neon drift (Confirmed live 2026-07-02, read-only neonctl):** (a) **history/PITR retention is 21,600s = 6 HOURS**, not the "7-day Launch PITR window" claimed in `NEON.md:51` and assumed by both Gate-6 plans + the s1 reclaim assessment — post-drain billed-storage reclaim arrives ~6h after rollback-branch deletion (better than planned), but point-in-time restore reaches back only 6h (named rollback branch still pins its LSN — Gate-6 safety intact); (b) **compute is FIXED 0.25 CU min/max, no autoscaling** → max 180 CU-hr/mo < 300 baseline → $19/mo flat confirmed, zero overage risk (closes the "compute size ABSENT" console-check gap); (c) storage: main logical 1,435 MB · rollback branch 1,433 MB (CoW) · synthetic/billed 1,493 MB = 14.6% of 10 GB cap · 2 branches. **NEON.md correction drafted — awaiting Maya approval before editing the canonical doc.** Wording rule (Maya 2026-07-02): these are **verified directly from Neon configuration** (project API via neonctl), NOT inferred from runtime behavior | **P2** | Decision needed (Maya): approve NEON.md §51 correction | Claude (draft) / Maya (approve) | Neon configuration API (neonctl) 2026-07-02 |
...
| DOC-001 | **Platform Architecture document missing** (Maya directive 2026-07-02): the platform needs a real architectural map — not a handoff, not an issue list — covering the full data flow (Cotality → sync → normalization → compliance → archive → search → website) including EVERY cron (23), queue, webhook, API boundary (284 routes), and database boundary, so any future issue can be placed immediately (where data entered / what transformed it / what persisted it / what published it / what archived it). Becomes canonical doc #6 (AGENTS.md Canonical Documentation) once written. Interactions now harder to understand than components — this reduces debugging time dramatically | **P1** | Open — dedicated lane (fresh session; needs full context budget) | Claude (author) / Maya (review) | Maya directive 2026-07-02 |
