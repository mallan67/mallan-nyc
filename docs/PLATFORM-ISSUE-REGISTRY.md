...
| OPS-010 | DB size/cost; audit-query performance over time. **[2026-07-25 · PR #569]** live read-only pg_stat: `sync_errors`=0 (empty — NOT growing), `audit_events` append-only (89,867 rows): NON-exempt actions are bounded by rolling 2yr retention (purge >2yr; pre-2028 fill), BUT `email_unsubscribed` is purge-EXEMPT + create-only (no dedup) → **the table as a whole is NOT strictly bounded** (that subset grows permanently); `listing_media` soft-deleted=28,664 (never purged) | Static analysis + live read-only pg_stat 2026-07-25 (PR #569) | Claude | Audit |
...
| **7/10** | **OPS-010A** — ✓ surface (media-sync/idx-sync upsert paths) ✓ source (`upsertListingMedia` in `lib/idx/media-sync.ts`; full-row UPDATE in `lib/idx/sync.ts`) ✓ log-derived live measurement (38.4K no-op media rewrites/day × 6 days, frozen-cursor week) ✓ DB query result (pg_stat cumulative tuple updates: listing_media 1.82M, listings 838K — 2026-06-12 operator audit) ✓ frequency/timestamps ✓ environment (prod) ✓ blast quantified (residual = `raw_data_only` + delivery-refresh AFTER base suppression; the "every synced row" figure is the pre-suppression 2026-06-12 baseline) | ✗ request/response capture · ◐ fresh steady-state re-measurement — PR #569 (2026-07-25, **draft; pending review + merge**) IMPLEMENTS the cause telemetry, and this session captured fresh read-only pg_stat (listing_media 3.40M / listings 1.40M cumulative updates; ≈118 KB WAL/light cycle; audit_events 199→566/day) BUT the live before/after capture (≥3 cycles under `DIAG_RAW_DATA_KEYS_UNTIL`, production) still needs a bounded window AFTER merge · ✗ single no-op-write repro with WAL delta. Close via: live capture + WAL/history trend pre/post fix (evidence: neon-write-amplification-forensic-2026-07-25.md) |
