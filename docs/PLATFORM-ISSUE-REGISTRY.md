...
| OPS-010 | DB size/cost; audit-query performance over time. **[2026-07-25 · PR #569]** live read-only pg_stat: `sync_errors`=0 (empty — NOT growing), `audit_events` append-only (89,867 rows) but BOUNDED to a rolling 2yr window (purge >2yr except `email_unsubscribed`; pre-2028 fill → not yet purging → will plateau), `listing_media` soft-deleted=28,664 (never purged — the genuinely unbounded one) | Static analysis + live read-only pg_stat 2026-07-25 (PR #569) | Claude | Audit |
...
| **6/10** | **OPS-010** — strict 10-field score. Captured (6): ✓ endpoint/surface ✓ source ✓ DB query (live read-only pg_stat 2026-07-25, PR #569: `sync_errors`=0; `audit_events` 89,867 append-only/2yr-bounded + 199→566/day; `listing_media` tombstones 28,664) ✓ user impact (Neon storage/cost only, no user-facing) ✓ frequency/timestamps ✓ environment (prod). Missing (4): ✗ request · ✗ response · ✗ stack trace/log · ✗ repro (no controlled no-op-write→WAL-delta repro). NOTE: DB-query graduated ✗→✓ vs the prior static classification; numeric stays 6/10 because request/response/stack-log/repro remain uncaptured (a multi-day trend series is desirable but is NOT one of the 10 scoring fields). | close via retention + a controlled no-op-write repro |
