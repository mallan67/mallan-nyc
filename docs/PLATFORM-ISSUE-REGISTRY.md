...
- **Lesson kept on record:** the schema.prisma declaration diverges from the live DB (schema says
  required, DB says nullable) — that drift itself remains real (see OPS-001 note) but it is not a
  failure mechanism for inserts.
...
- **Evidence:** Vercel `get_runtime_errors` 7d capture 2026-07-01 (timestamps correlate); H-001
  disproof removes the only competing mechanism on the table.
- **Missing:** the raw error text behind the contact log's redacted `category=db`; a reproduction;
  proof the errors stopped when the connectivity cluster stopped (no contact errors since 06-28 —
  consistent, not conclusive).
- **Confidence:** Medium.
- **Next verification:** one controlled `POST /api/contact` with a never-seen email while capturing
  runtime logs (Maya approval — production write); plus continued clean-window watch (0 contact
  errors since 06-28).
...
| OPS-020 | **OPS-009 03:00 UTC behavioral proof — RUNBOOK (due ~03:00Z 2026-07-03, first retention run on the deployed kill switch):** from the data-retention run's result JSON + audit event + runtime logs confirm ALL FOUR (Maya criteria): (1) `archive_control_state: "OFF"` · (2) `archive_skipped_reason: "ARCHIVE_ENABLED off"` · (3) T+24h off-market display-compliance step STILL RAN · (4) NO Gate-6 drain executed (archived_total stays 2,032; no drain audit events). **If clean → ask Maya to set `ARCHIVE_ENABLED=true` → verify one clean MAINTENANCE cycle (next 03:00 run: archive loop runs, 500-cap, T+24h unaffected) → PREPARE the 5K pilot command (do NOT execute) → 5K only on Maya's explicit approval.** Holds: `ARCHIVE_BACKLOG_DRAIN_ENABLED` stays unset until the pilot window | **P1** | Awaiting 03:00 UTC run | Claude (check) / Audit (verify) | Maya sequence 2026-07-03 |
| OPS-021 | **CODEX CHECK-BACK ALERT (Maya directive 2026-07-02 ~21:30 ET): Codex hit its review rate limit — re-trigger `@codex review` in a few hours** on every PR awaiting a verdict, then act per standing rules (merge on clean where pre-authorized). Awaiting verdicts as of this entry: **#472** (SELLER-001 report) @ `93dc97ff` — 5 rounds addressed, merge pre-authorized on clean · **#473** (SELLER-002 tracking) @ `aa931a7c` — 2 rounds addressed · **#467** (docs/registry) @ latest — 13+ rounds addressed, do-not-merge-until-clean per Maya. Re-trigger comments are already queued on each PR; if no verdict after the limit resets, post a fresh `@codex review`. Also still pending: OPS-020 03:00 UTC retention-run proof | **P1 (process)** | Awaiting limit reset — first action of the next session | Claude (re-trigger + act) | Maya directive 2026-07-02 |
...
| ID | Finding | Severity | Status | Owner | Evidence |
|---|---|---|---|---|---|
| OPS-001 | Contact funnel errors (06-28, ×2) — **Hypothesis H-001 DISPROVED 2026-07-02** (live info_schema: the suspect columns are NULLABLE; every NOT-NULL/no-default column is in the INSERT). Now tracked as **Hypothesis H-004**: same-day DB-connectivity incident cluster (correlates with 06-28 `25006` + `P1001` errors on idx-sync/lead-scoring). Zero contact errors since 06-28. Side-finding kept: `prisma/schema.prisma` declares those two `String[]` columns as required while the live DB has them nullable — schema↔DB drift to reconcile at next migration touchpoint | **P2** (downgraded — no recurrence, mechanism disproved) | Monitoring + Needs live test (H-004) | Claude (verify) | Live info_schema + Vercel logs 2026-07-02 |
