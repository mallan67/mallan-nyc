...
**Authority: the live authenticated Cotality API — not metadata artifacts, code, comments, or docs.** An INSTRUMENTED read-only authenticated probe was executed against `https://api.cotality.com/trestle` on **2026-07-21T06:22Z** (10 GET requests, token acquired once, zero writes): live `$metadata` + 5 live Property rows + scoped/unscoped Media for 3 live listings (`1177660623/RLS20104692`, `1177659553/RLS20104691`, `1177659552/RLS20104690`) + a **3-fetch** URL-rotation test (A now, B immediate, C after 60 s) + an aged-URL re-request. **Full request-level evidence is recorded** — per request: method, sanitized URL (query values redacted), HTTP status, duration, `content-type`/`date` and other headers, `@odata.nextLink`, and error payloads. Sanitized raw output: `docs/superpowers/specs/evidence/2026-07-21-live-cotality-contract-probe.json`. The committed `artifacts/metadata.xml` matched the live `$metadata` on every checked field and is demoted to corroboration only.

**Request log summary (all HTTP 200; `notes: []` = zero contradictions):** `$metadata`→200 (`application/xml`, 153 ms); `Property $top=5`→200 (`application/json`, 359 ms); 3× scoped + 3× unscoped Media→200 (174–220 ms); rotation B (06:22:00Z)→200, rotation C (06:23:00Z, +60 s)→200. No non-200, no error payloads, no `Retry-After`, no `nextLink` on the sampled galleries (all fit one page).
...
3. **URL rotation — the decisive finding, confirmed across 3 fetches:** the same listing's Media, re-fetched **immediately (B)** and again **after 60 s (C)**, returned **7/7 URLs different at origin+pathname level in BOTH comparisons** (B-vs-A: 7 exactDiff / 7 identityDiff / 0 equal; C-vs-A: identical result). `MediaURL` carries **no query string**; the rotating signature is embedded in the **path**. ⇒ Any URL-based change detection (exact OR normalized) is structurally unusable; identity must be MediaKey+revision (§3); downloads must use the current run's fresh URL (§4). This is the repeated-for-stability confirmation.
...
6. **Aged-URL fetchability — UNRESOLVED, fails closed (not concluded).** The original A `MediaURL`, re-requested ~60 s later, returned a generic `fetch failed`. This is **NOT proof of expiry**: the request used `redirect:'error'` + an `Authorization` header, so a redirect-to-CDN or an auth-header mismatch would produce the same error, and no fresh-URL control fetch was run with identical options. Per the "contradictions fail closed / no conclusion from one ambiguous sample" rule, this stays **unresolved** and is settled at Runbook Step 0 with a clean redirect-following control (fresh-URL vs aged-URL, identical options). The design is safe either way — it never relies on a stored URL and drives the R2 mirror to durability (§4, Phase 4).
...
3. **Aged stored-URL fetchability — UNRESOLVED (see A.2 item 6):** the one aged-URL data point (`fetch failed` under `redirect:'error'`+auth) is ambiguous (redirect/auth vs expiry) and is explicitly NOT concluded. Step 0 resolves it with a clean control: fetch a FRESH `MediaURL` and an AGED one with identical `redirect:'follow'`, no-auth-then-auth, and compare. Until resolved the design treats the un-mirrored proxy fallback as best-effort and prioritizes R2 mirroring.
