...
|---|---|
| UCBA Art. I §4 — RLS only accepts Exclusive Listings | External inventory never enters RLS. Submission to RLS is via the legacy upstream intermediary (not mallan.nyc), unchanged. |
...
>
> Yes. §1 acknowledges the competitor LMP B2B-only pattern as the reference posture. §3 notes the Phase 3 scraper requires written legal review of StreetEasy/Zillow ToS exposure as a gating criterion. §9.5 cross-references each REBNY/UCBA/NY DOS rule and explains how the design preserves it. Photo licensing per source is called out (§11). Owner-contact licensing is governed by the publishing platform's terms (cited in the disclaimer text §6.1).
