...
> **SUPERSEDED AS PLAN-OF-RECORD (2026-09-10) — retained as the record of the A1–A17 batch track.** Product and sequencing authority for search, listings and agent workflow is `MALLAN-PLATFORM-MASTER-PLAN.md` (repo root, canonical lineage PR #595 / `agent/publish-mallan-platform-master-plan-2026-08-04` (rank 1 in the authority order in `CLAUDE.md` / `AGENTS.md`), whose §5.1 separates Frontend Consumer Search from Backend Agent Search. The closing instruction below — "Next session should review §8 phase order and authorize the next batch" — is spent; do not open a batch from this document.
>
