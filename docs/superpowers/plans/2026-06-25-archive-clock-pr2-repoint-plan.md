...
**Compliance surface:** data-retention / §2.05 terminal removal / NY SHIELD §899-bb / NY DOS 6-year recordkeeping. Per `docs/compliance/COMPLIANCE-CANONICAL-INDEX.md` §5 (status/§2.05) + §14 (SHIELD retention) + §15 (audit retention). This PR does **not** change any §2.05 24-hour removal, SHIELD encryption, or DOS 6-year archive-table behavior — it only changes *which age clock* decides T+180 archive eligibility.
...
**Ordering safety property:** because every existing terminal row currently has `terminal_since = NULL` (verified: 0/110,114 set), the flag-on predicate `terminal_since < cutoff` matches **nothing** until the backfill runs. So even if the flag were flipped before the backfill, the drain size is 0 — fail-safe by construction.
...
- **NY SHIELD §899-bb / NY DOS 6-year:** the archive *target* (`listings_archive` summary + strip + `sync_status='archived'`, keeps the row for FK integrity) is unchanged; PR-2 only makes aging *more accurate*. Conservative residual handling (Q2) means we never archive a row whose terminal age is unknown.
