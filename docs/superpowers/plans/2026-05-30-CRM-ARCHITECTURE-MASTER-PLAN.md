...
**Goal:** Build the search-centric Mallan CRM as defined in the spec `docs/superpowers/specs/2026-05-30-listing-search-business-spine-master-plan.md`, one branch per subsystem, each isolated, journaled, and rule-bound.
...
1. **Each branch has its own plan file** in `docs/superpowers/plans/branches/`, named for the branch.
2. **To start or restart a branch:** open its file, read the **🛑 MANDATORY RULES** block at the top *every time*, read its journal (`ops/agent-journals/<slug>.*`), resume from the last completed step.
