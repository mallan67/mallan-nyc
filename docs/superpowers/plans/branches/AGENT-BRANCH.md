...
> **Git branch:** `feat/agent-search-shell` · **Journal:** `ops/agent-journals/02-agent-search.*` · **Index:** `docs/superpowers/plans/2026-05-30-CRM-ARCHITECTURE-MASTER-PLAN.md`
...
0. **RESUME FIRST.** Read `ops/agent-journals/02-agent-search.state.json` + tail journal. Resume after `last_completed_step`. Append journal + update state each action. Block → record, STOP, surface to Maya.
