...
> **Git branch:** `feat/broker-command-center` · **Journal:** `ops/agent-journals/04-broker.*` · **Index:** `docs/superpowers/plans/2026-05-30-CRM-ARCHITECTURE-MASTER-PLAN.md`
...
0. **RESUME FIRST.** Read `ops/agent-journals/04-broker.state.json` + tail `04-broker.journal.jsonl`. Resume after `last_completed_step`. Append journal + update state on every action. On block: `status:"blocked"`, record, STOP, surface to Maya.
