...
- **Worktree:** `p-unified-wt` on `agent/unified-feed-media-system`, `node_modules` symlinked to the primary checkout. Production build runs in the primary checkout at the branch HEAD (Turbopack rejects the symlinked worktree).
...
- [ ] **Step 1: Failing test** — with the flag ON (test-injected), a rotated-URL-only re-run performs ZERO writes; an empty-200 performs ZERO tombstones; the Media query string is Property-scoped and selects `MediaType`.
...
- [ ] **Step 1: Failing test** — source-scan: no second hero implementation remains (Similar/`/api/listings` Phase-1 call `selectHero`); `img.cotality.com` is in the proxy allowlist; batch emits `isPrimary`.
