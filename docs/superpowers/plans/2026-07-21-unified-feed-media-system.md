...
- **Live-proven facts to honor (probe 2026-07-21T06:22Z):** `MediaURL` rotates at origin+pathname on EVERY request → exclude it from all change comparison; `MediaKey` is the Media PK; `ListingKey` is the Property PK; `Order` is nullable; `PreferredPhotoYN` was null on every sampled row (treat null = not-preferred); plain enum-literal `$filter ResourceName eq 'Property'` and multi-key `$orderby Order asc,MediaKey asc` both return 200; `PhotosCount` is populated; classification signal is `MediaCategory` (runtime `Photo`/`FloorPlan` seen) while `MediaType` is the FILE FORMAT (`Jpeg`/`Pdf`), NOT the semantic type.
- **Live pagination facts (probe 2026-07-21, `…-pagination-findings.md`) — stated only to what 3 sampled pages show:** `@odata.nextLink` is returned on every sampled page (server page size 10), each followed page is 200, and 0 duplicate MediaKeys were observed in the sample. **NOT proven:** completeness or global no-skip (the probe did not reach the final page or reconcile against an independent total) — do not claim it. **CONTRADICTED on the MEDIA endpoint (fail-closed):** a bulk-import Media block shares ONE `ModificationTimestamp`, and within it the server does NOT order by `MediaKey` — so a keyset resume `ModificationTimestamp eq ts AND MediaKey gt lastKey` WOULD SKIP rows. **Rule:** completeness = an explicit `pageChainComplete` (nextLink drained to exhaustion); never treat one page as complete; the (ts,key) keyset is a coarse between-run bound only; the cursor advances only when `pageChainComplete` AND past a fully-drained timestamp, else it freezes. **The Property endpoint is UNVERIFIED** — the Media tied-ordering result is NOT applied to Property without a separate direct live Property probe; the Property cursor back-off is a defensive default only.
