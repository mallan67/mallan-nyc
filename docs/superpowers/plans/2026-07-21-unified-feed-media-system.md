...
- **Non-negotiable acceptance invariants (each MUST have a failing-first test; carry through Phase 2 Task 10 + Phase 6):** (1) **CRM tombstone carve-out** — feed reconciliation never tombstones `crm:`-prefixed (seller-uploaded) media rows; (2) **all-status detail-page distinction** — the reconciler/readers preserve the "never-imported vs all-deleted" signal (detail page reads ALL statuses; `_count` existence retained); (3) **floor-plan hero protection** — a FloorPlan/Document is never selected as hero on any surface (strict classifier + `selectHero` Photo-only).
...
- [ ] **Step 1: Failing test** — with the flag ON (test-injected), a rotated-URL-only re-run performs ZERO writes; an empty-200 performs ZERO tombstones; the Media query string is Property-scoped and selects `MediaType`. **Acceptance invariants: (a) a `crm:`-prefixed (seller-uploaded) row is NEVER tombstoned by feed reconciliation even when absent from the feed; (b) the all-status "never-imported vs all-deleted" distinction is preserved (readers still see tombstoned rows' existence via `_count`).**
