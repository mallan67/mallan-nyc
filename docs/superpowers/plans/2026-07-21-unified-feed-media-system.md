...
- **Live-proven pagination facts (probe 2026-07-21, `…-pagination-findings.md`):** server-driven paging via `@odata.nextLink` returns nextLink on every page (server page size 10), each page 200, 0 duplicate/0 skipped MediaKeys across pages → nextLink drain is complete + non-overlapping. **CONTRADICTED assumption (fail-closed):** a large bulk-import Media block shares ONE `ModificationTimestamp`, and within it the server does NOT order by `MediaKey` — so a keyset resume `ModificationTimestamp eq ts AND MediaKey gt lastKey` WOULD SKIP rows. **Rule:** completeness = follow nextLink to exhaustion (never treat one page as complete); the (ts,key) keyset is a coarse between-run bound only; a tied-timestamp block is drained fully in one run; the cursor advances only PAST a fully-drained timestamp (an incomplete drain freezes it). Applies to Media AND the Property cursor.
