...
| BIZ-006 | Business | Public search `total`/`hasMore` are wrong on **both** runtimes, by two different mechanisms: the DB path and the unbounded fallback branch count **before** display filtering and suppression; the bounds/borough/neighborhood fallback branch counts a **truncated candidate prefix** (`fetchTop` capped at 1,000). → incomplete pages, inflated *or* undercounted totals, wrong `hasMore`, phantom pages. Canonical entry (**BIZ-008 superseded**). Static-only, Evidence Score 3/10 — verify first | P1 | Open — unfixed; runtime PR not yet raised | Claude |
...
| Unbounded tables | 🟡 | 2026-07-25 | **Measured (PR #569, live pg_stat):** `sync_errors`=**0** (empty — NOT growing; corrects the old static note); `audit_events`: NON-exempt actions bounded by rolling 2yr retention (non-exempt bulk plateaus; rate 199→566/day post-`*/10`; 30 MB frozen burst purges ~2028) BUT `email_unsubscribed` is purge-exempt + create-only → **`audit_events` as a whole is NOT strictly bounded**. Genuinely unbounded: `listing_media` soft-deleted tombstones **28,664** (never purged) + the `email_unsubscribed` audit subset (exempt, create-only) (OPS-010/010A) | live pg_stat 2026-07-25 |
