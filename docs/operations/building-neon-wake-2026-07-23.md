...
Unit cards must show the BUILDING's ownership form — Condo / Co-op / Condop / Rental Building — never raw 'Apartment'. APPROVED rule: the displayed type is determined by the BUILDING ownership structure, never by the transaction type. One canonical classifier — classifyBuildingOwnership() (exported) — with priority: unit CommonInterest → building CommonInterest → unit OwnershipType → building OwnershipType; Condominium→Condo, Stock Cooperative→Co-op, Condop→Condop, Rental/Apartment Building→Rental Building. With NO ownership signal anywhere, a lease unit is by definition in a building with no individually owned units → Rental Building (never Apartment); a signal-less sale falls to the canonical mapper. All three card sites (Trestle active, Trestle closed/sale-history, DB active) route through it; the building header keeps its existing displayLabel pipeline whose four canonical mappings are identical. Tests: all seven required examples + four conflict cases on the classifier, plus payload scenarios (rental-in-co-op → Co-op, rental-in-condop → Condop, no-signal lease → Rental Building, rental building → Rental Building). Live-feed note: no rental-in-co-op/condop specimen currently exists in the gated feed — those two are test-proven; rental-in-condo and true rental building are also browser-verified on the preview. This is the ONE deliberate payload divergence from production; the parity harness strips exactly this field from deep-equal and pins the new mapping explicitly (sale/coming-soon → Condo; rental → Apartment).
