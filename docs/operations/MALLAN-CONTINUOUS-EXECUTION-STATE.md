...
```json
{
  ...
  "objective": "Ledger row 19 disposition (control-root maintenance). Add to scripts/ci/mallan-execution-control.mjs a bounded Master-amendment mode, with negative tests in tests/runtime/mallan-execution-control.test.ts, so that MALLAN-PLATFORM-MASTER-PLAN.md can change only through a base-authorized, Master-only packet. Each property must be proven by a test, and every new property by a test that fails against the current controller: (1) a Master amendment is authorized only by the BASE Execution State contract, never by the PR HEAD; (2) a PR cannot self-authorize a Master amendment by editing the Execution State; (3) an amendment PR may change MALLAN-PLATFORM-MASTER-PLAN.md only, and the Execution State, code, tests, workflows or provider, environment or database paths in the same PR fail; (4) implementation mode still refuses Master changes; (5) control-update and control-root-maintenance still refuse Master changes; (6) an unauthorized Master edit fails in every mode; (7) the amendment mode exits only through a state-only PR back to control-update; (8) the amendment mode requires the same live proof as control-root maintenance that authority-root is a required main check; (9) no weakening of authority-root, pr-check, release-truth, branch-authority, the database chain, the direct-Neon capability scan, the mutation-class guards or NONDELETABLE_CONTROL_ROOT_PATHS; (10) CONTENT PINNING IS MANDATORY: the BASE contract of every Master-amendment packet must carry a machine-readable content envelope with all five of: (a) the exact BASE Master blob SHA; (b) the exact permitted Master section heading, as the start anchor; (c) the exact section end boundary, derived from the next Master heading at the same level (a higher-level heading also closes the section), never from line numbers; (d) the exact digest of the authorized section before amendment; (e) the exact digest of the authorized section after amendment. Both section digests are SHA-256 (lowercase hex) computed over the exact Git-blob bytes of the section range with no normalization of any kind (no line-ending, whitespace, Unicode or encoding normalization): the range starts at the first byte of the start-heading line and ends immediately before the first byte of the closing heading line, or at end of file if there is none, so the start heading line is included and the closing heading line is excluded; the after-digest range is located the same way in the amended file. A packet missing any of the five fails, and a full-file post-amendment SHA may not be the only mechanism. The controller tests must prove: (t1) correct base Master, correct section and exactly the expected replacement passes; (t2) a wrong or stale base Master blob fails; (t3) a wrong section-before digest fails; (t4) a wrong section-after digest fails; (t5) an extra unintended edit elsewhere inside the authorized section fails; (t6) an edit outside the authorized section fails; (t7) the authorized section edit plus an edit to another Master section fails; (t8) heading or boundary manipulation (renaming, moving, adding or removing the start heading or the closing heading) fails; (t9) HEAD cannot change its own authorization; (t10) the Master plus the Execution State in the same amendment PR fails; (t11) the Master plus any second repository path fails; (t12) an equal-length byte substitution inside the authorized section fails, and so does a change that differs only in line endings or whitespace. Changes are limited to the two authorized paths. This packet changes no Master text and authorizes no provider, environment, Neon, Vercel, Cotality, database, documentation or branch mutation. It exits through a state-only PR back to control-update; the Master-only amendment of Master section 0.12 (live Vercel resource discovery) is authorized separately afterwards.",
  "impact_graph": {
    "root_owner_paths": [
