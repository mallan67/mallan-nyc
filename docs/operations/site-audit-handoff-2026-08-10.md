...
| **OPS-027** PHASE 4a sibling-revalidation race | **FIXED** — persistence-time transactional revalidation; listings whose siblings moved are dropped, deferred honestly, and excluded from cursor advancement. |
| **OPS-028** PHASE 4a reserved time slice | **FIXED** — Phase 3 stops at `phase2ReserveMs + sweepReserveMs`; reserve derived from the sweep worst case and clamped to a fifth of the post-finalize budget. Total budget unchanged. |
