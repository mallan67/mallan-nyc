...
> **Tooling defect found while doing this (not fixed here — this PR is documentation-only).** `scripts/health/probe.ts:55` resolves the SHA with `git rev-parse --short main`, i.e. the **local** ref. This worktree's local `main` was stale at `e113a1ef`, so the first probe run wrote `main e113a1ef` into the canonical dashboard — authoritative-looking and wrong. That output was discarded, local `main` was fast-forwarded to `origin/main`, and the probe re-run. **`probe.ts` should read `origin/main` (or fail loudly when the local ref is behind); it deserves its own issue and fix.**
