...
**Dashboard auto tier is STALE and was NOT refreshed.** `docs/PROJECT-HEALTH-DASHBOARD.md`'s `HEALTH:AUTO` block still reads *Last probed 2026-07-28T04:10:36Z* with `main ccfb4e85` and *2 open PRs (#581, #579)* — both now wrong. **`npm run health:probe` could not be run**: `tsx` is not installed in this documentation worktree. The block is generated and must not be hand-edited, so it is left untouched and its staleness is recorded here instead of being faked. **Refreshing it is a prerequisite for the next handoff.** The manually assessed runtime-error row WAS refreshed with the live capture above.
