...
- **Telemetry — Implemented on draft PR #569; pending review and merge (NOT in production):** media physical-write cause counters (`delivery_url_refreshed`, `suppressed_url_signature_rotation`, `suppressed_url_identity_changed`, `write_failures`) in `media_sync_cron`; flag-gated (`DIAG_RAW_DATA_KEYS_UNTIL`, auto-expiring) raw_data changed-key histogram to runtime logs (never audit_events).
...
- **Phase-2 = HELD.** No suppression of `PhotosChangeTimestamp` / URL identity changes; no retention / R2 / Neon / cadence / env / cron change made. Decision pending Maya from this record.
