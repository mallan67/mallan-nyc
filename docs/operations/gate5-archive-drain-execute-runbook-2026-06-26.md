...
8. **CRM smoke:** the archived listing's **live row is kept**, so FK integrity holds and CRM detail/history should still resolve the listing **directly from `prisma.listing`**. Verify the preserved **scalar** fields remain visible where CRM expects them — status, address, price, beds/baths, dates, typed agent columns, and related `PriceHistory`/`Showing`/`MarketingActivity` links. Do **not** expect `raw_data`/`media`/`compliance`-derived rich content after archive; those are intentionally stripped/emptied. **There is no `listings_archive` CRM fallback today** — `listings_archive` is recordkeeping only; operators should not expect CRM to reconstruct archived detail from it.
