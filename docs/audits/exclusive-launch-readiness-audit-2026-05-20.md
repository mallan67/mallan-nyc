...
Four Class-A frontend / compliance blockers must close before the new exclusive can take paid-social traffic on mobile.
...
| **Frontend visual + UX (mobile 390 + desktop 1440)** | ✗ **FAIL** | Listing detail page horizontally overflows on every mobile device (scrollX = 1266 px masked only by `body { overflow-x: hidden }`); no 44×44 contact CTA above the fold on mobile; intent-routed contact form is not implemented on main; FARE Act disclosure does not render on actual production rental listings. |
...
| **Compliance — FARE Act on rendered rental detail** | ✗ **FAIL** | Live probe of `/listing/815-5th-avenue-apt-duplex-...rls20091223` shows the LL 119/2024 disclosure not rendered. Source-grep in `app/listing/[id]/page.tsx:1546-1555` finds the code, but it is not firing for production rentals — a rendering-conditional gap, not a missing file. |
...
| **A4** | **FARE Act disclosure (NYC LL 119/2024) NOT rendered on actual production rental listings** — frontend-auditor verified missing on live `/listing/815-5th-avenue-apt-duplex-...rls20091223`. The disclosure code at `app/listing/[id]/page.tsx:1546-1555` exists but the conditional rendering is not firing for this listing. **NYC DCWP penalty schedule: §20-699.21 $1,000–$1,800, §20-699.22 up to $2,000 per violation.** | frontend-auditor live URL probe; tristle agent confirmed code presence but did not verify rendering | **A** | Compliance + frontend rendering |
