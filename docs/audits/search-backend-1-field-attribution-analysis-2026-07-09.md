...
Coming-Soon is display-permitted **with a required badge**, max 14 days, **no DOM accrual**, no showings before `ActivationDate`. Participant-Only = `Permission = 'Private'`. Owner-Opt-Out is enforced via `Permission`, **not** a phantom `OwnerOptOut` member. The flat 8-bucket enum does **not** yet encode DOM-accrual or audience-scope rules — see §strategic gap 15 and the UCBA "Off-Market" label prohibition.
...
|---|---|---|
| `live-truth.ts` | **KEEP (high value)** | The Cotality-verified enum ground (11 `StandardStatus` members + live counts, `Residential`/`ResidentialLease`, `StockCooperative`/`Condominium`/`Condop`, `MLS_STATUS_FILTERABLE=false`, `DEAD_OR_INVALID_VALUES`). This is exactly the "Cotality = source of truth" registry the contract needs. Re-verify counts are current before relying on them [needs probe], but the structure is right. |
