> **STATUS HEADER (added 2026-09-10) — findings preserved; the remedy below has already been executed, and two of the four audited files no longer exist.** This is the dated read-only audit of 2026-09-09. §5–§7 were carried out on 2026-09-09/10: the surviving listing editors — `public/crm/SALE-FORM-REDESIGN.html` and `public/crm/RENTAL-FORM-REDESIGN.html` — now rebuild their status select from `GET /api/crm/status-options` at load and fail closed to corrected hardcoded markup.
>
> **Correction to D1’s file list (verified 2026-09-10).** The other two forms this audit inventories, `public/crm/SALE-FORM-WITH-TOOLS.html` and `public/crm/RENTAL-FORM-WITH-TOOLS.html`, were **deleted** on 2026-09-09 in commit `67b13219` ("DELETE the WITH-TOOLS form forks — one Sale editor, one Rental editor, no copy left to revive"). Neither file exists on disk or in `git ls-files`. Every finding scoped to them is moot, including the WITH-TOOLS half of D6a. The prescribed equivalence test shipped as `tests/runtime/four-listing-forms-status-select-equivalence.test.ts` but now covers the **two** surviving forms, not four — it is green at **9 cases** (re-run 2026-09-10), not the 18 an earlier note claimed; `tests/runtime/listing-forms-status-write-roundtrip.test.ts` is green at 11.
>
> **Read `docs/status/crm-audit-2026-09-09/repair/listing-forms.md` before acting on anything here** — it also records an incident (uncommitted status-facts work in `public/crm/SALE-FORM-REDESIGN.html` destroyed by a `git checkout`) and a seven-item open handoff. Two of those items re-confirmed still open 2026-09-10: D6a `STATUS_TRANSITIONS` drift (still present at `public/crm/SALE-FORM-REDESIGN.html:8813`) and the fact that `app/api/crm/status-options/route.ts` still does not project `transitions` / `canonicalTransitions`.
>
> Architecture note: the sale and rental listing editors belong to the **Backend Agent Search / Listings** application — `CLAUDE.md` §A.0 lists "sale + rental listing editors" among what it owns — not to the Brokerage CRM (`public/crm/dashboard.html` + `js/dashboard/**`, served at `/crm`). The editors are served at their own routes, `/crm/sale-listing` and `/crm/rental-listing` (`vercel.json:76–77`); Backend Agent Search itself is `public/crm/index.html` → generated `index-built.html` at `/crm/search`. See Master Plan §5.1.
