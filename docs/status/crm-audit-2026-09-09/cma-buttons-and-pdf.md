> **STATUS HEADER (added 2026-09-10) — findings preserved; most of the fix plan has already been executed.** This is the dated read-only audit of 2026-09-09. The recommended Option B was carried out on 2026-09-09/10: the seven dead Comparables buttons are wired (`openCompReport`), and D9’s labels now read "Print / Save as PDF" in `public/crm/html/modals/report-preview.html:25` and `public/crm/html/modals/reports.html:181`. **Still genuinely open, and blocked on Maya, not on effort:** D11 (`public/crm/js/output/report-package.js` is still neither `<script>`-registered in `public/crm/index.html` nor deleted — a source-of-truth-charter deletion), D10 (`public/crm/html/modals/client-delivery.html` is still an unbuilt orphan), D12 and D14. D10 and D11 re-confirmed open 2026-09-10. What was done and what was consciously refused is recorded in `docs/status/crm-audit-2026-09-09/repair/reports-and-pdf.md` and `.../repair/search-cma-nav.md`. **Read those two before acting on any row of §2.** Architecture note: the surfaces audited here (`public/crm/index.html` → generated `index-built.html`, `js/output/**`, `js/search/**`) belong to the **Backend Agent Search / Listings** application, served at **`/crm/search`** — not to the Brokerage CRM (`public/crm/dashboard.html` + `js/dashboard/**`, served at `/crm`). See `CLAUDE.md` §A.0 and Master Plan §5.1.
