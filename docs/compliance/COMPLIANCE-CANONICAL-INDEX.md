...
>
> **Authority order.** Per-area rows below are the authoritative pointers. The canonical files cited are the source of truth. The skill at `.claude/skills/rebny-compliance/SKILL.md` is the leaner mirror that auto-loads at session start; when the two disagree, the canonical file wins. ⚠ **`SKILL.md` is NOT tracked in git** (`git ls-files .claude` → 0 files; `.gitignore:155` is `.claude/*`; verified 2026-08-19, re-verified 2026-08-20). It exists only in a local working copy, so **every pointer into it is unresolvable in a fresh clone, in CI, and in a git worktree** — which is a different defect from a wrong path, and must never be reported as one. It may therefore be cited as *backup/mirror* only — **never as Canonical**. Any rule that exists only in `SKILL.md` is, for repo purposes, undocumented, and demoting it does not delete it: where this index moves a Canonical pointer off `SKILL.md`, the replacement must carry the SAME rule, and any rule the replacement does not carry is listed as an open gap in that row. Section anchors below (§N) refer to its `## SECTION N` headings. **This note is carried on this line deliberately: it adds no line to this file, because other files cite rows here by line number (see the Maintenance protocol at the foot of this file).**
...
| **Backup** | `.claude/skills/rebny-compliance/SKILL.md` §2 (← corrected 2026-08-19: was §3, which is IDX Plus/Trestle, not UCBA); `compliance/rules/ucba-audit-checklist.json` (machine-readable checklist of all 145 rules) |
...
| | |
|---|---|
| **Canonical** | `compliance/IDX-VOW-DISPLAY-RULES.md:25` “6 Distribution Gates” — tracked, and it carries the SIX GATES ONLY; `lib/idx/trestle-mapper.ts` (the writer-side implementation — `TERMINAL_STATUSES`, `normalizeStandardStatus`, `computeGateColumns` post-PR-#165); `memory/IDX-PLUS-DISPLAY-GATE-2026-04-30.md` (tracked) is CO-CANONICAL and governs NULL-HANDLING per gate. ⚠ **RULE-CHANGE CORRECTION 2026-08-20.** On 2026-08-19 this cell was repointed (from the untracked `.claude/skills/rebny-compliance/SKILL.md` §2) to `IDX-VOW-DISPLAY-RULES.md` and presented as a tracked-vs-untracked pointer repair only. It was not only that. The mirror (`SKILL.md` §2.1) marks Gate 3 (`InternetEntireListingDisplayYN`) and Gate 4 (`InternetAddressDisplayYN`) **fail-OPEN (`!== false`)** and explains why in its §2.1.1; `IDX-VOW-DISPLAY-RULES.md` contains no two-tier rule at all and states a blanket **“Fail closed = NON-DISPLAY”** at `compliance/IDX-VOW-DISPLAY-RULES.md:10`. Making that file the sole canonical authority therefore silently reinstated the exact reading that corrupted 7,594 rows on 2026-04-30 and contradicted this index's own §7 Fail-closed row. The gate LIST is canonical there; the blanket fail-closed line at :10 is **SUPERSEDED for Gates 3 and 4** and must not be applied to them. Open gap: that §2.1.1 evidence narrative is not yet mirrored into a tracked file beyond the incident report. |
...
| | |
|---|---|
| **Canonical** | `lib/idx/auth.ts` (OAuth2 client_credentials, token cache, 8s timeout); `lib/idx/fetch.ts` (OData fetch + pagination + AbortController + retry); `lib/idx/trestle-mapper.ts` (the mapper); `compliance/THIRD-PARTY-AND-FEED-GOVERNANCE.md:40` — “⚠️ TRESTLE MEDIA API RULES — VENDOR-CONFIRMED (2026-04-07)” (← **corrected 2026-08-19**: previously cited `.claude/skills/rebny-compliance/SKILL.md` “Trestle Media API Rules §4”, a heading that has never existed in that file — its §4 is Fair Housing — and the file is untracked) |
...
| | |
|---|---|
| **Canonical** | `compliance/THIRD-PARTY-AND-FEED-GOVERNANCE.md:40` — “⚠️ TRESTLE MEDIA API RULES — VENDOR-CONFIRMED (2026-04-07)”, the 4-rule table + field-mapping reference (← **corrected 2026-08-19**: previously cited `.claude/skills/rebny-compliance/SKILL.md`, which contains no such heading and is untracked; under the fail-closed default above that made every media decision a STOP); `lib/idx/sync.ts` (batch media sections); `lib/idx/fetch.ts:fetchListingMedia` |
...
| | |
|---|---|
| **Canonical** | `compliance/NYC-NYS-REQUIREMENTS.md:17` (§175.25), `:30` (§175.28), `:36` (§175.12) — tracked (repointed 2026-08-19 from the untracked mirror `.claude/skills/rebny-compliance/SKILL.md` §6, now backup only; the three cited sections are carried in full, verified line-by-line 2026-08-20); `app/components/Footer.tsx`; `app/components/AntiDiscriminationNotice.tsx`; lead-capture endpoints. ⚠ **OPEN GAP (2026-08-20):** (`SKILL.md` §6) also carried §6.3 (§175.29 Fair Housing in advertising) and §6.5 (the “Off-Market” language prohibition). `NYC-NYS-REQUIREMENTS.md` carries neither (0 hits for `175.29` and for `off-market`), so both rules are, for repo purposes, UNDOCUMENTED and are enforced only by `npm run compliance-check`. Do not treat their absence here as their repeal. |
...
| | |
|---|---|
| **Canonical** | **The data-retention schedule in the Fail-closed row below is the authority for this area** (corrected 2026-08-19); `app/api/cron/data-retention/route.ts` (enforcement). ⚠ Do **not** take the retention figures from the mirrors (`compliance/DATA-LIFECYCLE-POLICY.md` §1 and `.claude/skills/rebny-compliance/SKILL.md` §8.5 are backup/mirror only). `DATA-LIFECYCLE-POLICY.md` was corrected 2026-08-20 in the same pass as this row; the skill mirror (`SKILL.md` §8.5) still prints an uncorrected NY DOS figure and cannot be corrected from a worktree because the file is gitignored — that correction is owed in the main tree. |
| **Backup** | `.claude/skills/rebny-compliance/SKILL.md` §8.1 (SHIELD Act) + §8.5 (retention table) (← corrected 2026-08-19: was §1); `prisma/schema.prisma` (model timestamps that gate retention) |
...
| **Fail-closed** | NY SHIELD §899-bb. Encrypted-at-rest (Neon TDE). Retention schedule: **NY DOS 19 NYCRR 175.23 = THREE years**, verbatim “*Each licensed broker shall keep and maintain for a period of three years*” — scope: residential 1–4 family / condo / co-op **sales**; enumerated records are (1) names+addresses of seller and buyer, (2) the purchase contract or the price + deposit, (3) commission paid, (4) gross profit if bought for resale, (5) **any document required under RPL Article 12-A**, (6) the listing / commission / buyer-broker agreement. Evidence: verbatim capture pinned at `.cache/closure3/r2-final/legal/19-NYCRR-175.23-VERBATIM.md` (sha256 31ac0e51…), two independent sources. **SUPERSEDED 2026-08-19:** this row previously read “Listing data + agreements 6 years; transaction records + commissions 6 years”; that “6 years” has no basis in the text of 175.23 and is withdrawn. 175.23 makes **no** mention of photographs, listing images, or any binary media, so it imposes **no** retention duty on mirrored Cotality photo bytes in R2. Any window longer than three years must cite **its own** authority in this row (e.g. IRS 26 USC 6001 for financial records) and may **not** be attributed to the Department of State. Continuing, on their own authorities: audit event logs 2 years (REBNY RLS — see §15 for the ONE narrow operational-diagnostic exception); Trestle/IDX access logs 12 months (REBNY RLS data licence); lead PII inactive 3 years then archive (NY SHIELD); session tokens 24 hours auto-expire; closed listing display removed within 24 hours (REBNY §2.05). |
...
2. **Update an existing area** — edit the row's canonical file (or the row itself if file paths change). Bump the "last verified" date in the row's last cell if you're changing the rule rather than the file location. **Line-number stability (added 2026-08-20).** Other files cite rows in THIS file by line number (`docs/compliance/COMPLIANCE-CANONICAL-INDEX.md:NN`). An edit here MUST NOT change this file's line count or move a cited row: edit a row in place, on its own single line. On 2026-08-19 a two-line note was inserted at the top of this file and shifted every row below it by +2, silently breaking `lib/idx/one-cycle-preflight.ts:322` and `tests/runtime/one-cycle-execution-plan.test.ts:14`, both of which cite `:118` as the §8 Fail-closed row. Enforced by `.cache/closure3/r2-final/check_pointers.py` test R.
