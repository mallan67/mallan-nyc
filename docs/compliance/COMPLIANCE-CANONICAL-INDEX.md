...
| | |
|---|---|
| **Canonical** | `lib/idx/trestle-mapper.ts:610-720` (`TERMINAL_STATUSES` set + `normalizeStandardStatus()` function); `lib/idx/trestle-mapper.ts:722-870` (`computeGateColumns` helper — post-PR-#165) |
...
| **Validator** | `lib/compliance/__tests__/c2-terminal-idx-display.test.ts`; `tests/runtime/h1-dual-write-tier1.test.ts`; `tests/runtime/projection-dual-write-tier2.test.ts`; `tests/runtime/listing-writer-projection-coverage.test.ts` (post-PR-#165) |
...
| | |
|---|---|
| **Canonical** | `lib/idx/trestle-mapper.ts:722-870` (`computeGateColumns` helper, post-PR-#165) — single source of truth for `idx_display_yn`, `internet_entire_listing_display_yn`, `internet_address_display_yn`, `internet_automated_valuation_display_yn`, `internet_consumer_comment_yn` |
...
| **Validator** | `lib/compliance/__tests__/compliance-gates.test.ts`; `lib/idx/__tests__/compute-gate-columns.test.ts` (post-PR-#165); `scripts/ci-compliance-check.js` regression guards (lines 471-507) |
...
| **Backup** | `lib/auth/log-audit-event.ts` (the canonical write helper); pattern enforced in every CRM mutation, status PATCH, listing-create, deal-create, commission-update, projection-reconcile |
