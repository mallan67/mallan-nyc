...
const DATASET = process.env.SODA_DATASET_DOB_PERMITS!;
...
export async function GET(req: Request) {
  try {
    ...
    const items = await (soda as any)({ resource: DATASET, where: where.join(' AND '), limit: 100, order: 'job_filed_date DESC' });
