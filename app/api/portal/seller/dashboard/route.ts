...
/**
 * GET /api/portal/seller/dashboard
 * Returns seller dashboard data: listing stats, recent activity.
...
export async function GET(request: NextRequest) {
  ...
  const listing = await prisma.listing.findFirst({
    where: { listing_id: lead.active_sale_listing_id },
    select: {
  ...
  const [showingsCount, offersCount, inquiriesCount, activity] = await Promise.all([
    prisma.showing.count({
      where: { listing: { listing_id: lead.active_sale_listing_id } },
    }),
    prisma.portalEvent.count({
      where: { lead_id: lead.id, workspace: 'seller', event_type: 'offer_view' },
    }),
    prisma.portalEvent.count({
      where: { listing_id: lead.active_sale_listing_id, event_type: 'listing_view' },
    }),
    prisma.portalEvent.findMany({
      where: { lead_id: lead.id, workspace: 'seller' },
