...
export async function POST(
  ...
  if (body.signature_id) {
    try {
      ...
      return NextResponse.json({ item: { ...sig, id: sig.id.toString(), document_id: sig.document_id.toString() },
      });
    } catch (err: unknown) {
  ...
  if (!signer_name || !signer_email || !signer_role) {
    return NextResponse.json({ error: "signer_name, signer_email, signer_role required" }, { status: 400 });
  }
