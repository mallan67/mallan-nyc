...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  ...
  if (body.IDXEntireListingDisplayYN !== undefined) {
    // H1 fix (2026-05-13): close the secondary-writer §2.05 gap. An agent
    // editing a listing whose effective status is terminal MUST NOT be able
    // to set `idx_display_yn=true`. Effective status = the merged update's
    // MlsStatus if the body changes it, else the existing listing's status —
    // identical to the resolution the RLS enforcement gate already uses at
    // line 112 above (`merged.MlsStatus || listing.status`). Reuses the C2
    // canonical TERMINAL_STATUSES set so writer and cron stay aligned.
    const effectiveStatus = String((merged.MlsStatus as string | undefined) || listing.status || "");
