 ...
 *
 * Commands:
 *   backup  <search|sale|rental|all>   Create timestamped backup before editing
 *   validate <search|sale|rental|all>  Verify CRM integration contract after changes
...
function resolveTargets(key) {
  ...
  console.error(C.red(`Unknown target: "${key}". Use: search, sale, rental, or all`));
  process.exit(1);
}
...
function universalChecks(content, filePath) {
  ...
  // 4. Agent identity initialization (initLoggedInAgent function OR LOGGED_IN_AGENT object)
  results.push(check(
...
if (!command) {
  ...
  ${C.cyan('Commands:')}
    backup   <search|sale|rental|all>   Create timestamped backup
    validate <search|sale|rental|all>   Verify CRM integration contract
