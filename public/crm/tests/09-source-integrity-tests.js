...
function SourceIntegrityTests(options) {
    ...
    (function() {
        if (typeof listings === 'undefined' || listings.length === 0) { addResult('SRC-02', 'Enum Token Validity', 'SKIP', 'No listings loaded yet (waiting for API). Re-run after data loads.'); return; }
    ...
    (function() {
        if (typeof listings === 'undefined' || listings.length === 0) { addResult('SRC-03', 'Compliance Flags Fail-Closed', 'SKIP', 'No listings loaded yet (waiting for API). Re-run after data loads.'); return; }
