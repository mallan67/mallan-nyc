        ...
        function _serverSearch(criteria) {
            ...
            MallanAPI.idx.search(params).then(function(result) {
                ...
                // Rewriting the page downward is only justified by a proven total. From an uncertain one it
                // would silently move the agent off a page the server may well be able to serve.
                if (_countHasExactPageCeiling()) {
                ...
                // An empty LATER page used to be invisible: the downward clamp moved the agent off it before
                // anything rendered. With the clamp gated on an exact count, say what happened — and claim
                // only that, never that it was the last page unless the count proves one.
                else if (rows.length === 0 && page > 1) {
                    showToast(_countHasExactPageCeiling()
                        ? 'No more listings — page ' + page + ' is past the last page of results.'
                        : 'No listings returned for page ' + page + '.', 'info');
                }
            }).catch(function(err) {
        ...
        /**
         * May serverTotal be used as a HARD navigation ceiling?
         *
         * Only when the count is exact. The engine's four states (lib/search/engine/universe.ts) say what
         * the total is worth: a `lower_bound` means there may be MORE, so capping navigation at
         * ceil(total/perPage) makes real later pages unreachable — the toolbar reads "At least 100 Results"
         * while Next refuses to pass page 2, and the screen contradicts itself. An `upper_bound` ceiling may
         * sit past the real end, and `indeterminate` justifies no ceiling at all.
         *
         * When there is no exact ceiling the SERVER response decides whether a page has rows. The browser
         * does not get a second membership engine, and it does not invent a last page. A null meaning is
         * non-exact too — fail closed to the old behaviour until the server has answered.
         *
         * This is the one browser expression of the rule; pagination.js's _lastPageIsKnown() delegates here.
         */
        function _countHasExactPageCeiling() {
            return !!searchResultsState && searchResultsState.serverPaged
                && searchResultsState.serverTotal != null
                && searchResultsState.serverCountMeaning === 'exact';
        }
        window._countHasExactPageCeiling = _countHasExactPageCeiling;

        window.goToServerPage = function(n) {
            if (!searchResultsState || !searchResultsState.serverPaged) return false;
            var perPage = Math.min(Math.max(parseInt(searchResultsState.perPage, 10) || 50, 1), 200);
            // Floor always; ceiling only from a proven total.
            n = Math.max(parseInt(n, 10) || 1, 1);
            if (_countHasExactPageCeiling()) {
                var totalPages = Math.max(1, Math.ceil((searchResultsState.serverTotal || 0) / perPage));
                n = Math.min(n, totalPages);
            }
        ...
        /**
         * ONE interpretation of the server's countMeaning, for the sentence AND the page total.
         *
         * The engine answers with four states (lib/search/engine/universe.ts): `exact`, `lower_bound` (the
         * provider walk did not complete — there may be MORE), `upper_bound` (rows were counted and then
         * removed downstream — there are FEWER) and `indeterminate` (both at once). This browser used to
         * recognise only `lower_bound` and printed everything else as a bare "N Results", so the two states
         * added precisely because a count can be wrong in the other direction were shown as certainty.
         *
         * `indeterminate` deliberately gets neither "at least" nor "at most": when neither direction is
         * guaranteed, claiming either is a third wrong answer.
         *
         * `pageMark` marks the derived "Page X of Y". Y comes from an uncertain total, so it may understate
         * pages (lower_bound) or invent them (upper_bound). The browser carries no `hasMore`, so the honest
         * correction is to stop stating Y as a certainty rather than to guess a real last page.
         */
        function _countMeaningPresentation(meaning) {
            switch (meaning) {
                case 'lower_bound':   return { prefix: 'At least ', suffix: ' Results', pageMark: '+',  exact: false };
                case 'upper_bound':   return { prefix: 'At most ',  suffix: ' Results', pageMark: '≤', exact: false };
                case 'indeterminate': return { prefix: '', suffix: ' currently counted', pageMark: '~', exact: false };
                default:              return { prefix: '', suffix: ' Results', pageMark: '',  exact: true };
            }
        }
        window._countMeaningPresentation = _countMeaningPresentation;

        /** The server's meaning, but only when the server actually produced this count. */
        function _serverMeaning() {
            return (searchResultsState.serverPaged && searchResultsState.serverTotal != null)
                ? searchResultsState.serverCountMeaning
                : null;
        }

        function _resultsCountLabel() {
            var count = _resultsCount();
            var p = _countMeaningPresentation(_serverMeaning());
            return p.prefix + count.toLocaleString() + p.suffix;
        }
        ...
        function updateResultsCount() {
            ...
            // "Page X of Y" derived from an inexact total is a false certainty: an upper_bound manufactures
            // pages that may not exist and a lower_bound hides pages that do. The figure still shows —
            // it is the best available — but marked so it never reads as exact.
            var meaningPres = _countMeaningPresentation(_serverMeaning());
            var totalPagesText = meaningPres.pageMark + totalPages;
