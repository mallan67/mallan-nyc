        ...
        function toggleSearchV2() {
            ...
            var generalSearch = document.getElementById('generalSearchSection');
            var comparablesSection = document.getElementById('comparablesSection');
            var toggleBtn = document.getElementById('btnSearchV2Toggle');

            if (_searchV2Active) {
                // Switch to V2 — hide old content but keep toggle visible
                if (searchTypeTabs) searchTypeTabs.style.display = 'none';
                if (generalSearch) generalSearch.style.display = 'none';
                if (comparablesSection) comparablesSection.style.display = 'none';
                if (v2Container) {
                    v2Container.style.display = 'block';
                    v2Container.innerHTML = '<div style="text-align:center;padding:40px;"><i class="fas fa-spinner fa-spin text-3xl text-amber-600"></i><p style="margin-top:12px;color:#6B7280;">Loading new search...</p></div>';

                    // Load V2 scripts dynamically
                    if (typeof SearchV2 === 'undefined') {
                        _loadSearchV2Scripts().then(function() {
                            v2Container.innerHTML = '<div id="searchV2Form"></div><div id="sv2-filter-bar-container" style="display:none;"></div>';
                            return SearchV2.init('#searchV2Container');
                        }).then(function() {
                            console.log('[SearchV2] Ready');
                        }).catch(function(err) {
                            console.error('[SearchV2] Failed:', err);
                            v2Container.innerHTML = '<div style="text-align:center;padding:40px;color:#EF4444;"><i class="fas fa-exclamation-triangle text-3xl"></i><p style="margin-top:12px;">Failed to load new search: ' + (err.message || err) + '</p><button onclick="toggleSearchV2()" style="margin-top:12px;padding:8px 16px;border:1px solid #D1D5DB;border-radius:8px;cursor:pointer;">Back to Classic Search</button></div>';
                        });
                    } else if (!SearchV2.isInitialized()) {
                        v2Container.innerHTML = '<div id="searchV2Form"></div><div id="sv2-filter-bar-container" style="display:none;"></div>';
                        SearchV2.init('#searchV2Container').catch(function(err) {
                            console.error('[SearchV2] Init failed:', err);
                            v2Container.innerHTML = '<div style="text-align:center;padding:40px;color:#EF4444;">Failed to initialize. <button onclick="toggleSearchV2()" style="margin-top:8px;padding:6px 12px;border:1px solid #D1D5DB;border-radius:6px;cursor:pointer;">Back to Classic</button></div>';
                        });
                    }
                }
                if (toggleBtn) {
                ...
                if (generalSearch) generalSearch.style.display = '';
                if (comparablesSection) comparablesSection.style.display = '';
                if (toggleBtn) {
