        ...
        function collectSearchCriteria() {
            ...
            // Quick Search (listing ID / ZIP / unit) and keyword, FROM CANONICAL STATE.
            //
            // This read a second, private id table — and every Basic id in it was
            // WRONG. `searchQuickRls`, `searchQuickZip`, `searchQuickUnit` and
            // `searchKeyword` do not exist in the markup: the real controls are
            // per-workflow (`saleQuickRls` / `rentalQuickRls` / `buildingQuickRls`,
            // and `saleKeywordSearch` / `rentalKeywordSearch` /
            // `buildingKeywordSearch`). `getElementById` returned null every time,
            // so an agent could type a listing ID, ZIP, unit or keyword into a
            // Basic form and it silently never reached the wire.
            //
            // A second id table is what allowed that: it could be wrong without
            // anything disagreeing with it. These now come from the canonical
            // state, whose adapters carry the real ids and are checked against the
            // markup by test.
            var _qs = canonicalCriteriaFor(currentSearchTab) || {};
            ...
            if (_qs.public_remarks_keyword) criteria.keyword = _qs.public_remarks_keyword;
