        ...
        //
        // ONE criteria object per workflow. Basic and Advanced are two VIEWS of
        // it; neither is the authority.
        //
        // WHAT THIS FIXES. Showing exactly one Basic surface stopped the collector
        // reading a hidden container, but Basic and Advanced remained two separate
        // DOM stores: the collector chose ids by (tab, isAdvanced), and the two
        // sets never met. Type a rent range in Basic, open Advanced, press Search
        // while Advanced is still open, and the EMPTY Advanced controls executed —
        // the same substitution as before, one layer up.
        //
        // NOT A COPY BETWEEN FORMS. Values never move control-to-control. They
        // move DOM -> canonical state -> DOM: the active view is read INTO the
        // canonical object when it is left, and the canonical object is RENDERED
        // into a view when it is entered. That is what makes them views rather
        // than stores, and it is why switching tabs cannot leak — each workflow
        // holds its own object.
        //
        // COVERAGE IS DECLARED, NOT ASSUMED. Only the criteria listed below have
        // divergent per-view controls and therefore need carrying. Every other
        // control lives in exactly one surface, and since exactly one surface is
        // visible, reading it is already unambiguous. A criterion that later gains
        // a second view MUST be added here, or it silently reverts to being
        // view-local — which is the defect this table exists to close.
        var CRITERION_VIEW_BINDINGS = {
            list_price: {
            ...
            // Building facts diverge through a THIRD idiom — _resolveBuildingFieldIds(tab, isAdvanced)
            // — which is why the first pass missed them entirely.
            year_built: {
                sale:     { basic: ['saleBuildingMinYear', 'saleBuildingMaxYear'], advanced: ['adv-year-built-from', 'adv-year-built-to'] },
                rent:     { basic: ['rentalBuildingMinYear', 'rentalBuildingMaxYear'], advanced: ['adv-year-built-from', 'adv-year-built-to'] },
                building: { basic: ['buildingMinYear', 'buildingMaxYear'], advanced: ['adv-year-built-from', 'adv-year-built-to'] }
            },
            units_total: {
                sale:     { basic: ['saleBuildingMinUnits', 'saleBuildingMaxUnits'], advanced: ['adv-bldg-units-min', 'adv-bldg-units-max'] },
                rent:     { basic: ['rentalBuildingMinUnits', 'rentalBuildingMaxUnits'], advanced: ['adv-bldg-units-min', 'adv-bldg-units-max'] },
                building: { basic: ['buildingMinUnits', 'buildingMaxUnits'], advanced: ['adv-bldg-units-min', 'adv-bldg-units-max'] }
            },
            stories_total: {
                sale:     { basic: ['saleBuildingMinFloors', 'saleBuildingMaxFloors'], advanced: ['adv-floors-min', 'adv-floors-max'] },
                rent:     { basic: ['rentalBuildingMinFloors', 'rentalBuildingMaxFloors'], advanced: ['adv-floors-min', 'adv-floors-max'] },
                building: { basic: ['buildingMinFloors', 'buildingMaxFloors'], advanced: ['adv-floors-min', 'adv-floors-max'] }
            }
        };

        /** Per-workflow canonical criteria. Separate objects, so tabs cannot leak. */
        var _canonicalCriteria = { sale: {}, rent: {}, building: {} };

        /** Which view is on screen for the purposes of binding lookup. */
        function activeViewName() {
            return isAdvancedViewVisible() ? 'advanced' : 'basic';
        }

        function _bindingIds(key, tab, view) {
            var perTab = CRITERION_VIEW_BINDINGS[key] || {};
            var perView = perTab[tab] || {};
            return perView[view] || [];
        }

        /**
         * Read the view the agent has been editing INTO the canonical object.
         * Called before any view or tab change, and before collecting.
         */
        function syncActiveViewToCanonical(tab, view) {
            ...
            if (!store) return;
            Object.keys(CRITERION_VIEW_BINDINGS).forEach(function (key) {
                var ids = _bindingIds(key, tab, view);
                ...
                // That is the only case where a read is ignored — and it is what
                // the "all values empty" guard was really trying to express.
                if (!elements.some(function (el) { return !!el; })) return;

                var values = elements.map(function (el) {
                    return el && el.value != null ? String(el.value) : '';
                });

                // A DELIBERATELY EMPTIED control removes the criterion.
                //
                // This previously refused to write when every value was empty,
                // which silently RESURRECTED filters: an agent searching
                // $3,000-$7,000, then setting both price controls back to Any,
                // had the empty read ignored and the stored $3,000-$7,000
                // rendered straight back. Mallan restored a criterion the agent
                // had just removed, and the results looked correct.
                //
                // Absence of a CONTROL and emptiness of a control are different
                // facts; only the first is silence.
                if (values.some(function (v) { return v !== ''; })) store[key] = values;
        ...
        function renderCanonicalToActiveView(tab, view) {
            ...
            Object.keys(CRITERION_VIEW_BINDINGS).forEach(function (key) {
                ...
                ids.forEach(function (id, i) {
                    ...
                    if (!el) return;
                    var next = values[i] != null ? values[i] : '';
        ...
        function collectSearchCriteria() {
            ...
            // tab, so a Rental or Building search was scoped to the Sale form.
            var activeBasicForm = activeBasicSurface();
