// ═══════════════════════════════════════════════════════════════════════════
// LISTING EMAIL CAMPAIGN — create an investor / 1031 email FROM a listing.
//
...
//
// All compliance gating (REBNY distribution gate, Fair Housing, suppression,
// fail-closed live send) is enforced SERVER-SIDE. This UI only composes, shows
// the review counts, and surfaces server errors. It never sends on its own and
// live delivery stays disabled until CAMPAIGN_LIVE_SEND_ENABLED is set.
// ═══════════════════════════════════════════════════════════════════════════
...
(function () {
  ...
  // ─── Public entry point (called from the listing detail action row) ───────
  ...
  function field(label, key, ph, type) {
    ...
    var input = (type === 'textarea')
      ? '<textarea id="lc_' + key + '" rows="2" class="w-full border rounded px-2 py-1 text-xs" placeholder="' + esc(ph || '') + '">' + v + '</textarea>'
      : '<input id="lc_' + key + '" value="' + v + '" class="w-full border rounded px-2 py-1 text-xs" placeholder="' + esc(ph || '') + '">';
  ...
  function renderCompose() {
    ...
    return errorBanner() +
      '<div class="grid grid-cols-2 gap-4">' +
        ...
        '</div>' +
        '<div class="space-y-2">' +
          '<div class="text-[11px] font-semibold text-gray-600">Live preview' +
            (s ? ' — <span class="text-gray-400">' + esc(s.address) + ' · ' + esc(s.price) + ' · ' + s.photoCount + ' photos</span>' : '') + '</div>' +
          '<iframe title="preview" class="w-full border rounded bg-white" style="height:420px" srcdoc="' + esc(state.previewHtml) + '"></iframe>' +
        '</div>' +
      '</div>' +
  ...
  function renderReview() {
    ...
    return errorBanner() +
      ...
      '<div class="flex flex-wrap gap-2 items-center">' +
        '<button onclick="window._lcSend(\'dry_run\')" class="px-3 py-2 bg-gray-800 text-white rounded text-xs"><i class="fas fa-vial mr-1"></i>Dry run (no delivery)</button>' +
        '<div class="flex items-center gap-1">' +
          '<input id="lcTestList" class="border rounded px-2 py-1 text-xs" placeholder="test@ (comma-separated)">' +
          '<button onclick="window._lcSend(\'test\')" class="px-3 py-2 bg-gray-800 text-white rounded text-xs"><i class="fas fa-flask mr-1"></i>Send test</button>' +
        '</div>' +
        '<button onclick="window._lcSend(\'live\')" class="px-3 py-2 bg-red-600 text-white rounded text-xs"><i class="fas fa-paper-plane mr-1"></i>Send live</button>' +
      '</div>' +
