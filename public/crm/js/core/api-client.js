...
var MallanAPI = (function () {
  ...
  var auth = {
    ...
    /**
     * Return to broker — end an active delegated session.
     * Backend (app/api/auth/impersonation/stop) writes an AuditEvent
     * ("impersonate_stop") carrying BOTH identities, destroys ONLY the
     * delegated session row, and rotates a FRESH token onto the broker's
     * preserved parent session. There is no re-login and no second MFA:
     * the broker's own session was never destroyed to begin with.
     *
     * Response shape: { success: true, restored: { id, name, role } }
     *
