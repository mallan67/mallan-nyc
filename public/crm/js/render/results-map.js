  ...
  function buildGeoJSON(listings) {
    ...
    for (var i = 0; i < listings.length; i++) {
      ...
      if ((!lat || !lng) && _centroids && l.neighborhood) {
        ...
        if (centroid) {
          // Spiral offset: each listing in same neighborhood gets a different angle+radius
          var key = l.neighborhood;
          if (!centroidCounts[key]) centroidCounts[key] = 0;
          var idx = centroidCounts[key]++;
          // Spread across ~0.012° ≈ 8-10 blocks (visible at neighborhood zoom)
          var angle = idx * 2.399963; // golden angle in radians for even spread
          var radius = 0.003 + Math.sqrt(idx) * 0.002; // grows outward
