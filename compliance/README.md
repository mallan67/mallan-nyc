...
> **LMP:** the legacy upstream intermediary (listing input to RLS — external to mallan.nyc) | **IDX Display:** Trestle IDX Plus WebAPI (public display + internal CRM + reporting) | **Stage:** Live Production
>
> **IDX SCOPE (Confirmed by REBNY 2026-03-27):** IDX feed powers: (1) public website listing display, (2) internal backend dashboard with client management, and (3) reporting. Client data stays on mallan.nyc — never passes through the legacy upstream intermediary or third parties. IDX feed is limited to the IDX-released field set and IDX-eligible inventory only — it is NOT full-market search. Agents use the legacy upstream intermediary for full RLS inventory and listing submission. mallan.nyc does NOT submit listings to the RLS and is NOT an LMP.
...
| LMP (the legacy upstream intermediary) | Listing input to RLS (REBNY does not grant LMP to individual brokers) |
