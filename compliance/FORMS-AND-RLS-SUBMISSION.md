...
|------|------|--------|---------|
| Sale Listing | `SALE-FORM-REDESIGN.html` | 719 | Sale listing data entry + RLS submission |
| Rental Listing | `RENTAL-FORM-REDESIGN.html` | 525 | Rental listing data entry + RLS submission |
...
## 2. RLS Submission Workflow

```
Agent fills form → Auto-save (30s) → Validate (47+ fields) → Content scan
  → Fair Housing scan → Distribution gate check → Preview → Submit to RLS
  → Trestle accepts/rejects → Log result → Update status
```
