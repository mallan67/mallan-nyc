 ...
 * (= COTALITY_PROPERTY_SELECT_FIELDS minus exclusions, sourced from B27_RENTAL). Unless the
...
describe('MoveInCosts* ingestion chain (live → select → raw_data)', () => {
  ...
  it('2. present in B27_RENTAL / COTALITY_PROPERTY_SELECT_FIELDS', () => {
    for (const f of LIVE) expect(COTALITY_PROPERTY_SELECT_FIELDS).toContain(f);
  });
  ...
  it('6. MoveInCostsAmountTotal stays OUT of every link (still phantom)', () => {
    ...
    expect(COTALITY_PROPERTY_SELECT_FIELDS).not.toContain('MoveInCostsAmountTotal');
