...
describe("upsertListingMedia — Checkpoint 2", () => {
  ...
  it("skips rows without MediaKey (cannot dedupe)", async () => {
    ...
    expect(result).toEqual({ inserted: 0, updatedChanged: 0, skippedUnchanged: 0, skippedInvalid: 3, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });
  ...
  it("skips rows without MediaURL", async () => {
    ...
    expect(result).toEqual({ inserted: 0, updatedChanged: 0, skippedUnchanged: 0, skippedInvalid: 2, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });
  });
  ...
  it("inserts new rows when no matching media_key exists", async () => {
    ...
    expect(result).toEqual({ inserted: 2, updatedChanged: 0, skippedUnchanged: 0, skippedInvalid: 0, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });
  ...
  it("updates existing rows when media_key matches", async () => {
    ...
    expect(result).toEqual({ inserted: 0, updatedChanged: 1, skippedUnchanged: 0, skippedInvalid: 0, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });
    expect(mockCreate).not.toHaveBeenCalled();
  ...
  it("running twice with identical input produces 1 insert then 1 update (no duplicates)", async () => {
    ...
    expect(r1).toEqual({ inserted: 1, updatedChanged: 0, skippedUnchanged: 0, skippedInvalid: 0, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });

    // Second run — same row, now exists.
    mockFindUnique.mockResolvedValueOnce({ id: 1n, listing_id: "RLS20012345", media_key: "MK-1" });
    mockUpdate.mockResolvedValueOnce(undefined);
    const r2 = await upsertListingMedia("RLS20012345", [makeRow({ MediaKey: "MK-1" })]);
    expect(r2).toEqual({ inserted: 0, updatedChanged: 1, skippedUnchanged: 0, skippedInvalid: 0, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });

    // Third run — still exists.
    mockFindUnique.mockResolvedValueOnce({ id: 1n, listing_id: "RLS20012345", media_key: "MK-1" });
    mockUpdate.mockResolvedValueOnce(undefined);
    const r3 = await upsertListingMedia("RLS20012345", [makeRow({ MediaKey: "MK-1" })]);
    expect(r3).toEqual({ inserted: 0, updatedChanged: 1, skippedUnchanged: 0, skippedInvalid: 0, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });
  ...
  it("explicit-delete tombstone PLUS vanished tombstone are both counted", async () => {
    ...
    expect(result).toEqual({ inserted: 1, updatedChanged: 0, skippedUnchanged: 0, skippedInvalid: 0, deleteSignalsReceived: 1, tombstonedExplicit: 1, tombstonedVanished: 3, tombstoned: 4 });
  ...
  it("on empty input with no options — performs zero DB calls", async () => {
    const result = await upsertListingMedia("RLS20012345", []);
    expect(result).toEqual({ inserted: 0, updatedChanged: 0, skippedUnchanged: 0, skippedInvalid: 0, deleteSignalsReceived: 0, tombstonedExplicit: 0, tombstonedVanished: 0, tombstoned: 0 });
    expect(mockFindUnique).not.toHaveBeenCalled();
    expect(mockCreate).not.toHaveBeenCalled();
    expect(mockUpdate).not.toHaveBeenCalled();
    expect(mockUpdateMany).not.toHaveBeenCalled();
