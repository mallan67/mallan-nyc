...
export async function runMediaSync(options: RunMediaSyncOptions = {}): Promise<RunMediaSyncResult> {
  ...
  // policy this run, to be POLICY-PARKED (r2_attempts =
  // R2_POLICY_PARKED_ATTEMPTS + r2_last_attempt_at = now) in ONE batched
  // updateMany after the Phase-3 loop. Without this write the rejected rows'
  // DB state would be untouched, they would re-match the backlog where every
  // firing, and — combined with the bounded candidate batch — the same
  // oldest rejected rows would refill the batch forever and starve valid
  // heroes (Maya Blocker-1: no select-all-then-reject-without-state).
  ...
  while (remainingMs() > phase2ReserveMs) {
    ...
    //
    // Blocker-1 — rejection is NEVER stateless. Two rejection classes:
    //   DETERMINISTIC (fact cannot change for this row: it is not the hero /
    //   its media_type is outside the scope's approved set) ⇒ POLICY-PARK:
    //   id collected into `policyParkIds`, flushed after the loop in ONE
    //   `updateMany` writing r2_attempts = R2_POLICY_PARKED_ATTEMPTS (9) +
    //   r2_last_attempt_at = now. The backlog predicate (r2_attempts < 8)
    //   then permanently excludes the row — it NEVER returns as a candidate.
    //   Un-parking is R2-3's job.
    //   TRANSIENT (hero lookup failed; scope 'none' — the DB-side where
    //   excludes such rows, so reaching here means a select/re-check race or
    //   where drift on MUTABLE listing-level state) ⇒ NOT parked: the row
    //   re-surfaces next firing and is re-decided. Fail-closed for
    //   mirroring, fail-open for retry.
    ...
    for (const row of backlogRows) {
      ...
      // re-verified here FAIL-CLOSED so a drifted where can never widen the
      // mirror set. A type mismatch is DETERMINISTIC for the row
      // (media_type is fixed per media_key) ⇒ park it.
      const approvedTypes =
      ...
      if (heroKey === null) {
        // TRANSIENT: hero unknown (lookup failed — the fetched row is itself
        // an active Photo, so "no eligible hero exists" is unreachable when
        // the lookup succeeds). NOT parked; re-decided next firing.
        mirrorRejectedPolicy++;
      } else if (row.media_key === heroKey) {
        admittedRows.push(row);
      } else {
        // DETERMINISTIC: this row is not the canonical hero. This branch
        // also covers every gallery row of a listing whose hero is ALREADY
        // mirrored (the hero row no longer matches the backlog where, but
        // the resolver still identifies it as hero) — those are parked, not
        // silently skipped. PARK: never a candidate again.
        mirrorRejectedPolicy++;
        policyParkIds.push(row.id);
      }
    }
