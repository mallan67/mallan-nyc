...
export const FIELD_REGISTRY: readonly FieldSpec[] = Object.freeze([
  ...
  f({ canonicalKey: 'bedrooms', searchParams: ['minBeds', 'maxBeds', 'beds'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Beds', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'BedroomsTotal', dbColumn: 'bedrooms_total', projectionColumn: 'bedrooms_total', searchParam: 'beds/maxBeds', visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'yes', reportable: 'yes' }),
  f({ canonicalKey: 'bathrooms', mappingConflict: 'THE EXECUTOR QUERIES A FIELD THIS PROJECT HAS ALREADY REJECTED. lib/search/crm-idx-filter.ts emits BathroomsTotalInteger ge/le, while lib/search/canonical/bath-contract.ts lists BathroomsTotalInteger under rejected on an exhaustive 8,103-row live read: it is an Int32 so it cannot represent 1.5, and it disagrees with its own components on ~1% of rows (best of four hypotheses 98.8%; counterexample RLS20105072 reports full=2, half=1, TotalInteger=0). Three consequences: half-baths are unexpressible on the CRM Search path though BathroomsHalf is non-zero on 2,023 Active rows; minBathsOData/maxBathsOData have NO production caller while the Prisma engine DOES use the contract, so the two engines answer the same bath question differently; and crm-idx-filter.test.ts locks in "BathroomsTotalInteger ge 1.5", its own comment conceding it is not strictly OData-numeric on Edm.Int32. RECORDED, NOT FIXED HERE: resolving it is registry-to-executor consolidation, which is Section 5.', searchParams: ['minBaths', 'maxBaths'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Baths', category: 'rooms_size', type: 'number', providerMappingStatus: 'partial', cotalityField: 'BathroomsTotalInteger', dbColumn: 'bathrooms_full', searchParam: 'minBaths/maxBaths', visibility: V_PUBLIC, filterable: 'yes', alertable: 'yes', reportable: 'yes', semanticEquivalenceProven: false, notes: 'MAPPING CORRECTED 2026-08-27 to the field Mallan ACTUALLY asks for. This said BathroomsFull/BathroomsHalf - a prose composite, not a filter mapping - while lib/search/crm-idx-filter.ts has always emitted BathroomsTotalInteger ge/le. Recording what executes is a fact about Mallan code, NOT a live provider verification: whether BathroomsTotalInteger is the right fact for a baths range, versus full+half, is UNPROVEN against live Cotality and is why semanticEquivalenceProven is false. ORIGINAL NOTE: Diverges by engine: BathroomsFull vs BathroomsTotalInteger (analysis B-2) — canonicalize.' }),
  f({ canonicalKey: 'rooms_total', searchParams: ['minRooms', 'maxRooms'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Rooms', category: 'rooms_size', type: 'number', providerMappingStatus: 'partial', cotalityField: 'RoomsTotal', visibility: V_CLIENT, filterable: 'needs_probe', reportable: 'yes', notes: 'DTO reads features.Rooms but mapper stores RoomsTotal → always undefined today (analysis B-1).' }),
  f({ canonicalKey: 'living_area', searchParams: ['minSqft', 'maxSqft'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Square Feet', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'LivingArea', dbColumn: 'living_area', projectionColumn: 'living_area', searchParam: 'minSqft/maxSqft', visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'yes', reportable: 'yes', notes: 'Reliability drives $/sqft (needs_probe on completeness).' }),
...
export interface TransportFacts {
  ...
  /** The executor demonstrably builds a provider clause for it. */
  providerClauseProven: boolean;
}
...
export function executionReadiness(
  ...
  if (field.filterable === 'unsupported') return 'unsupported';
  if (!transport.providerClauseProven) return 'no_runtime_path';
  if (!transport.reachesServer) return 'not_yet_wired';
  // Capability yes WITHOUT a live probe is still unproven against the provider.
  if (field.filterable !== 'yes' || !field.liveEvidence) return 'needs_probe';
