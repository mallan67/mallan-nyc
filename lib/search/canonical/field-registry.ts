...
export const FIELD_REGISTRY: readonly FieldSpec[] = Object.freeze([
  ...
  f({ canonicalKey: 'listing_key', authorityResolution: 'fixed', sourceAuthority: 'cotality_rebny', uiLabel: 'Listing Key', category: 'identity_source_attribution', type: 'string', providerMappingStatus: 'mapped', cotalityField: 'ListingKey', dbColumn: 'listing_id', searchParam: 'listingId', visibility: V_PUBLIC, filterable: 'yes', reportable: 'yes', notes: 'Public DTO currently emits internal listing_id as mlsId (analysis B-12) — registry key is ListingKey.' }),
  f({ canonicalKey: 'listing_id_mls', authorityResolution: 'fixed', sourceAuthority: 'cotality_rebny', uiLabel: 'MLS #', category: 'identity_source_attribution', type: 'string', providerMappingStatus: 'mapped', cotalityField: 'ListingId', visibility: V_PUBLIC, filterable: 'needs_probe', reportable: 'yes' }),
  f({ canonicalKey: 'source', authorityResolution: 'fixed', sourceAuthority: 'cotality_rebny', uiLabel: 'Source', category: 'identity_source_attribution', type: 'enum', providerMappingStatus: 'partial', cotalityField: 'SourceSystemName', visibility: V_PUBLIC, reportable: 'yes', requiresAttribution: true, notes: 'No typed source column today (acris|mls|mallan_exclusive|internal) — provenance is inferred per-surface; contract makes it first-class. CONCEPTUALLY CORRECTED 2026-08-21: SourceSystemName/OriginatingSystem* describe the PROVIDER PIPELINE (RLS -> REBNY -> Trestle), NOT canonical listing authorship. Live census of 35 Mallan-office rows: SourceSystemName 0/35, SourceSystemKey 0/35, SourceSystemID=TRESTLE 35/35, OriginatingSystemName=RLS 35/35. None answers "is this Mallan-authored or third-party inventory" — that comes from the local-vs-provider identity contract. Retain these as RAW LINEAGE only; never map pipeline provenance onto listing ownership or edit authority.' }),
  ...
  f({ canonicalKey: 'geo', authorityResolution: 'mallan_derived', sourceAuthority: 'mallan_derived', uiLabel: 'Map Location', category: 'address_location_building', type: 'geo', cotalityField: null, dbColumn: 'latitude/longitude', visibility: V_PUBLIC, filterable: 'needs_probe', reportable: 'yes', providerMappingStatus: 'none', attributionObligations: ['mallan_derived_disclosure', 'provenance_disclosure'], notes: 'CORRECTED 2026-08-20. A nullable Latitude/Longitude declaration in $metadata is NOT a Search capability and must never be used as one on that basis. Mallan geo is Cotality address -> canonical Mallan address -> Google geocoding -> canonical coordinate, with MTA/Google transit enrichment layered on. Mallan-owned, attributable to Mallan, never to the provider.' }),
