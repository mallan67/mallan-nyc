...
export const FIELD_REGISTRY: readonly FieldSpec[] = Object.freeze([
  ...
  f({ canonicalKey: 'listing_contract_date', uiLabel: 'Listed Date', category: 'dom_dates', type: 'date', providerMappingStatus: 'mapped', cotalityField: 'ListingContractDate', dbColumn: 'listing_contract_date', visibility: V_PUBLIC, sortable: 'yes', reportable: 'yes', notes: 'Canonical "newest" sort key (NOT ModificationTimestamp, which orders by last touch). BUILDER live evidence 2026-09-05 (pending Validator re-check): ListingContractDate non-null on every active sale (6639) and rental (918) row; select, ne null, orderby and ge all SUPPORTED (docs/search/checkpoints/2026-09-05-registry-vs-live-comparison.md, Builder evidence section).' }),
  ...
  // ── media ────────────────────────────────────────────────────────────────
  f({ canonicalKey: 'media', uiLabel: 'Photos / Tour', category: 'media', type: 'array', providerMappingStatus: 'mapped', cotalityField: 'Media (ResourceRecordKey = ListingKey; ResourceRecordID = ListingId)', visibility: V_PUBLIC, reportable: 'yes', notes: 'Live 2026-09-05: Media joins to Property by ResourceRecordKey eq ListingKey OR by ResourceRecordID eq ListingId — each in its own domain, both return the same rows, crossed forms return 0. The Validator confirmed the two join keys and that Order is not unique per listing (checkpoint-contract §M). BUILDER live evidence 2026-09-05 (pending Validator re-check): Order restarts at 1 per MediaCategory, so a photo-first hero is chosen by category and Order; PreferredPhotoYN is null on 180 of 200 rows and is not a dependable marker; media-level InternetEntireListingDisplayYN is populated and filterable (docs/search/evidence/2026-09-05-live-cotality-system-study.md §5, §5a).' }),
