...
export const FIELD_REGISTRY: readonly FieldSpec[] = Object.freeze([
  ...
  // ── open house ───────────────────────────────────────────────────────────
  f({ canonicalKey: 'open_house', criterionRole: 'broker_input', criterionValueShape: 'range_date', executionStrategy: 'provider_filter', workflows: ['sale', 'rental'], searchParams: ['openHouse', 'openHouseDateFrom', 'openHouseDateTo'],  authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Open House', category: 'open_house', type: 'object', providerMappingStatus: 'mapped', cotalityField: 'OpenHouse (resource)', searchParam: 'openHouse', visibility: V_PUBLIC, filterable: 'yes', alertable: 'unsupported', reportable: 'yes', failureBehavior: 'fail_loud', liveEvidence: { probedAt: '2026-09-01', source: 'artifacts/p0-search-acceptance/identity-probes.json - OpenHouse resource $count 1993; OpenHouseDate ge <today> and le <+30d> -> 1970; + OpenHouseStatus eq Active accepted; $orderby OpenHouseDate asc accepted; Property.ListingKey eq <OpenHouse.ListingKey> -> 1; Property.ListingId eq <OpenHouse.ListingKey> -> 0' }, notes: 'UNBLOCKED 2026-09-02. The 2026-08-29 block was correct and its cause is gone: the criterion applied AFTER pagination, so the open-house test ran over rows already sliced into a page. Membership is now settled BEFORE count and page cut, as a corpusFilter keyed on ListingKey (lib/search/open-house-provider.ts -> app/api/idx/search/route.ts). SHAPE CORRECTED boolean -> range_date: the broker picks a WINDOW (Today / This Weekend / Next 7 / Next 30 / Custom), never a yes-no. FILTERABLE is proven on the OpenHouse RESOURCE, not on Property - the structured liveEvidence above carries the probe record. A preset travels as a TOKEN and the server resolves the window in America/New_York, so the browser timezone cannot decide a NYC brokerage day. STILL INCOMPLETE: authorityByListingKind declares mallanLocal -> mallan_crm and that half is NOT implemented - authenticated Search reads no Mallan storage, so a Mallan-authored SL-/RL- listing with a Showing-backed open house cannot appear under this criterion. A real gap against Master Plan 5.1, not a design decision, and NOT to be closed by manufacturing a provider key for a local listing.' }),
