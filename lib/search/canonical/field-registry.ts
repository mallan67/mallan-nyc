 ...
 */
/**
 * THE PERSISTENCE VOCABULARY — one key per canonical business concept.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * WHY IT MOVED HERE (type direction inverted 2026-08-28)
 *
 * `filter-keys.ts` used to declare this union, and this file imported it to type
 * `FieldSpec.filterKeys`. That made the registry depend on its own derivative:
 * generating the vocabulary FROM the registry while the registry imports its
 * type FROM the vocabulary is circular, and populating the bridge in that shape
 * would have cemented the circle.
 *
 * The registry now owns the literal list and `filter-keys.ts` derives its type
 * and its runtime set from here. That ELIMINATES a vocabulary rather than
 * relocating one: there is no second hand-maintained list to drift.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ONE KEY PER CONCEPT, NOT ONE PER BOUND
 *
 * The old list split ranges — `price_min` / `price_max` — so a range criterion
 * had two persistence keys and no single business identity. The concept is
 * `list_price`; its VALUE carries the bounds. `SavedSearchCriteria.filters` is
 * `Partial<Record<CanonicalFilterKey, unknown>>`, so a `{ min, max }` value is
 * already permitted by the type.
 *
 * This is safe to change because `CanonicalFilterKey` was never wired: the
 * browser persists its own snake_case record (`min_price`, `close_date_from`),
 * so NO stored row is keyed by this vocabulary today. Legacy snake_case rows stay
 * readable through the boundary adapter in `filter-keys.ts`, which is now
 * explicitly a legacy READ adapter and not business authority.
 *
 * Every member below is a `canonicalKey` of an entry in FIELD_REGISTRY. The
 * mutation test in `one-search-mapping-authority.test.ts` fails if that stops
...
export const CANONICAL_FILTER_KEYS = [
  ...
  // Not a criterion — the result ordering, persisted alongside them.
  'sort',
] as const;
...
export interface FieldSpec {
  ...
  filterKeys?: readonly CanonicalFilterKeyName[];
  type: FieldType;
...
export const FIELD_REGISTRY: readonly FieldSpec[] = Object.freeze([
  ...
  f({ canonicalKey: 'bedrooms', searchParams: ['minBeds', 'maxBeds', 'beds'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Beds', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'BedroomsTotal', dbColumn: 'bedrooms_total', projectionColumn: 'bedrooms_total', searchParam: 'beds/maxBeds', filterKeys: ['bedrooms'], visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'yes', reportable: 'yes' }),
  f({ canonicalKey: 'bathrooms', searchParams: ['minBaths', 'maxBaths'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Baths', category: 'rooms_size', type: 'number', providerMappingStatus: 'partial', cotalityField: 'BathroomsTotalInteger', dbColumn: 'bathrooms_full', searchParam: 'minBaths/maxBaths', filterKeys: ['bathrooms'], visibility: V_PUBLIC, filterable: 'yes', alertable: 'yes', reportable: 'yes', semanticEquivalenceProven: false, notes: 'MAPPING CORRECTED 2026-08-27 to the field Mallan ACTUALLY asks for. This said BathroomsFull/BathroomsHalf - a prose composite, not a filter mapping - while lib/search/crm-idx-filter.ts has always emitted BathroomsTotalInteger ge/le. Recording what executes is a fact about Mallan code, NOT a live provider verification: whether BathroomsTotalInteger is the right fact for a baths range, versus full+half, is UNPROVEN against live Cotality and is why semanticEquivalenceProven is false. ORIGINAL NOTE: Diverges by engine: BathroomsFull vs BathroomsTotalInteger (analysis B-2) — canonicalize.' }),
  f({ canonicalKey: 'rooms_total', filterKeys: ['rooms_total'], searchParams: ['minRooms', 'maxRooms'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Rooms', category: 'rooms_size', type: 'number', providerMappingStatus: 'partial', cotalityField: 'RoomsTotal', visibility: V_CLIENT, filterable: 'needs_probe', reportable: 'yes', notes: 'DTO reads features.Rooms but mapper stores RoomsTotal → always undefined today (analysis B-1).' }),
  f({ canonicalKey: 'living_area', searchParams: ['minSqft', 'maxSqft'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Square Feet', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'LivingArea', dbColumn: 'living_area', projectionColumn: 'living_area', searchParam: 'minSqft/maxSqft', filterKeys: ['living_area'], visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'yes', reportable: 'yes', notes: 'Reliability drives $/sqft (needs_probe on completeness).' }),

  // ── monthly carrying costs ───────────────────────────────────────────────
  f({ canonicalKey: 'maintenance_common_charge', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Maintenance / CC', category: 'carrying_costs', type: 'money', providerMappingStatus: 'mapped', cotalityField: 'AssociationFee', visibility: V_PUBLIC, filterable: 'unsupported', sortable: 'unsupported', reportable: 'needs_probe', failureBehavior: 'fail_loud', notes: 'PRODUCT GAP (analysis B-15): not filterable/sortable anywhere today. FIELD FAMILY, not one field. AssociationFee alone is NOT a monthly maintenance number: live AssociationFeeFrequency includes Monthly/Quarterly/Annually/SemiMonthly/Weekly, and AssociationFee2 / AssociationFee3 families also exist. For NYC the business meaning also depends on CommonInterest — co-op MAINTENANCE and condo COMMON CHARGES are different facts, not two labels. Never normalise money without its units/frequency.' }),
  f({ canonicalKey: 'taxes', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Taxes', category: 'carrying_costs', type: 'money', providerMappingStatus: 'mapped', cotalityField: 'TaxAnnualAmount', visibility: V_PUBLIC, reportable: 'needs_probe', notes: 'Input to total carrying cost.' }),
  ...
  f({ canonicalKey: 'year_built', filterKeys: ['year_built'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Year Built', category: 'address_location_building', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'YearBuilt', dbColumn: 'year_built', searchParam: 'minYear/maxYear', searchParams: ['minYear', 'maxYear'], visibility: V_PUBLIC, filterable: 'needs_probe', reportable: 'yes', notes: 'ADDED 2026-08-27. The executor has emitted YearBuilt ge/le since before this registry existed, and no entry claimed it — one of sixteen criteria that were executable while the declared mapping authority had never heard of them. filterable is needs_probe, not yes: Mallan asks the question, and whether the provider answers it is a separate live fact this file has no probe record for.' }),
  f({ canonicalKey: 'stories_total', filterKeys: ['stories_total'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Floors', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'StoriesTotal', searchParam: 'minFloors/maxFloors', searchParams: ['minFloors', 'maxFloors'], visibility: V_PUBLIC, filterable: 'needs_probe', reportable: 'yes', notes: 'ADDED 2026-08-27 — see year_built. Executable with no registry entry until now.' }),
  f({ canonicalKey: 'units_total', filterKeys: ['units_total'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Units in Building', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'NumberOfUnitsTotal', searchParam: 'minUnits/maxUnits', searchParams: ['minUnits', 'maxUnits'], visibility: V_AGENT, filterable: 'needs_probe', reportable: 'yes', notes: 'ADDED 2026-08-27 — see year_built. Executable with no registry entry until now.' }),
  f({ canonicalKey: 'activity_date', filterKeys: ['activity_date'], authorityResolution: 'fixed', sourceAuthority: 'cotality', uiLabel: 'Listing Activity Date', category: 'dom_dates', type: 'date', providerMappingStatus: 'mapped', cotalityFields: ['ListingContractDate', 'ModificationTimestamp'], searchParam: 'dateFrom/dateTo/dateType', searchParams: ['dateFrom', 'dateTo', 'dateType'], visibility: V_AGENT, filterable: 'needs_probe', reportable: 'yes', notes: 'ADDED 2026-08-27. A COMPOSITE criterion: dateType selects WHICH provider date the range applies to — Listed -> ListingContractDate ge/le, Updated -> ModificationTimestamp gt/le — so it is enumerated rather than collapsed into one field name. EXECUTABLE_DATE_TYPES in lib/search/crm-idx-filter.ts is the closed set; an unrecognised dateType with a range present raises UnsupportedSearchCriterionError rather than silently defaulting to Listed, which is the collapse that set exists to prevent.' }),
  f({ canonicalKey: 'public_remarks_keyword', filterKeys: ['public_remarks_keyword'], authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Keyword', category: 'identity_source_attribution', type: 'string', providerMappingStatus: 'mapped', cotalityField: 'PublicRemarks', dbColumn: null, searchParam: 'keyword', searchParams: ['keyword'], visibility: V_PUBLIC, filterable: 'needs_probe', reportable: 'no', notes: 'ADDED 2026-08-27. Executes as contains(PublicRemarks, ...). A substring match over free text is NOT a structured criterion: it cannot express negation, it matches inside words, and its recall depends entirely on how a listing was written. needs_probe covers whether the provider supports contains() on this field at all; the SEMANTIC question — whether a keyword hit means what a broker thinks it means — is not a probe question and is recorded here rather than assumed.' }),
  f({ canonicalKey: 'management_company', filterKeys: ['management_company'], authorityResolution: 'unresolved', uiLabel: 'Management Company', category: 'ownership_common_interest', type: 'string', providerMappingStatus: 'none', cotalityField: null, searchParam: 'managementCompany', searchParams: ['managementCompany'], visibility: V_AGENT, filterable: 'unsupported', sortable: 'unsupported', alertable: 'unsupported', reportable: 'no', failureBehavior: 'fail_loud', notes: 'ADDED 2026-08-27 as an EXPLICIT REFUSAL. The UI can send it; Cotality declares no ManagementCompany Property field, and lib/search/crm-idx-filter.ts throws UnsupportedSearchCriterionError rather than substituting one. Listing office is a DIFFERENT fact and must never stand in for it. Recorded here because a refusal is part of the contract — omitting it would make the authority look as though it had never heard of a criterion the product offers.' }),
  f({ canonicalKey: 'map_grid_filter', filterKeys: ['map_grid_filter'], authorityResolution: 'mallan_derived', sourceAuthority: 'mallan_derived', uiLabel: 'Map Grid', category: 'address_location_building', type: 'geo', providerMappingStatus: 'none', cotalityField: null, searchParam: 'gridFilter', searchParams: ['gridFilter'], visibility: V_AGENT, filterable: 'unsupported', sortable: 'no', alertable: 'no', reportable: 'no', failureBehavior: 'fail_loud', notes: 'ADDED 2026-08-27 as an EXPLICIT REFUSAL. Coordinates are map support, not a canonical Search axis, and the executor refuses a caller-supplied coordinate predicate rather than passing it to the provider. B5 requires the map to express canonical GEOGRAPHIC criteria that the authoritative server search executes — not a raw viewport predicate — so this stays unsupported until that path exists.' }),
  f({ canonicalKey: 'sponsor_unit', filterKeys: ['sponsor_unit'], authorityResolution: 'fixed', sourceAuthority: 'cotality', uiLabel: 'Sponsor Unit', category: 'ownership_common_interest', type: 'boolean', providerMappingStatus: 'partial', cotalityField: null, searchParam: 'sponsorUnit', searchParams: ['sponsorUnit'], visibility: V_AGENT, filterable: 'unsupported', sortable: 'unsupported', alertable: 'unsupported', reportable: 'needs_probe', failureBehavior: 'fail_loud', semanticEquivalenceProven: false, notes: 'ADDED 2026-08-28 as an EXPLICIT REFUSAL, and it was the only executable-path criterion with NO registry owner at all. SponsorUnit lives inside CustomProperty.CustomFields — a REBNY-specific JSON-string field — not as a top-level OData property, so the generic checkbox path would silently drop it. The serializer pulls it out of feature_criteria into its own sponsorUnit param and app/api/idx/search/route.ts throws UnsupportedSearchCriterionError for it. NOTE THE REFUSAL LIVES IN THE ROUTE, NOT IN crm-idx-filter.ts: a refusal audit that scans only the filter builder reports this criterion unverified, which is the same one-file blind spot that let the missing params.status assignment survive a test written to prevent it. cotalityField is null because the containing field is CustomProperty.CustomFields and the exact extraction contract is unproven; do NOT record a top-level field name here.' }),
  f({ canonicalKey: 'max_financing_percent', filterKeys: ['max_financing_percent'], authorityResolution: 'unresolved', uiLabel: 'Max Financing Allowed %', category: 'ownership_common_interest', type: 'number', providerMappingStatus: 'needs_probe', cotalityField: null, searchParam: 'financingMin', searchParams: [], visibility: V_AGENT, filterable: 'unsupported', sortable: 'unsupported', alertable: 'unsupported', reportable: 'needs_probe', failureBehavior: 'fail_loud', semanticEquivalenceProven: false, notes: 'ADDED 2026-08-28. The building/board financing limit a co-op or condo permits. It had NO registry owner and NO runtime path, and being broken is not a reason to fall outside the authority graph — Section 4 requires every visible criterion to be explicitly accounted for. PRESERVED, NOT DELETED, per the standing rule against discarding difficult Search criteria: the business concept is real and awaits authorized live Cotality verification. CURRENT STATE, TRACED 2026-08-28: the control is LIVE AND ENABLED on three ids (saleBuildingFinancingMin, rentalBuildingFinancingMin, buildingFinancingMin) and is absent from init-disable-dead-controls.js; collectSearchCriteria writes criteria.financingMin; and NOTHING READS IT — zero reads repo-wide, and buildCrmIdxODataFilter contains no occurrence of financ at all. So the broker types a narrowing value, receives HTTP 200, and gets a WIDER result set wearing the costume of a narrower one. That is the same silent-widening class as the dropped status param, and it is why filterable is unsupported (must fail loud) rather than needs_probe. SECOND DISTINCT DEAD PATH for the same concept: the checkbox family data-field=MaximumFinancingPercent carries magic operator values gt:0 and eq:0 that no canonical parser owns, and is neutralised before collection. Financing has two dead paths, not one — B1 must not fix one and call the concept done.' }),
]);
