...
export const FIELD_REGISTRY: readonly FieldSpec[] = Object.freeze([
  ...
  f({ canonicalKey: 'unit', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Unit', category: 'address_location_building', type: 'string', providerMappingStatus: 'mapped', cotalityField: 'UnitNumber', searchParam: 'unit', visibility: V_PUBLIC, filterable: 'needs_probe', reportable: 'yes' }),
  ...
  f({ canonicalKey: 'close_date', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Sold Date', category: 'close_rental_history', type: 'date', providerMappingStatus: 'mapped', cotalityField: 'CloseDate', dbColumn: 'contract_closed', visibility: V_AGENT, reportable: 'yes', notes: 'Comp windowing uses CloseDate (comp-eligibility.ts), never ModificationTimestamp.' }),
  ...
  f({ canonicalKey: 'bedrooms', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Beds', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'BedroomsTotal', dbColumn: 'bedrooms_total', projectionColumn: 'bedrooms_total', searchParam: 'beds/maxBeds', filterKeys: ['beds_min', 'beds_max'], visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'yes', reportable: 'yes' }),
  f({ canonicalKey: 'bathrooms', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Baths', category: 'rooms_size', type: 'number', providerMappingStatus: 'partial', cotalityField: 'BathroomsFull/BathroomsHalf', dbColumn: 'bathrooms_full', searchParam: 'minBaths/maxBaths', filterKeys: ['baths_min', 'baths_max'], visibility: V_PUBLIC, filterable: 'yes', alertable: 'yes', reportable: 'yes', notes: 'Diverges by engine: BathroomsFull vs BathroomsTotalInteger (analysis B-2) — canonicalize.' }),
  f({ canonicalKey: 'rooms_total', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Rooms', category: 'rooms_size', type: 'number', providerMappingStatus: 'partial', cotalityField: 'RoomsTotal', visibility: V_CLIENT, filterable: 'needs_probe', reportable: 'yes', notes: 'DTO reads features.Rooms but mapper stores RoomsTotal → always undefined today (analysis B-1).' }),
  f({ canonicalKey: 'living_area', authorityResolution: 'by_listing_authority', authorityByListingKind: { mallanLocal: 'mallan_crm', providerListing: 'cotality' }, uiLabel: 'Square Feet', category: 'rooms_size', type: 'number', providerMappingStatus: 'mapped', cotalityField: 'LivingArea', dbColumn: 'living_area', projectionColumn: 'living_area', searchParam: 'minSqft/maxSqft', filterKeys: ['sqft_min', 'sqft_max'], visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'yes', reportable: 'yes', notes: 'Reliability drives $/sqft (needs_probe on completeness).' }),
  ...
  f({ canonicalKey: 'listing_universe', authorityResolution: 'fixed', sourceAuthority: 'cotality', uiLabel: 'Sale / Rental Universe', category: 'property_type', type: 'enum', providerMappingStatus: 'mapped', cotalityField: 'PropertyType', visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'needs_probe', reportable: 'yes', failureBehavior: 'fail_closed', notes: 'VERIFIED LIVE 2026-08-22. Nullable enum, Edm.Int64, THIRTEEN members: BusinessOpportunity=0 CommercialLease=1 CommercialSale=2 DisasterReliefRental=3 Farm=4 HighRise=5 Land=6 ManufacturedInPark=7 MultiFamily=8 Residential=9 ResidentialIncome=10 ResidentialLease=11 Specialty=12. \'Commercial\' is NOT a member - HTTP 400, not a valid enumeration type constant (note lib/compliance/reso-mapper.ts:17 types it as one). MALLAN SEMANTICS, DECIDED NOT INFERRED: SALE = {Residential}. RENTAL = {ResidentialLease}. Every other member is UNKNOWN and belongs to NEITHER universe. MultiFamily, ResidentialIncome and Land are plausibly sales in ordinary speech and are deliberately EXCLUDED - a member existing in the vocabulary is not a decision that it belongs in a residential sale search. Widening either set is a product decision for Maya. SALE MUST NEVER BE DEFINED AS THE COMPLEMENT OF RENTAL. Measured live: PropertyType ne \'ResidentialLease\' returns exactly the same rows as eq \'Residential\', so the negation is INDISTINGUISHABLE FROM CORRECT by observation - it agrees only because the other eleven members are unpopulated, and it silently absorbs each of them into residential sale inventory the moment one is populated. Render filters as positive eq predicates joined by or. The canonical implementation is lib/search/canonical/property-type-universe.ts; do not re-derive this split anywhere else. Evidence: docs/idx/COTALITY-SALE-RENTAL-STATUS-EVIDENCE-2026-08-22.md' }),

  f({ canonicalKey: 'standard_status', authorityResolution: 'fixed', sourceAuthority: 'cotality', uiLabel: 'Standard Status', category: 'property_type', type: 'enum', providerMappingStatus: 'mapped', cotalityField: 'StandardStatus', visibility: V_PUBLIC, filterable: 'yes', sortable: 'yes', alertable: 'needs_probe', reportable: 'yes', failureBehavior: 'fail_closed', notes: 'VERIFIED LIVE 2026-08-22. Nullable enum, Edm.Int64, ELEVEN members: Active=0 ActiveUnderContract=1 Canceled=2 Closed=3 ComingSoon=4 Delete=5 Expired=6 Hold=7 Incomplete=8 Pending=9 Withdrawn=10. Always populated on this feed. OPERATOR: eq and ne both behave correctly - ne \'Closed\' returns exactly Active + Pending, and this enum has no case-variant member pairs. STATED PER FIELD ON PURPOSE: the eq/ne asymmetry recorded on media_classification does NOT apply here, and neither field\'s operator behaviour may be inferred from the other. Compound conjunction with PropertyType is SUPPORTED. DISTINCT FROM MlsStatus - see that entry. These are different vocabularies (11 members vs 25) and are not interchangeable. Evidence: docs/idx/COTALITY-SALE-RENTAL-STATUS-EVIDENCE-2026-08-22.md' }),

  f({ canonicalKey: 'mls_status', authorityResolution: 'fixed', sourceAuthority: 'cotality', uiLabel: 'MLS Status', category: 'property_type', type: 'enum', providerMappingStatus: 'partial', cotalityField: 'MlsStatus', visibility: V_PUBLIC, filterable: 'unsupported', sortable: 'no', alertable: 'needs_probe', reportable: 'yes', failureBehavior: 'fail_closed', notes: 'VERIFIED LIVE 2026-08-22. Nullable enum, Edm.Int64, TWENTY-FIVE members including Leased, AttorneyReview, Contingent, CompSold, PendingShortSale, OptionPeriod, PrepShow and PendingBackupsRequested - none of which exists in StandardStatus. FILTERING IS SUPPRESSED AT PROVIDER LEVEL. All 25 members and the null probe return HTTP 400: "Results from \'RLS\' has been suppressed (provider Level) as field MlsStatus cannot be used for filtering or ordering". That is NOT zero rows and NOT an absent field - it is the provider declining to answer, so POPULATION IS UNVERIFIED. It IS $select-able and returned null on every sampled row; sampled, not proven universal, because the only instrument that could prove it is the suppressed filter. NO MALLAN QUERY FILTERS ON IT (verified across lib/ and app/), so the suppression breaks nothing today. FOLLOW-UP, NOT YET FIXED: lib/search/crm-idx-mapper.ts:198 reads String(raw.MlsStatus || raw.StandardStatus || \'\'), substituting a 25-member vocabulary for an 11-member one. Harmless only while MlsStatus is null. If it ever populates, the CRM receives statuses its normalisation has never seen. Deciding what Leased or AttorneyReview MEAN to a Mallan workflow is a product decision, so it was recorded rather than guessed. Evidence: docs/idx/COTALITY-SALE-RENTAL-STATUS-EVIDENCE-2026-08-22.md' }),
