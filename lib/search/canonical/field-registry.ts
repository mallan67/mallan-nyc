...
export const FIELD_REGISTRY: readonly FieldSpec[] = Object.freeze([
  ...
  f({ canonicalKey: 'price_per_sqft', criterionRole: 'broker_input', criterionValueShape: 'range_number', workflows: ['sale', 'comparable'], searchParams: [],  authorityResolution: 'unresolved', uiLabel: '$/Sqft', category: 'pricing', type: 'computed', providerMappingStatus: 'none', cotalityField: null, visibility: V_PUBLIC, filterable: 'unsupported', sortable: 'unsupported', reportable: 'needs_probe', failureBehavior: 'fail_loud', notes: 'UNBLOCKED 2026-09-01/02. The 2026-08-29 block was correct and its cause is fixed: the criterion applied AFTER pagination, so the open-house test ran over rows already sliced from an arbitrary page - a wrong answer, not a missing one. Membership is now settled BEFORE count and page cut, as a corpusFilter keyed on ListingKey (lib/search/open-house-provider.ts -> app/api/idx/search/route.ts). SHAPE CORRECTED from boolean to range_date: the broker picks a WINDOW (Today / This Weekend / Next 7 / Next 30 / Custom), never a yes-no. FILTERABLE proven on the OpenHouse RESOURCE, not on Property - live 2026-09-01 api.cotality.com: OpenHouse $count 1993; OpenHouseDate ge X and le Y -> 1970; + OpenHouseStatus eq Active accepted; $orderby OpenHouseDate accepted; OpenHouse.ListingKey -> Property.ListingKey count 1 and -> Property.ListingId count 0. A preset travels as a TOKEN and the server resolves the window in America/New_York, so the browser timezone cannot decide a NYC brokerage day. STILL INCOMPLETE, STATED PLAINLY: authorityByListingKind declares mallanLocal -> mallan_crm, and that half is NOT implemented. Authenticated Search reads no Mallan storage at all (app/api/idx/search/route.ts has zero prisma references), so a Mallan-authored SL-/RL- listing with a Mallan-managed open house in the showings table cannot appear under this criterion. That is a real gap against Master Plan 5.1, not a design decision, and it is NOT to be closed by manufacturing a provider key for a local listing.' }),
