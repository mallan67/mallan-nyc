...
- PR #603 (`feat/listing-external-media-authority-2026-08-12`) is at verified head `68ab6944`: canonical external-media writer/composer/readers, six Cotality tour slots, filter parity, combined CRM/IDX dedupe before one sort/slice, contracted shared fallback cache, and 12-key public-card raw-data compatibility reads are implemented. Exact-ID and verified physical duplicates are removed before pagination with Mallan/CRM authority winning.
- PR #603's GitHub PR checks, real-PostgreSQL migration/convergence proof, Guardrails, Node.js 24 Target Platform Build, Release Truth workflow and Claude review are green; its Vercel Preview is Ready. The Release Truth commit context correctly remains pending until authorized Production runtime proof exists.
- The Vercel alert investigated on 2026-08-12 was two historical failed Preview commits; later Preview heads, including `68ab6944`, are Ready. No Production failure was established by that alert.
- PR #603 remains draft/unmerged. Production migration, deploy, historical external-media recovery, backfill, shedding and archive execution have not run.
