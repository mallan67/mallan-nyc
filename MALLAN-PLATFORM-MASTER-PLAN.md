...
# 0. PLATFORM AUTHORITY, ACCESS AND PROVIDER OPERATING CONTRACT

> This section is mandatory operating architecture, not optional setup documentation.
> Agents may not begin implementation from a mapper, CSV, stale audit, Desktop checkout,
> remembered field name, Vercel screenshot or Neon URL. Establish authority first.
>
> Provider observations and numeric counts preserved below are last-verified evidence.
> They are useful because they record failure modes already paid for, but they are not
> permission to skip a live probe. If live provider behavior differs, the live provider
> wins and the discrepancy is reconciled deliberately.

## 0.1 Mandatory startup sequence — before any technical change

Every engineering or AI session that can affect Mallan must establish the same chain:

~~~text
1. GITHUB / GIT
   identify mallan67/mallan-nyc
   → read current main
   → read this Master
   → read MALLAN-CONTINUOUS-EXECUTION-STATE.md
   → identify PR base/head and required checks
   → do not use Maya's Desktop or an old worktree as authority

2. COTALITY / TRESTLE when provider facts are involved
   authenticate through the configured live path
   → read entitlement/service/$metadata
   → verify resource + field + exact picklist/string + query capability
   → verify attribution/display rights
   → only then map into Mallan

3. VERCEL when deployment, environment or Marketplace state is involved
   identify the live mallan-nyc Vercel project and team
   → read current deployment/environment/integration state
   → use current Vercel documentation for platform semantics

4. NEON when database/control-plane state is involved
   enter the Vercel-managed Neon resource through Vercel
   → reconcile live Neon project/branch/endpoint identity to the Execution State
   → never infer Neon authority from an old DATABASE_URL or an unrelated Neon login
...
If any required authority is unavailable, ambiguous, rate-limited, unauthorized or contradictory:

~~~text
UNVERIFIED → STOP THE DEPENDENT CLAIM / CHANGE
~~~

Do not replace live authority with a local CSV, generated mirror, mapper, old audit, agent memory or guessed equivalent.
...
Last-verified entitlement observation — re-read DataSystem, the service document and $metadata before implementation:
...
This was the last-verified behavior of Mallan's feed. Re-probe it before geography work; do not assume either continued absence or new availability without live evidence.
...
Last verified, `StandardStatus` and `MlsStatus` exposed separate picklists and behaved as separate fields. Neither is derived
from the other. **Never substitute one for the other.** Re-read the live picklists/codes before any status implementation; matching display labels do not prove matching provider values. `StandardStatus` filters and
...
8  Record the live proof in the packet evidence / Execution State. Amend this Master only when a durable semantic rule changes.
```
...
Repository authority is GitHub, repository mallan67/mallan-nyc.
...
Vercel is Mallan's hosting/deployment/environment/Marketplace control plane.

The current project identity is the mallan-nyc project under the Mallan team. Mutable IDs, deployments and environment values belong in the Execution State and must be re-read live.

For Vercel work:

1. identify the connected Mallan team/project live;
2. inspect the current deployment/environment/integration state;
3. consult current official Vercel documentation for platform behavior;
4. distinguish project-owned environment variables from Marketplace-generated variables;
5. never infer an effective runtime value merely because a variable name exists;
6. never create or reconnect a Marketplace resource as a workaround for an unexplained state;
7. environment/resource mutation requires explicit Maya authorization.
...
RUNTIME DATABASE AUTHORITY
  bare DATABASE_URL / DATABASE_URL_UNPOOLED
  → classified endpoint identity
  → canonical Production OR explicitly approved nonproduction

MARKETPLACE METADATA / GENERATED VARS
  database_*
  → integration-owned
  → never automatically promoted into runtime authority
~~~

Unknown and stale database targets fail closed. A host string, pooled/unpooled variant, branch name or environment label does not independently grant authority.

For every environment-variable cleanup or re-scope:
...
Scope is a separate question from model duplication. Mallan operates as an independent New York City brokerage, and it does not need to duplicate every business line of a national brokerage, a developer-marketing company, a media company, a property-management company or a listing portal.

```text
WHAT MALLAN MUST DO EXCEPTIONALLY WELL

from

WHAT MALLAN DOES NOT NEED TO BECOME
```

Mallan's competitive product focus is:

```text
STRONG AGENT / BROKER RELATIONSHIP
+
DEEP CLIENT UNDERSTANDING
+
PROFESSIONAL PROPERTY SEARCH
+
PROPERTY / BUILDING / MARKET INTELLIGENCE
+
CMA / DECISION SUPPORT
+
OWNER / LANDLORD INTELLIGENCE
+
BUYER / TENANT REPRESENTATION
+
NEW-DEVELOPMENT BUYER INTELLIGENCE
+
MARKETING FOR MALLAN BUSINESS
+
DEAL / RELATIONSHIP CONTINUITY
+
POST-DEAL / FUTURE-OPPORTUNITY INTELLIGENCE
```

Each item is defined by the section that owns it — Agent/Broker relationship §2, §17 and §18; client understanding §22.4; professional property search §5; property/building/market intelligence §6 and §22; CMA and decision support §6 and §8; owner/landlord intelligence §13 and §22.10; Buyer and Tenant representation §14 and §15; new-development Buyer intelligence §4.5; marketing §9; deal and relationship continuity §3.1 and §20; post-deal and future-opportunity intelligence §22.7. **§1 states priority. It does not restate or redefine any of them.**

**This is an emphasis list, not a scope boundary.** Listing Reporting (§10), agreements and documents (§11), Agent support (§17), Brokerage View (§18), commissions/referrals/money (§19) and Transactions (§20) are in scope, are governed by their own sections, and are not made optional by their absence from the focus list.

New development is the worked example of what Mallan does not need to become. Mallan is not becoming developer marketing, sponsor sales, developer sales-office infrastructure or a new-development brokerage platform competing for project representation.
...
**Mallan does not need to be the developer's broker. Mallan should be the Buyer's better-informed broker.** The operational purpose and handling of Offering Plan / Schedule A material is §4.5.

**Mallan should not build functionality merely because Compass, Corcoran, Elliman, SERHANT. or another major brokerage has it.**

The nine questions above fire on a proposed model and ask whether a second data truth is justified. The questions below fire on a proposed capability and ask whether it belongs to Mallan at all. A capability can pass these seven and still fail the nine — build it, but on the canonical record. A capability can pass the nine and still fail these seven — it would be a clean model of something Mallan should not be building.

Before committing to any major capability, answer:

1. Does it materially improve Mallan's ability to represent a client?
2. Does it help an Agent understand or serve a client better?
3. Does it improve listing/property/market intelligence?
4. Does it help Mallan generate, convert or retain legitimate brokerage business?
5. Does it reuse Mallan's canonical Party, Property, Listing and history?
6. Is the compliance/source-rights boundary clear?
7. Does it produce a meaningful advantage that justifies maintaining it?

If not, it is not automatically part of Mallan's scope.

---

# 2. SIMPLE BROKERAGE / AGENT OPERATING MODEL

## 2.0 Mallan's actual operating model

Mallan is intentionally a lean brokerage. The software must model the firm that exists rather than invent a corporate hierarchy.

- Maya is the Representative Broker and may also be the producing Agent on the same canonical business record.
- There is no automatic Office Manager role and no support-staff layer that owns an Agent's work.
- Salespersons and Associate Brokers operate as producers unless Maya explicitly creates a separate supervisory appointment.
- Agents remain responsible for their own day-to-day professional work, records and follow-through inside Mallan's governed platform.
- Brokerage supervision, approval, compliance, canonical records and auditability remain Mallan/Broker responsibilities.
- A future staff/manager capability must be explicitly authorized; it may not appear because a generic brokerage template assumes one.
...
## 3.0 Client/Party data lifecycle and disposition

Canonical identity includes a governed end-of-active-use state. "Inactive" is not permission to keep personal data everywhere forever, and "delete" is not permission to destroy records Mallan is legally/business required to retain.

Disposition classes:

~~~text
ACTIVE
→ operational CRM/search/portal/marketing use permitted by role/consent

INACTIVE
→ relationship retained, ordinary active workflows stopped

RETAINED-RESTRICTED
→ retention obligation exists
→ minimum required record retained
→ excluded from ordinary operational consumers

ANONYMIZED / DELETED
→ no retention requirement for the personal fact
→ remove/anonymize through a proven reader/writer impact graph
~~~

Retained-restricted records must not silently remain available to CRM pickers, Search/Saved Search, recommendations, CMA/report generation, marketing audiences, assignment, dashboards, tasks, portal/auth invitations, exports or automations merely because a row still exists.

Historical transactions, executed agreements, audit evidence and legally required records retain only the identity/context required for their retention purpose. Data-disposition work must census every reader/publisher before removal so history remains truthful without keeping unnecessary active-profile access.
...
## 3.5 Agent responsibility on canonical records

The canonical record is shared. Responsibility for the work on it is not.

Every canonical record that carries active business work identifies the Mallan Agent responsible for it, or is explicitly recorded as unassigned. That responsibility relationship is what places a shared record inside a producer's My Business under §2.1 and §2.5, and what Brokerage View supervises firm-wide under §18. It never creates a second record, a per-Agent copy or a per-Agent system.

```text
Lead / Inquiry              → assigned Agent — §19.1
Role Opportunity            → responsible Agent
Listing Episode             → responsible listing Agent — §17.6
Search / Saved Search       → owner Agent + client/opportunity — §5.8
Transaction / Deal          → responsible Agent — §19.6
Commission / Referral       → responsible Agent — §19.9
Task / Calendar / Reminder  → assigned Agent
```

This is a responsibility map over records §3 already carries, not a new object set. Where an existing canonical model or field already carries the responsibility, use it. **Do not create a per-Agent client, listing, search, deal, task or money store to express responsibility**, and do not add schema for it outside the questions in §1 and the authorization rule in §27.12.

Explicitly unassigned is a recorded state, not an empty field. It is not an accepted resting state: §18 continues to raise a Lead with no assigned Agent as a firm exception, and recording the state does not retire that exception.

Reassignment moves the responsibility, not the record. The rule in §17.6 holds generally: reassignment moves ownership of the work and does not alter the canonical Party, Property, Listing or Transaction identity underneath it. A prior responsibility relationship survives reassignment as history rather than being overwritten.

A Task, follow-up or reminder carries the Agent it is assigned to and uses the canonical Tasks / Calendar / Reminders object. §13 already forbids a role-specific reminder mechanism; an assigned Agent does not create one.

Two decisions sit under this rule and are not made here:

- **Whether one Deal may carry more than one responsible Agent.** §19.6 already provides for internal co-Agent allocation where applicable. Whether a co-Agent also holds a responsibility relationship that places the same Deal in a second Agent's My Business, or whether co-Agent participation remains a compensation allocation only, is a Broker decision that has not been made.
- **Whether Lead is a canonical object.** §5.8 records that decision as open, while §3 above carries Lead / Inquiry in the canonical shared foundation and §19.1 calls a Lead a first-class record. This rule reads only the assignment §19.1 already requires; it does not close, restate or retire the §5.8 decision.
...
New development / Schedule A is **Buyer-representation intelligence**. Mallan is not assuming the role of sponsor, developer, developer sales office or listing broker merely because Mallan holds Offering Plan or Schedule A information. Developer/sponsor representation, project marketing and developer sales-office infrastructure are outside current scope unless Maya explicitly adds them.
...
MALLAN BUYER
↓
FULLER NEW-DEVELOPMENT INTELLIGENCE
↓
BUILDING / OFFERING PLAN
↓
SCHEDULE A UNIT UNIVERSE
↓
CURRENT COTALITY / MARKET LISTINGS
↓
AUTHORIZED SUPPLEMENTAL INFORMATION
↓
CURRENT AVAILABILITY VERIFICATION
↓
PROPERTY / UNIT COMPARISON
↓
BUYER DECISION SUPPORT
```

This gives a Mallan Buyer a fuller understanding of the building and its potential unit opportunities than an active-listing-only Search can produce.
...
# 5. SEARCH — PROFESSIONAL BROKERAGE INFRASTRUCTURE

## 5.0 One Search foundation, one result population

Search is infrastructure for the brokerage. Basic Search, Advanced Search, Building Search, Map, Saved Search, Compare, CMA candidate selection, reports and client matching are views/actions over one corrected Search foundation — never separate engines with separate field truth.

The canonical search order is:

~~~text
USER / SAVED CRITERIA
→ VERIFIED FIELD / RESOURCE CONTRACT
→ SOURCE QUERY
→ MALLAN POST-FILTER FOR PROVIDER-REFUSED CRITERIA
→ RIGHTS / VISIBILITY / COMPLIANCE
→ CANONICAL IDENTITY + DEDUPE
→ ONE RESULT POPULATION
→ SORT / PAGINATION / MAP / REPORT / CMA / SHARE
~~~

A criterion the provider cannot filter server-side is not silently dropped. Where Mallan is authorized to receive the fact, retrieve the required fields and apply the criterion in Mallan's layer. If the fact is not available/authorized, refuse or mark the criterion unavailable visibly.

## 5.0.1 Building Search is first-class

Building Search is not a text shortcut layered over listing Search. It resolves canonical Building identity, then uses the same Property/Unit/Listing/Search foundation to expose building facts, available/recent units, media, ownership/property intelligence, comparables and client opportunity context.

Cotality Building availability may be incomplete or unavailable on the current entitlement; Building Search therefore uses verified source observations and Mallan's canonical Building identity without inventing a second building database.

## 5.0.2 Map and transportation are part of Search correctness

Map, neighborhood/location boundaries and transportation context are not optional decoration. They are part of professional search and reporting. When provider geography is unavailable, Mallan's governed geography layer supplies geocoding/polygons/location relationships with provenance.

## 5.0.3 Search output actions

From one result population an Agent can, subject to rights and client context:

- open the professional Listing/Opportunity workspace;
- assign/share with a Client;
- save/update a Saved Search;
- compare selected properties;
- create/use CMA candidates;
- create showing/open-house workflow;
- email/share/print a governed result/report;
- add a durable comment or client-listing disposition.

These actions may not fork result identity or recreate provider data.
...
`NEW DEVELOPMENT / SCHEDULE A` is a Buyer-representation mode under §4.5.4. It exists to help an Agent representing a Buyer, and it remains a view over the one canonical Search foundation rather than a second new-development Search engine.
...
Where a unit has both an offering-plan price and a current market ask, the result carries them as two separately labeled facts. They are never collapsed into one price, sorted as one price, or presented as a price change from one to the other.
...
Mallan has one CMA/Property Intelligence engine built on the corrected Search foundation. It may expose three broker-approved presentation/workflow variants, but those variants share the same subject identity, market universe, comp facts, source hierarchy, adjustment logic, calculations, provenance and version history. A presentation variant may not become a second CMA engine.

The exact user-facing names/layout of the three variants may evolve without changing this invariant.

### Sale evidence layers

For sale valuation:

~~~text
CLOSED / ACHIEVED TRANSACTIONS
→ final valuation comp evidence

ACTIVE / PENDING / IN-CONTRACT MARKET
→ current competition and market-position context

EXPIRED / WITHDRAWN / HOLD / REMOVED / TOM HISTORY
→ separately labeled market-resistance / history evidence
→ never silently treated as an achieved sale
~~~

Current provider availability and exact status strings are re-verified through §0 before use.

### Rental evidence layers

Rental CMA distinguishes asking-market evidence from achieved-rent evidence where achieved terms are actually available/authorized. Gross asking rent, concessions/net-effective rent and achieved rent must not be collapsed.

### Three CMA presentations, one truth

The three required CMA presentation modes are a product/presentation requirement, not permission to duplicate data or methodology. Until Maya approves a label/content change, all modes must preserve:

- Subject Property;
- market universe;
- selected comps and exclusion reasons;
- source/provenance;
- adjustments/assumptions;
- output range/strategy;
- version/as-of time;
- creator/report-author identity;
- client-safe transformation and attribution.
...
For a new-development subject under §4.5.4, the sponsor/offering-plan set, current resale competition and closed resale evidence in the same building are three distinct bodies of evidence and stay distinct. The sponsor offering and a resale of the same unit line are not one market. The subject building is also positioned against competing new developments in the same segment, and where the source supports it the analysis compares by unit line and by floor/exposure inside the building.
...
For a new-development Buyer, the result must be able to answer the Buyer's actual questions:

```text
HOW DOES THIS UNIT COMPARE TO OTHER UNITS IN THE BUILDING?
HOW DOES THE OFFERING COMPARE TO CURRENT RESALE INVENTORY?
HOW DOES THIS BUILDING COMPARE TO COMPETING NEW DEVELOPMENTS?
WHAT DOES THE CURRENT MARKET SEGMENT LOOK LIKE?
WHAT COST DIFFERENCES MATTER?
WHAT COMES FROM THE OFFERING PLAN AND WHAT COMES FROM A CURRENT LISTING?
WHAT REMAINS UNVERIFIED?
```

Building, segment, price-band and carrying-cost answers come from the §6.3 axes; the offering-versus-resale and competing-development answers come from the new-development evidence separation in §6.3; the source and unverified answers come from the §4.5.5 field-by-field composition, the §6.5 source hierarchy and the §4.5.4 availability states. §6.7 presents them and does not build a second analysis model.
...
# 7. LISTING INTAKE / BACKEND LISTING WORKSPACE

## 7.0 Sale and Rental Listing Intake — one canonical writer

Sale and Rental intake are first-class brokerage operating workflows. They write the same canonical Mallan Listing used by Search, Workspace, Open Houses, CMA, Marketing, Reporting, Portals and Public Web.

There is no separate "form listing" truth.

Required flow:

~~~text
ADDRESS / BUILDING RESOLUTION
→ OWNER / LANDLORD PARTY + owner_client_id
→ LISTING EPISODE
→ SALE OR RENTAL BUSINESS FIELDS
→ AGENT / OFFICE / CO-LISTING
→ MEDIA / DOCUMENTS
→ OPEN HOUSES
...
### Required intake capabilities

The Sale and Rental workflows must support, where applicable and source/rights-verified:

- canonical address/unit/building selection;
- Seller/Landlord owner link through Listing.owner_client_id;
- listing Agent, office, co-listing Agent/office;
- property/listing facts and economics;
- dates and governed status/transition state;
- remarks and attribution;
- media and floor plans with rights/provenance;
- showing instructions/access;
- Open Houses, including Public, Broker Only, Virtual and **By Appointment**;
- documents/disclosures;
- compensation terms in their correct separate layers;
- Save Draft and Activate/Publish actions with explicit eligibility.

"Open House By Appointment" remains a public open-house designation requiring an appointment/RSVP; it is not silently converted into Broker Only.

Provider-facing status must use the exact current Cotality contract/mapping. UI/business labels such as Active, In Contract, Closed/Sold, Withdrawn or Canceled are presentation/business vocabulary and may not be substituted into provider queries without live verification. Do not invent an Expired rental state merely because an old schema includes one if the authorized provider returns none.

### Sale / Rental Tools are views, not duplicate forms

Professional Sale Tools and Rental Tools read the same canonical Listing/Search/Building/Calculator/Report data. They may expose transit/location, economics, compare/CMA, print, email/share and client-safe outputs, but they may not maintain separate listing values.

### Downstream completion

A Listing Intake change is incomplete until the same saved value is proven where relevant in:

- backend Listing Workspace;
- Agent Search / own-listing result population;
- CMA/compare;
- Open House/shows;
- CRM owner/client workspace;
- Marketing/e-blast/share;
- Seller/Landlord report;
- Client Portal;
- Public Web/SEO if publication-eligible;
...
No control, label or action here may imply that Mallan is the sponsor, the developer, the developer's sales office or the listing broker for the unit unless Mallan actually holds that authority. Existence of a unit in Schedule A does not produce an `Edit Listing` control, because it does not make the unit a Mallan listing.
...
- **Target purchase range** answers what a client could purchase in, from that client's own stated resources, the financing structure being considered and, for an owner, the equity released by a sale and the timing of a 1031 where one applies. It is one calculation with one home: the Tenant-to-Buyer analysis of §15 and the Landlord investment branch of §13 read the same range rather than a buyer version and an owner version;
- **Monthly ownership cost** answers what owning would cost each month — payment, tax, maintenance or common charges, insurance where applicable and the other explicit carrying-cost assumptions rolled up as one figure. It is the figure a rent-v-buy comparison is compared against, and it travels with the assumptions that produced it rather than standing alone;
...
└── BROKERAGE MARKETING
    ...
    ├── public brokerage-service and client-education content
    └── other Broker-approved business materials
```

Public brokerage-service and client-education content explains what Mallan does for a client — how the firm represents a Buyer, Seller, Landlord, Tenant or Investor and what a given service actually involves — rather than advertising one property. It is a Toolkit output like any other: it carries the governed professional identity of §2.4, runs the §9.7 compliance routing before it reaches a public or client-directed surface, and uses the same versioned Broker-approved template governance as §11.4 and §11.5. It is not a second website content system with its own approval path.
...
**Mallan markets new-development Buyer representation.** A Toolkit output may not present or imply Mallan as the sponsor, the developer, the developer's sales office or the listing brokerage for a building or unit where Mallan does not actually hold that authority, and holding an Offering Plan or Schedule A is not that authority. What the output may claim is the service: that Mallan researches new developments, explains Offering Plans, compares units and unit lines, analyzes Schedule A, compares new development with resale, explains carrying costs, evaluates purchase scenarios, identifies appropriate opportunities, coordinates with the appropriate listing or sales professional and independently represents the Buyer. **That is a brokerage service proposition, not a claim of listing authority.** The same rule governs any output describing Mallan's role in a transaction Mallan does not hold. The advertising requirement behind it is carried in the Rule Registry under its issuing authority like every other rule this section applies.
...
**Schedule A intelligence is richer internally than publicly, and marketing does not close that gap.** A Schedule A or private supplemental unit does not become a public Mallan listing page because a marketing output was composed for it. Whether it may reach a public surface at all is the §4.5 exclusion and the §5.1 Search boundary, unchanged here. Where public display is separately authorized, marketing proves before advertising the unit as available: current availability verified rather than inferred from the existence of the plan; Mallan's own advertising or share authority for that unit; source rights; required attribution; media rights; current price and status; and the applicable REBNY, RLS, Cotality and advertising rules the Rule Registry holds for that surface and audience. Publication then runs the §21.10 gate in order like any other listing.

Where any of those is unproven, the unit keeps its working classification:

```text
AGENT RESEARCH / BUYER-REPRESENTATION INTELLIGENCE
```

It stays Agent-private and client-share-gated under §4.5.7. It is not published with the unproven parts removed, and it is not published as a building or plan page a consumer would read as available inventory.

---

# 10. LISTINGS REPORTING SYSTEM

## 10.0 Reports are live views over the canonical Listing

Seller and Landlord reporting is directly tied to the live Mallan Listing and its canonical activity. The report system is not a second database and must not maintain manually copied counters that drift from the listing/workspace.

The authoritative report input graph is:

~~~text
LISTING + OWNER/LANDLORD via owner_client_id
├── current listing facts / price / status / dates
├── Listing change history
├── Marketing campaigns / sends / delivery state
├── Inquiries / leads attributable to the listing
├── Showings
├── Open Houses / RSVPs
├── Showing / Open House feedback
├── client/listing engagement that Mallan is permitted to use
├── offers / applications / material deal activity
├── Search/CMA/market-position evidence
└── Agent comments/recommendations that are explicitly authored
        ↓
ONE REPORT ENGINE
        ↓
INTERNAL REPORT / SELLER REPORT / LANDLORD REPORT
        ↓
VIEW / EMAIL / PRINT / PDF OR OTHER APPROVED DELIVERY
~~~

Report generation reads the current canonical records at generation time. A delivered report becomes a versioned "as of" snapshot so the client can later see what was reported at that time. A later live change produces a new version; it does not silently rewrite the historical delivered report.

A missing or unavailable source fact is **unknown/unverified**, not zero.

### Seller report live content

Seller reporting can include, when actually supported by canonical data:

- Executive Summary;
- current listing status/price and material changes;
- Marketing Activity;
- Buyer/Market Engagement;
- inquiries;
- showings and Open Houses;
- feedback themes without fabricating sentiment;
- offers/material activity;
- market position and verified comparable/competition context;
- Agent/Broker recommendation and next steps.

### Landlord report live content

Landlord reporting uses the same live-listing foundation with rental-specific facts:

- current rent/terms/status and changes;
- marketing/distribution;
- inquiries;
- showings/Open Houses;
- applicant/application activity where appropriate;
- feedback;
- competing rental/market context;
- lease/listing timing;
- Agent/Broker recommendation and next steps.

A Landlord report never becomes a separate tenancy ledger; after a lease is completed the continuing owner/tenancy lifecycle remains on the canonical Party/Property/Transaction history.

### Report author and truth

The report author comes from the governed Agent professional identity. Provider facts retain provider/source attribution. Mallan-authored analysis is identified as Mallan/Agent analysis. AI may draft narrative only from the verified report fact set and may not manufacture activity, feedback, market facts or recommendations.

All CMA, listing, market/share and client report outputs reuse common rendering/delivery/provenance infrastructure wherever the business contract overlaps. Do not build parallel report engines for each screen.
...
## 11.0 Document separation and brokerage/agent operating records

Agency disclosures, Fair Housing disclosures and other required disclosures remain distinct governed records from the representation/listing agreement even when delivered together operationally.

Brokerage ↔ Agent records are also a governed document family, not ad-hoc uploads. Depending on the person's license/relationship they can include:

- Independent Contractor Agreement;
- Agent / Associate Broker Policy;
- Policy Acknowledgment;
- compensation/commission plan;
- referral policy/agreement;
- confidentiality and technology/trade-secret obligations;
- E&O proof where firm policy requires the licensee to maintain their own coverage;
- license / professional-requirement proof;
- continuing-education record/reminder appropriate to the person's actual license type;
- W-9 and onboarding records;
- offboarding/association-change records.

These documents reuse the canonical Agent/professional identity and immutable executed-document history. Do not create a second "agent documents" identity store.
...
**Decided — §20.3's rule reads as the prohibition on contextless documents, not as a requirement of Transaction context outright.** A document always carries business context; which context it carries depends on the stage the obligation arose at. The interim rule above is the standing rule: an early-stage required document attaches to the Client/Opportunity — and to the Property/Listing where one applies — that generated the obligation. When a Transaction later exists, that record is linked to the Deal context. **It is not copied into a second document truth.** Carrying forward is a link, on the same rule §20 already states at acceptance and §11.9 states for the one canonical document infrastructure.
...
The annual contractor tax reporting form Mallan issues an Agent for a tax year is a member of this class and runs on the same infrastructure as the rest of it: catalog record and version under §11.4, generation, delivery and recorded supply under §11.7, executed original, correction/replacement version and retention under §11.8. A corrected form is a new version. The form as originally issued is preserved, its issued date and delivery/access state are recorded, and prior years remain retained and readable.

**Tax truth comes from payment truth.** §19.6 remains the single authority for what Mallan actually paid an Agent, its paid date and its tax year, and §19.9 for referral fee terms and payment state. The reporting form reports those records for the tax year and holds no annual total of its own. **A hand-entered annual figure may not become a second payment truth.** The read direction is one-way: the reporting record reads §19.6 and §19.9, and neither reads back. This is the rule already stated in §1 and §19.8 applied to tax reporting, not a new one.

The exact form, reporting threshold, filing and furnishing deadlines and tax treatment come from then-current authoritative tax rules. No form number, dollar threshold or date is hard-coded from this plan. **Held for Maya decision — where the tax rule is registered.** Every other then-current rule in this plan carries a §21.3 Rule Registry record, but §21.1's authority stack and §21.3's rule families are a New York law, REBNY/RLS/UCBA, provider and source-terms set with no tax family. Maya decides whether §21.3 carries a tax rule family or whether the tax rule is governed outside §21. Do not enlarge §21's charter by side effect.

Reporting needs the Agent's reportable identity as data and not only as a stored image: the name as reported, whether the payee is an individual or an entity, and the taxpayer identification number. These are **SENSITIVE / LEGAL RESTRICTED** under §11.1. Access otherwise follows §2.5 — the Agent reads the Agent's own tax forms and the Broker reads the firm's — on the same reasoning §19.9 uses to refuse treating an Agent's own money as Broker-only. **Held for Maya decision — who may read an Agent's taxpayer identification number.** That is narrower than an Agent's own money, and this plan sets no masking, access or retention standard for one. It is decided before implementation rather than inferred here.

Tax-record retention is not the three-year brokerage-record minimum in §11.8. The applicable period comes from then-current authoritative tax rules under that section's own override clause and is verified rather than guessed.

**This class remains subject to the HR-boundary decision held at the end of this section.** The reading here is that a contractor tax form is payment administration and an affirmation of the independent-contractor posture in §2.2 rather than a departure from it: Mallan reports money §19.6 already records it paying, and adds no payroll, performance-management or employment-file function. That is a reading, not the decision.

The HR-boundary decision recorded at the end of this section adopts that reading.
...
A Seller Opportunity does not always begin at a Lead. One may also open from an existing Owner or Landlord relationship — at the lease-expiration decision point in §13, or on a role-transition signal under §22.7. The canonical Party already exists in that case and Party Resolution is already satisfied; the Opportunity opens on the existing identity and the rest of this journey runs unchanged. **Do not manufacture a Seller Lead to give an existing client a door into §12.**
...
├── BUY ANOTHER INVESTMENT PROPERTY → INVESTOR / BUYER OPPORTUNITY
├── SELL + REINVEST → SELLER OPPORTUNITY + INVESTOR / BUYER OPPORTUNITY
├── HOLD / REVIEW LATER → DATED NEXT REVIEW
└── OTHER BUSINESS DECISION
```

Every branch stays on the same canonical Landlord Party. **A branch that leads to a different property opens a further Opportunity on that same Party; it never creates a second owner.**
...
Monitoring runs on a lead time, not on the expiration date. **Approximately six months before lease expiration**, the monitored expiration raises the Agent action. Six months is the stated business lead time for both sides of the lease and is set once, here; §15 works the Tenant side of the same window, and §18 and §22 read it rather than restating it.

The six-month event is a decision point. **It is not automatically a new Listing and it is not automatically a new Opportunity.** The Opportunity rule is §22.7 — every transition routes through Agent review, and Mallan does not open an Opportunity, start a campaign or contact a client because a signal fired. The listing side is held to the same restraint: a monitored expiration never creates a rental Listing Episode. A new episode requires the owner's decision and the representation and authorization steps above.
...
RE-RENT re-enters this journey at the representation and authorization steps above. It produces a new rental Listing Episode on the same canonical Property when the owner authorizes one, under a new or renewed Landlord Opportunity for the new term. Prior episodes are retained, not replaced. **Re-renting creates neither a second Property nor a second Landlord identity.**
...
BUY ANOTHER INVESTMENT PROPERTY opens an Investor or Buyer Opportunity on the same canonical Party. Unlike SELL it concerns a **different** Property, so it opens nothing against this one and leaves the Landlord relationship and the tenancy running. It is a new role Opportunity under §1, not a converted Landlord Opportunity, and the acquisition search, analysis and workflow behind it are §16 and §5.8. §13 is one entry point into that chain and does not restate what is inside it. A Property acquired through it returns to §13 as a new Landlord relationship on the same Party once it is rented.

SELL + REINVEST is the one branch that produces two Opportunities at once — a Seller Opportunity on this Property and an Investor or Buyer Opportunity for the replacement — held on the same canonical Party, with the timing of one a constraint on the other. It is also the one branch where a 1031 question arises. §8 and §16 own the replacement and 1031 analysis; what §13 records is that the two Opportunities are one owner decision and must remain readable as one.

OTHER BUSINESS DECISION is a recorded owner outcome, not an unhandled state.

HOLD / REVIEW LATER is the named form of that outcome where the owner decides nothing changes for now. It is a decision, not silence, and it is not a closed review: it carries a dated next review on the canonical Tasks / Calendar / Reminders object, and the relationship stays in Tenancy Period until that date or an earlier owner decision. OTHER BUSINESS DECISION remains the residual for owner outcomes that are none of the branches above.
...
Lease start and lease expiration are facts of one relationship, not two records that happen to agree. The Landlord Party, the Tenant Party, the Property/Unit and the lease term are joined once, and the Landlord-side view here and the Tenant-side view in §15 read the same dates. **A lease date corrected on one side is corrected on both, because there is only one of it.**

**Held for Maya decision — whether the tenancy is its own canonical object.** §26 states that if an ongoing tenancy is to be held as its own canonical state it is added to §3 through the §1 questions and not invented inside a rental screen, and §3's canonical object list carries Listing Episode, Documents/Agreements and Transactions but no Lease or Tenancy object. The joined relationship above is therefore written as facts held on the Landlord relationship and read by §15, which is what the current object list supports. If Maya intends a canonical Lease / Rental Deal object joining both Parties to the Property/Unit, it is added to §3 in the same decision and §26 records that the question is answered. Until she decides, **do not create it inside §13.**
...
The reverse is equally required. The Tenant workspace does not expose the Landlord's Opportunity, the owner's rent strategy or the owner's hold-v-sell analysis. Landlord and Tenant are two Parties in two separate first-class relationships under §1, each with its own confidential record, and the link between them carries occupancy and lease facts only. **The visibility class across this boundary is PARTICIPANT RESTRICTED under §11.1**, and only what is deliberately client-shared to a given side crosses it. A shared lease is not shared confidence.
...
At the six-month event the Agent is prompted to contact the owner, and Mallan assembles one owner conversation rather than sending the Agent to gather it surface by surface. The review is composed from work the plan already owns and prefilled from the canonical Property and the Landlord relationship:

```text
CURRENT PROPERTY INFORMATION — §3
CURRENT VALUE / CMA CONTEXT — §6
RECENT RELEVANT SALES — §6.3
CURRENT COMPETING SALE INVENTORY — §6.3
CURRENT RENTAL MARKET / RENTAL COMPETITION / RENT CONCLUSION — §6.3 RENTAL CMA + §6.7
KNOWN CURRENT RENT — §13
RENT HISTORY / PRIOR RENTAL LISTING EPISODES — §13
PRIOR-EPISODE RENTAL PERFORMANCE — §10.4
CURRENT CARRYING COSTS — §8
HOLD-v-SELL / SELL-v-RE-RENT — §8
CURRENT EQUITY / VALUE CONTEXT — §8
NOI / CAP / CASH-ON-CASH — §8
TARGET PURCHASE RANGE — §8
1031 REPLACEMENT ANALYSIS WHERE APPLICABLE — §8 / §16
CURRENT INVESTMENT PROPERTY OPPORTUNITIES — §5.8 SAVED SEARCH
RENEW / RE-RENT / SELL / BUY / REINVEST / HOLD — §13
```

Every line is a reference to where the work is done. **§13 computes none of it and stores no second copy of any of it.** The review is assembled at the moment of the conversation from the owning sections, and each value carries the truth/provenance labeling of §10.5 and §6.11 wherever it is shown. Its purpose is the owner decision in the branch above; it is not a report Mallan sends and it decides nothing by itself.

The current investment opportunities in the review are a Saved Search over the one canonical property universe under §5.8. §16's prohibition on a separate investor property or search universe applies here unchanged.

**Held for Maya decision — who presents the calculated lines.** §6 records that carrying cost and its sensitivity are computed by §8 and presented by §6.7, and that this ownership split must be confirmed before either section is built. This review puts carrying cost, hold-v-sell, equity, NOI, cap rate, cash-on-cash and the purchase range in front of an owner from a third surface. Maya decides whether §13's review presents §8 output under the same rule §6.7 follows, or whether the review is itself a §6/§8 output that §13 only branches on. Until she decides, treat it as presentation only: **§8 computes, §13 presents and branches, and no formula is written into §13.**
...
→ Backend Buyer Search
   ...
   │   ├── current Mallan-authored inventory
   │   └── current third-party Cotality inventory
...
The Buyer does not need to understand which technical source found the property. The Agent does: which source produced a fact, and how well that fact is verified, is what makes the advice correct, so source, currentness and rights state stay on the professional record whether or not they appear in the client presentation. **Sparing the Buyer the plumbing is not permission to drop a required attribution or to present unconfirmed availability as active** — §5.18 governs what may and may not cross into a client-facing payload.
...
├── NOT SURE YET → RECORDED OUTCOME / DATED NEXT REVIEW
...
The lead time is the one §13 sets: **approximately six months before lease expiration**, on the Tenant side as well as the Landlord side. It is one business rule read by both journeys, not two windows that can drift apart.

The two conversations are separate. The Agent prepares for the Tenant and for the Landlord independently, neither is evidence for the other, and neither crosses the confidentiality boundary of §13. The Tenant conversation does not assume the Tenant stays a renter; renew, relocate, buy and undecided are all outcomes of it. **NOT SURE YET is a recorded client outcome, not an unhandled state**, and it carries a dated next review on the canonical Tasks / Calendar / Reminders object.
...
At the six-month conversation that evidence is prepared rather than described. **Rent vs Buy / Cost to Become an Owner** is the client-facing report form of the §8 rent-v-buy preset — one analysis, one engine, one saved scenario — presented to the Tenant. It is not a second calculator and §15 holds no formula of its own. Its inputs and outputs are the §8 role presets — rent-v-buy, annual rent cost, target purchase range, down payment, cash-to-close, mortgage and payment, carrying cost, monthly ownership cost and appreciation/equity — prefilled from the actual Property and from current verified or explicitly stated assumptions under §8. §15 names the report; §8 owns every number in it.

**Do not present an assumption as a guarantee.** An equity or appreciation scenario shown to a renter is a projection over stated assumptions and is labeled as one. This is the client-output counterpart of §22.6: present the evidence, and do not turn a scenario into a promise.

**Held for Maya decision — the provenance labels on a client-facing analysis.** §6.11 names the labels a client-facing number carries — sourced fact, deterministic calculation, system analysis, Agent assumption, Agent professional conclusion — and states that they are the CMA-facing expression of the §10.5 truth/provenance categories, with the hard rule that no second provenance vocabulary is created. A rent-v-buy report needs two distinctions visible on its face that the current wording folds inward: a client-provided input, which §10.5 carries as `CLIENT / AGENT ENTERED`, and a current verified rate or cost, which §8 requires by effective date. Maya decides whether those become their own labels inside the §10.5 and §6.11 set or stay inside the existing ones. Until she decides, **§15 labels every value with the §6.11 vocabulary and defines none of its own.**
...
## 16.0 1031 / investor intelligence uses canonical identity

Investor/1031 work must not create a separate contact database, property universe or Search engine.

Authorized public-record, transaction, client and source signals may identify a potential exchange/investor opportunity, but the result resolves into:

~~~text
SOURCE SIGNAL
→ CANONICAL PARTY / ENTITY
→ TRANSACTION-SIDE FACT
→ INVESTOR / 1031 OPPORTUNITY
→ PROPERTY / LISTING / SEARCH / CMA / CALCULATOR
→ OUTREACH / RELATIONSHIP HISTORY
~~~

Signal type/confidence and actual transaction side are separate facts. A review/lead signal may not overwrite whether the underlying transaction is buyer-side or seller-side. Unallocated amounts, relinquished/replacement-property context and timing calculations remain sourced/provenanced rather than guessed.

Prospecting/import work deduplicates against Party/Entity and keeps consent/outreach/compliance state in the canonical relationship history.
...
A Schedule A / new-development opportunity under §4.5.4 may feed an Investor Opportunity where appropriate:
...
INVESTOR CRITERIA
↓
RENT / CARRYING-COST / NOI / RETURN ANALYSIS
↓
PROPERTY / BUILDING / MARKET INTELLIGENCE
↓
BUYER / INVESTOR DECISION
```

The return analysis is §8's and the property/building/market intelligence is §6's, run on the same canonical Building/Unit the Buyer path uses. There is no separate Investor new-development database or unit universe — that is the rule above applied to Schedule A, not a second rule.
...
Investor business does not depend on a new Investor Lead. An Investor Opportunity may open from a relationship Mallan already holds — a current Owner or Landlord, a past Seller, a closed Buyer, a past client — on the Party that already exists, as a new role Opportunity rather than a new client. This is the closing chain in §1 applied to the Investor role. §12 owns post-close Seller eligibility, §14 owns the closed Buyer becoming an Owner, §13 records the owner's business decision at expiration, and §22 owns the signals that surface any of them; §16 owns only the Investor Opportunity that results and does not restate them.

**Evidence is not intent.** A saved hold-v-sell, sell-v-re-rent or 1031 replacement analysis under §8 is one evidence point toward an Investor conversation and never the Opportunity itself; §8's rule that opening a calculator is not a statement of client intent applies here unchanged. Mallan states no 1031 tax treatment, timeline or eligibility conclusion — §22.3 forbids binding legal/tax conclusions, and the applicable rules are then-current authority rather than Master Plan text.

The resulting Investor Opportunity is attributed to the Agent who holds the underlying relationship and appears in that Agent's My Business under §2.5 as the Agent's own Opportunity. That is ordinary ownership of the Agent's own book and does not resolve the cross-role presentation question held for Broker confirmation below.
...
## 17.0 Mallan Agent operating policy

The platform reflects Mallan's actual lean operating policy:

- no automatic Office Manager/support-staff workflow;
- Agents and Associate Brokers perform their own ordinary brokerage work and follow-through;
- Associate Broker is a license class, not an automatic supervisory role;
- professional requirements/reminders are determined by the person's actual license type;
- where Mallan policy requires individual E&O coverage, the Agent/Associate Broker maintains and documents their own policy rather than assuming the firm provides it;
- technology, source logic, canonical mappings, proprietary brokerage workflows and confidential system information are protected under the governed confidentiality/technology obligations;
- Brokerage View may supervise/approve where law, policy or risk requires it without becoming the Agent's day-to-day task operator.

Onboarding and offboarding must update the same canonical Agent identity used by auth, CRM ownership, listings, reports, commissions, referrals, professional profile and historical transactions.


The system should make professional obligations visible and actionable without turning Mallan into an HR system.

That boundary is narrow rather than broad. Mallan governs the contractual, licensing, regulatory, tax and brokerage documents needed to operate licensed independent-contractor Agents and Associate Brokers — the class carried in §11.12, which is not relisted here. **Mallan is not building employee payroll, benefits administration, vacation/time-off management, employee HR files unrelated to brokerage operation, or employee disciplinary/performance-management bureaucracy.**
...
The departure year's tax reporting obligation survives deactivation. Mallan issues the departed Agent's applicable annual tax form for the year of departure, and the Agent retains access to that form and to prior years under §11.12.
...
Every exception below is read from the record that already owns it. Brokerage View is oversight and business support: it is not a second client roster, not a second lead queue and not a second lease database.
...
- lead outcome by source and by campaign, read as the §19.2 performance computation rather than as a marketing count;
...
- lease-expiration outreach that has not happened or has not landed — the Landlord not yet contacted, the Tenant not yet contacted, no response to the outreach and follow-up past its date — against those same cycles;
- expected renewal and expected re-rent, read as the renew/re-rent outcome states §13 already carries, so the Broker sees turnover before the unit is vacant;
...
- role-transition signals from §22.7 — a Tenant who may be ready to consider buying, a Landlord who may be ready for a Seller conversation — carried here under the §22.7 rules that Mallan does not invent intent and that every transition routes through Agent review;
...
- Leads with no movement since assignment — assigned but not yet worked, or worked with no next action dated — read from the last-activity and next-follow-up facts in §19.1;
...
## 19.0 Relationship protection before assignment

Before a new inquiry is assigned, resolve Party identity and inspect the existing Mallan/Agent relationship. A second form submission, email address variation or new inquiry source must not casually reassign an existing client to a different Agent.

Reassignment, when authorized, carries the full relationship/history forward; it does not restart the client as a new person.

## 19.0.1 Existing referral workflows are extended, not replaced

Incoming and Outgoing referral workflows remain the canonical referral surfaces. Repair/extend them rather than creating a second referral database.

The canonical referral record supports:

- incoming/outgoing direction;
- referring/receiving people and firms;
- Agent ownership/access;
- agreed referral fee terms/percentage;
- expected/calculated amount;
- stage/progress;
- persistent check-ins;
- next follow-up;
- expected close/completion;
- fee due/paid/received status;
- Broker firm-wide visibility/approval where required.

Referral financial state stays linked to the canonical Party/Opportunity/Deal and commission truth.
...
The same history also retains, as applicable:

- the existing-relationship check result recorded at assignment;
- Search or content context, where the Lead came from a Saved Search, a market report or another authored output, with the Saved Search lineage itself governed by §5.8;
- consent and communication eligibility, read from the Party under §3.2 rather than kept a second time on the Lead;
- date and time of last activity;
- next follow-up date;
- on any change of Agent: the Agent the work moved from, the Agent it moved to, date and time, the reason and the actor who made the change.

Assignment history is append-only. A reassignment appends the new decision; it does not overwrite the assignment it replaced. The shape is the one already used for referral check-ins in §19.9 — actor, timestamp, change, note and next follow-up.

**Every Lead has a named responsible Agent or is explicitly unassigned.** Ownerless by accident is not a state; the unassigned case is a recorded choice §18 already watches. Whether Lead becomes a canonical object carrying these facts as its own fields, or remains the assignment history described here, is the open decision recorded in §5.8 — the facts are required either way.

**Lead is that canonical object.** §3 carries Lead / Inquiry in the canonical shared foundation and §3.1 resolves it to the canonical Party; the facts above are its own.
...
Conversion does not end the relationship either. FUTURE above is a dated pre-conversion intent state on a Lead that has not converted; the relationship that continues after a deal closes is a different thing, governed by §12, §13 and §15 and noticed early by the role-transition signals in §22.7.
...
Mallan originates business as well as receiving it. Lead generation — the public website and listing pages, campaigns and authored marketing, prospecting, past clients and referrals, and the Agent's own book — is a brokerage capability, not a marketing side activity, and a generated Lead is the same Lead record under the same facts above. A lead reported as generated is a Lead that exists; §9.4 continues to own after-the-fact engagement measurement.
...
**Decided — the restraint limits routing only.** It governs the routing mechanism. The lifecycle around it — capture, Party resolution, contact, qualification, role and intent, Opportunity, follow-up, representation, active business and the converted/lost/nurture/future outcomes — is the business relationship, and a rule written about distribution does not narrow it. Lead distribution itself stays simple, explicit and auditable: auditable is the append-only assignment history above and §26's rule that a distribution which cannot be explained after the fact is not complete.

**Open decision — configurable distribution.** Distribution must be explicit and configurable rather than inferred from screen code; that much is the restraint above rather than an exception to it, and it is the same rule §11 already applies to the form catalog. Whether Mallan then builds a configurable rule that resolves the assigned Agent by itself is the unmade part: a rule that produces an assignment is the automatic distribution layer this section says is not built. Until Maya records the decision, assignment remains a named person's explicit act, and the checks below block an assignment rather than compute one.
...
A Lead produced by a Marketing Toolkit output in §9 — a market report, a newsletter, an authored market update, a social or digital post, a share page, video or printed collateral — records `marketing / e-blast` or `approved campaign` above, with the specific output named in the Campaign context. Public search activity that produces no inquiry is §9.4 engagement measurement, not a lead source.
...
**Existing relationship check.** Party resolution answers who this is. The relationship check answers whether Mallan is already working with them, and it runs before assignment rather than after. The Client Relationship state in §3.1 and any open Opportunity on the resolved Party say whether a responsible Agent already exists.

**A Lead on a Party who already has an Agent does not go to a second Agent by default.** Assigning the same person to another Agent while a valid Agent/client relationship is live creates competing relationships with one client inside one firm, and Mallan does not do that by routine. The Lead attaches to the relationship and the Agent that already exist, or the Broker makes a deliberate recorded reassignment. The check runs whether or not it changes the outcome, and its result is retained with the Party resolution result.

A second inquiry from a person Mallan already knows is new activity, and where the interest is new it is a new Opportunity on the Party that already exists. **A Lead is not duplicated because it arrived from another source, channel or campaign.**

**Transfer moves the work, not the record.** A Lead or Opportunity that changes Agents keeps what is already attached to it: communications and activity history, the client's stated requirements and Saved Search lineage under §5.8, the Client × Listing/Unit history under §5.11, documents and their retention under §11.8, and the prior Agent's recorded work. Reassignment does not create a new Party and does not clear history — the §17.6 rule that reassignment moves ownership without altering canonical identity governs any transfer, not only an Agent's departure. Every transfer is a deliberate recorded decision, never an automatic overwrite.
...
**Active business workflow.** From there the work is the role journey in §12 through §16. The lifecycle records that the handoff happened and keeps the Lead's origin attached to it; it does not restate the journey.
...
- leads/response;
- lead outcome by source and by campaign, computed from the source and campaign context already retained on the Lead in §19.1 rather than from a separate marketing metric store, with the numbers underneath governed by §9.4;
- representations;
- listings;
- transactions;
...
## 21.0 Source resilience — no single participant can break Mallan

Mallan's brokerage operating system must survive changes in provider participation, association distribution, brokerage syndication strategy or portal coverage without forking its core architecture.

If a major brokerage, portal or source stops participating in an existing shared feed:

~~~text
COVERAGE GAP
→ VERIFIED RIGHTS / ACCESS
→ SOURCE ADAPTER / OBSERVATION
→ CANONICAL PROPERTY / UNIT / LISTING IDENTITY
→ SOURCE-SPECIFIC RIGHTS ENVELOPE
→ SAME SEARCH / CMA / CRM / REPORT / CLIENT WORKFLOW
~~~

Do not build a Compass system, RealPlus system, StreetEasy system, REsource system or developer-feed system. External platforms may reveal useful product/workflow requirements, but they remain sources/references/adapters around Mallan's canonical operating system.

Mallan must be able to measure source coverage and expose gaps. A source being absent from Cotality does not make it public, shareable or editable. Supplemental/private inventory remains rights-gated and reconciles to canonical identity before Agent Search, client sharing or reporting.

## 21.0.1 Infrastructure/source boundaries cross-reference §0

Provider facts use §0 Cotality rules. Hosting/deployment/environment facts use §0 Vercel rules. Database/control-plane facts use §0 Neon-through-Vercel rules. Git/repository authority uses §0 GitHub/Git rules.

No subsystem may create its own "temporary" source authority to work around an inconvenient provider/control-plane constraint.
...
**That decision is made: the hierarchy is kept and the provider contract is not elevated.** Cotality/Trestle stays registered at SOURCE TERMS / LICENSE / WRITTEN PERMISSIONS as the current provider implementation contract. The rule that the current provider does not define Mallan's business model stands unamended, and the header rule that Cotality is the current provider implementation contract and not the brokerage business model stands unamended.

**Web and Media must comply with both stacks, and neither discharges the other.** Where provider data is involved, a public or client-facing surface satisfies (1) the applicable legal, business, advertising, Fair-Housing, REBNY, RLS and UCBA requirements and (2) the verified Trestle/Cotality technical, use, display, attribution and API contract. Conforming to the provider's display, attribution and API rules does not satisfy Fair Housing, New York advertising law or NYS DOS. Satisfying the legal stack does not license provider data or media beyond the verified provider contract. A surface that clears one stack and fails the other does not publish.

**Where provider data is involved, Web and Media comply with both authorities at once.** The legal, business, advertising, Fair Housing, human-rights, REBNY/RLS/UCBA and source-rights stack above applies, and so does the verified Trestle/Cotality technical, use, display, attribution and API contract. **Satisfying one does not discharge the other.** A provider rule that permits a display does not make the display lawful, and a lawful display is not thereby permitted by the provider contract. It is because the provider contract binds as an implementation contract rather than as a governing authority that provider replacement swaps an adapter instead of requiring Mallan's business architecture to be rewritten. §21.9 and §21.10 are the gates that run both.
...
This gate decides whether the listing may be published. It does not decide which components a published page may then carry. §21.10 supplies the COMPLIANCE / RIGHTS link of the contextual rendering contract in §23.10; component eligibility is settled there.
...
Compliance is a step content passes through before it is published. It is not a disclaimer appended after publication.
...
**Audience transformation happens after the compliance and rights check, never in place of it.** Stripping internal and client-only facts for a public payload, narrowing a client share, masking a professional or owner fact, or reshaping copy for a channel is the transformation step in that order. It is not the compliance step, and it does not repair content that failed the check. **Re-transforming already-cleared content for a different audience or channel re-enters the gate at COMPLIANCE / RIGHTS.**

§21.9 is this contract applied to media, §21.10 to listing publication, §21.11 to selection, ranking and targeting, and §9.7 to composed marketing output. They are four instances of one contract, not four contracts. **A content type that matches none of them still runs the contract.** Where content reaches a public or client-facing surface and no named gate owns it, the missing gate is the finding and the content does not publish until one exists.

The contract governs the whole public-content system:

```text
LISTING DESCRIPTION / HEADLINE / AGENT REMARKS — §21.10
MEDIA CAPTIONS — §21.9, §11.11
EMAIL / E-BLAST / SOCIAL / MARKET REPORTS — §9.7
SEARCH RANKING / PROPERTY RECOMMENDATIONS — §21.11
MARKETING AUDIENCE SELECTION — §21.11, §9.2
AGENT PROFILE AND BROKERAGE COPY — §2.4
...
**Neighborhood copy is a Fair-Housing surface in its own right.** Describing a place is describing who lives in it. Copy about a neighborhood, a school district, a building's population or the kind of person a property "suits" passes the screen before publication, and the prohibited-proxy and anti-steering rules in §21.11 apply to authored description exactly as they apply to a filter or a ranking weight. §22.5 governs what Mallan may infer about a client; this governs what Mallan may publish about a place.

**A lead-generation page is an advertising surface.** It carries brokerage identity, licensed-name, attribution and required-language obligations like any other public page, and its copy, imagery, claims and capture flow run the same screen. Lead generation is a brokerage capability under §19.1 and the inquiry it produces is intake under §23.9; the page that performs the generating is governed here.

**SEO copy, AEO copy and public structured data are authored content, not only publication targets.** §21.10 governs whether a listing may reach those surfaces. Page titles, meta descriptions, answer-engine copy, copy written for discovery and the values placed in structured data are themselves Mallan public statements and pass this contract before publication. Structured data may not assert more than the governed record it describes; a value that does is a public-truth defect under §27.2.

**AI-generated public copy passes the same screen as human-authored copy.** §22.3 forbids AI to publish without required approval — that is an authorization rule, not a content screen, and clearing it does not clear this one. Copy an AI drafted is Mallan's published advertising the moment it appears, and it is reviewed on its content rather than on its author.

Where the content carries provider data, it runs both authority stacks under §21.1.

This section states the breadth §26 already requires of the gate. It does not restate the Fair-Housing, human-rights and advertising obligations in §21.11 or the media rights in §21.9; it orders them and names the content they reach.
...
New investment analysis is an input to that signal and also an outcome of it. An Owner buying again is an Investor Opportunity on the same Party under §16, not a second identity and not a converted Buyer Opportunity.
...
Selling is not the only outcome that evidence can carry. The same signal may point at buying another investment property, or at selling and reinvesting, including a 1031 replacement where relevant. Those outcomes open an Investor Opportunity on the same Landlord Party and are analysed in §16; renew, re-rent and sell remain the §13 branch. §22 surfaces the outcome and links to the working record; it does not merge the Landlord and Investor records, and the cross-role question §16 leaves open for Broker confirmation applies here unchanged.

A past Seller is the fourth case. §12 returns the relationship to the canonical Party and sends the signal here.

```text
NET-PROCEEDS OR VALUATION ACTIVITY / CMA REQUEST
/ MARKET QUESTIONS / STATED PLANS AFTER SALE
/ REINVESTMENT OR 1031 ANALYSIS / TIME SINCE CLOSING
↓
AGENT REVIEW — PAST SELLER MAY BE APPROACHING A NEW DECISION
```

The next decision is usually a purchase, an investment purchase or a 1031 replacement, and each opens its own role Opportunity on the same identity under §1. A past client returning as a repeat client or as a referral source is that same loop, already closed in §1; past-client and referral-source audiences remain governed by §9.2 and §3.2, and referral records by §19.9.
...
### 22.7.1 Lease-expiration review

The renter signal and the landlord signal above are one lease seen from two sides.

```text
LEASE EXPIRATION APPROACHING
↓
LANDLORD DECISION REVIEW  +  TENANT DECISION REVIEW
```

Approximately six months before expiration is the default point at which that signal becomes actionable. It is a configured default where appropriate rather than a fixed rule, and exact policy may vary by lease, role and client in the same way §17.2 governs progressive reminder timing. The resulting Agent action uses the canonical Tasks / Calendar / Reminders object and the next-follow-up state §13 already retains.

**Two paths, not one review.** The Landlord review runs on the canonical Landlord Party and the Tenant review on the canonical Tenant Party. Evidence assembled for one does not surface in the other. §13 already holds the boundary in one direction — the current Tenant relationship is a link, not a copy, and does not expose the Tenant's own Opportunity inside the Landlord workspace — and the same boundary applies in the other direction. Anything either review puts in front of a client also passes the client-facing payload boundary in §5.18.

The Landlord review assembles, where evidence exists:

- current property value, sale market and rental market — §6;
- market rent — Rental CMA evidence under §6 and §13, not this unit's own achieved rent history and not a figure §22 produces;
- current rent, rent history, renewal state and vacancy expectation — the retained Landlord facts in §13;
- current carrying costs, hold-v-sell and sell-v-re-rent — §8;
- the sale and rental Opportunity options already open on the Property.
...
The Tenant review assembles, where evidence exists:

- current rent and the expected renewal cost, where the client or the offered terms have supplied one;
- current rental alternatives — the Tenant's Saved Search under §5.8;
- buying range — the client-provided financial readiness named in the signal above and in §22.6, priced through the buyer payment and cash-to-close presets in §8;
- rent-v-buy and estimated ownership cost — §8;
- current matching sale inventory — §5.8 and the reverse match in §5.16.
...
Both question sets are the outcomes the journeys already carry — the §13 landlord branch and the §15 tenant branch — surfaced early enough to be useful. §22 does not open a second set of outcomes, and the landlord frame is not exhaustive: the investment outcomes on the landlord signal above are available at the same review.

Neither review computes anything. Each composes canonical evidence and says so where an item has no evidence, under the briefing rule in §22.12. Neither is a second briefing format.

### 22.7.2 Lease-lifecycle follow-up

The lease lifecycle carries default follow-up checkpoints:

```text
6 MONTHS BEFORE EXPIRATION → EARLY DECISION / EDUCATION
90 DAYS → DECISION STATUS
60 DAYS → ACTIVE PLANNING
30 DAYS → EXECUTION / URGENCY
```

These are default business checkpoints where appropriate; exact policy may vary by lease, role and client, as §17.2 governs progressive reminder timing.

Each checkpoint is an Agent action on the canonical Tasks / Calendar / Reminders object. §13 forbids a landlord-only reminder mechanism by name; a tenant-only or intelligence-only one is forbidden on the same rule. **The checkpoints are not a lease reminder engine of their own.**

A fired checkpoint is a signal like every other signal in this section. **Do not invent intent** and **every transition routes through Agent review** govern it unchanged: a checkpoint does not contact the client, open an Opportunity or start a campaign. Whether a checkpoint may ever produce a governed client-facing send rather than only an Agent task requires Broker confirmation against this section and the auto-send rules in §5.12 before implementation.

What happens next comes from the client, not from the elapsed interval:
...
The response is a communication event on the one canonical history in §11.1, recorded against the Opportunity. It is what the client said under §3.4 and evidence under §22.4. **There is no fifth client-knowledge store for it.**

A client-stated date supersedes the default checkpoint for that client. Client-stated timing is already Client Intelligence evidence under §22.4; the follow-up is the same canonical Tasks / Calendar / Reminders entry moved to the date the client gave, with the CMA, market and scenario evidence refreshed before the call rather than delivered as it stood.

A recorded response also closes the track it answered.

```text
LANDLORD: "I WILL RENEW WITH THE CURRENT TENANT"
↓
RECORD RESPONSE
↓
RENEWAL FOLLOW-UP CONTINUES — SELLING PROMPTS STOP
```

```text
TENANT: "I AM RENEWING AND NOT INTERESTED IN BUYING"
↓
RECORD RESPONSE
↓
RELATIONSHIP CONTINUES — BUYING FOLLOW-UP STOPS
```

A stated response is a stated exclusion under §3.4 and stays in force until the client changes it, in the same way §5.13 never automatically resends an explicitly rejected listing. Enforcement is the central consent, unsubscribe and suppression governance in §3.2; **§22 does not keep a suppression record of its own.** Suppressing a topic is not a change to the client's stated criteria and does not end the relationship — the client stays on the lifecycle, the remaining checkpoints continue, and §13 and §15 keep working the relationship through the term.
...
### 22.8.1 New-development Buyer Intelligence

The Schedule A unit universe, its source and availability states, Cotality reconciliation and the client-share gate are governed by §4.5 and are not restated here. New-development Buyer Intelligence adds the signals that turn that universe into Agent action for a named Buyer.

```text
BUYER INTEREST SPREAD ACROSS SEVERAL UNITS IN ONE DEVELOPMENT
↓
AGENT REVIEW — ASSEMBLE THE WITHIN-BUILDING COMPARISON
→ UNIT LINE / FLOOR / EXPOSURE / SIZE / PRICE / CARRYING COST
```

The evidence is the Client × Listing/Unit memory in §5.11 read as §22.4 evidence. Repeat interest spread across different units in one building is a different pattern from repeat views of one property and is read as its own signal. The comparison itself is the existing Compare action over §6.3's Building axis and is not a second building report. Any unit-line or building affinity it produces stays an observation under §22.4 and does not become a stated requirement.

```text
CURRENT RESALE IN THE BUILDING IS PRICED DIFFERENTLY
FROM THE SPONSOR / NEW-DEVELOPMENT OFFERING
↓
AGENT REVIEW — GIVE THE BUYER THE COMPARATIVE CONTEXT
```

The spread between a sponsor offering and the current resale asks in the same building is Buyer-decision evidence and should reach the Agent. This signal reads §6.3's Building axis; it defines no evidence class and no valuation rule of its own. **An offering-plan price and a current market ask are two different facts**, each carrying its own source label under §5.7 and §7.1, and unconfirmed Schedule A opportunities remain outside verified valuation evidence exactly as §6.3 requires.

```text
ACCEPTED AMENDMENT CHANGED A MATERIAL UNIT / BUILDING FACT
↓
BUYER OPPORTUNITIES HOLDING THAT UNIT
+ SAVED ANALYSES THAT USED THE SUPERSEDED FACT
↓
AGENT REVIEW
```

§21.6 already raises the flag and §21.7 already maps the affected systems; §6.8 already lets a delivered analysis be marked stale and reversioned. What this signal adds is the step from the technology flag to the named client work — the Buyer Opportunities that hold the unit and the specific CMAs, comparisons and sends that consumed the superseded fact. Amendment truth stays in §4.5.4 and §4.5.5 and the document set stays in §11.10: **this creates no second amendment record, no second staleness store and no second provenance truth.**

Every signal in this lane routes through Agent review under §22.7 and carries its evidence under §22.2. None of them contacts a Buyer, reopens an analysis or broadens a Saved Search on its own.

**Held for Maya decision — a fact already shared.** When an accepted amendment supersedes a material fact that a Buyer was already given, whether Mallan owes that Buyer a client-facing correction rather than only an Agent task is a Broker decision. Until it is recorded, the signal produces an Agent task and the Agent decides what the client is told.
...
Field exposure is one half of that rule. Which components a screen may carry at all is the contextual rendering contract in §23.10.
...
Approaching lease expirations are contextual in the same way and are reached from the Landlord, Tenant and Listing workflows, but the Agent also needs one cross-client view of the tenancies coming up, grouped by time to expiration and by the outcome already recorded. That view reads the canonical tenancy facts in §13 and §15 — lease expiration, current rent, the current Tenant relationship, renewal state, vacancy expectation and next follow-up — together with the lease-expiration signals §22.7 and §22.11 already raise, and it identifies the actual record, the evidence and the next action the way §23.3 requires of any priority action. Renewal and re-rent are the outcome states §13 and §15 record. **A possible Seller conversation or a possible renter-to-buyer move is a §22.7 signal shown as a signal, never as recorded client intent.** The firm-wide exception view over the same tenancies stays with §18. It is not a separate lease database and not a second lease-expiration truth.
...
Where a new-development or Schedule A unit is shared, the Buyer receives one client-safe presentation rather than the Agent's research record with fields stripped out. It carries the building/property and the unit; the relevant price with an explicit source label, keeping the offering-plan or source price and the current market ask as two separate facts; the current availability state; the current market listing where one exists; approved building and unit media; the approved floor plan; carrying costs computed by §8; the selected Offering Plan information or documents supplied under the §11.10 courtesy rules; the Agent's analysis; Compare and CMA; comments; and a showing or appointment request. Whether the unit may be shared at all is the §4.5.7 gate, and which fields may travel is the §5.18 boundary — §23.4 names what the Buyer sees and decides neither.
...
Use the same overall structure but preserve the acquisition, rent, NOI, cap, cash-on-cash, ROI, financing, vacancy, hold, exit and 1031 semantics defined in §16. The Investor page links to the Seller, Landlord and Buyer Opportunities on the same Party; it does not absorb or restate them.

One Party may have multiple role Opportunities; role workspaces must not duplicate the Party.

The same rule organizes the Agent's book. `CLIENTS` in §23.2 is understandable by current relationship: buyers, sellers, landlords, tenants, investor/1031 clients, past clients and dated future opportunities are each reachable as a view of the Agent's own book. These are filters over the same canonical Party and Opportunity records, not rosters — a Party holding two role Opportunities appears in both views and remains one person. Past clients are the post-deal relationship in §1, not a closed file; future opportunities are the dated `FUTURE` outcome in §19.1. Leads, listings, deals and follow-ups already have their own entries in §23.2 and are not repeated here. **Do not create a My Business client list that holds its own client truth.**
...
Public Web exposes the brokerage relationship, not only the inventory. Someone who is not shopping a listing — an owner weighing a sale, a landlord whose lease is expiring, a renter, an investor, a Buyer looking at new development — must still be able to enter Mallan through the service they actually need.
...
TENANT / RENTER
INVESTOR / 1031
NEW-DEVELOPMENT BUYER REPRESENTATION
```

These are the roles §3 already names and the intents a Lead already records in §19.1. Public Web adds the entry, not a seventh role and not a second role vocabulary. Each path states what Mallan does in the one governed professional identity of §2.4, is composed as governed marketing content under §9 and published through §21.10, and stays inside the §4.5 exclusion the same way every other public surface does. The new-development path is the Buyer-representation service described in §4.5 and §9.7; it does not present Mallan as sponsor, developer or listing broker.

**A service path is an intake surface, not a standing contact address.** Each path resolves through the public conversion action below, so an owner asking about a valuation and a Buyer asking about a listing enter the same canonical intake.
...
Titles, meta descriptions, answer-engine copy and the values placed in structured data are themselves Mallan-authored public content and pass §21.12 before publication. Discoverability work does not route around the content screen any more than it routes around the eligibility rule.
...
## 23.10 Contextual rendering contract

An eligible record does not make every component eligible. **A component may not render merely because its code or data structure technically accepts the record.**
...
This chain is not a second eligibility truth. Its COMPLIANCE / RIGHTS link is the existing one — §4.5.7 for private supplemental sharing, §5.18 for the client-facing payload boundary, §11.1 for communication visibility, §21.9 for media, §21.10 for listing publication and §21.11 for selection, ranking and targeting — and it restates none of them. §23.10 adds only the links those rules never carried: the record's canonical business type, and whether this component belongs on this record for this viewer.

**A component that cannot prove contextual eligibility does not render.** It is absent — not empty, not a placeholder, and not filled with the nearest available substitute. This is the same fail-closed posture §26 requires of a rendering path that cannot prove eligibility.

The contract binds backend and public surfaces alike. §7.3's source-aware controls and §7.4's context-gated action bar are the Agent-side instance of it. The public side carries the same obligation and holds no exemption for being a marketing page.

**Canonical business type is the first link, not an afterthought.** A record whose canonical or provider property type falls outside the residential and investment business types Mallan actually serves does not inherit the residential component set by default. Which business types Mallan serves is a §1 and §3 decision; this contract governs what happens once that decision is recorded and does not itself widen Mallan's business model.

Sale and Rental, Seller and Landlord, Buyer and Tenant are already separated wherever a subsystem owns the difference — rental fee and disclosure treatment in §9.7, rental-specific Landlord reporting in §10.4, the governed rental form families in §11.4, the role-specific client pages in §23.4 and the separate Tenant and Buyer workflow semantics in §14 and §15. **What none of them reaches is the public surface.** A public rental presentation does not render sale components, and a public sale presentation does not render rental components, on the same reasoning those subsystems already apply internally.

Each component names the section that owns its business truth. A component with no owning section is the finding, not a licence to render it:

```text
RENT-v-BUY — §8
INVESTMENT ANALYSIS — §8, §16
CLOSING-COST CALCULATOR — §8; role gating is owned, the public component is the finding
CMA — §6, §23.4
OPEN HOUSE — §7.1, §7.4
OFFER CONTROLS — §12, §14, §20; sale context
APPLICATION CONTROLS — §13, §15, §20; rental context
NEW DEVELOPMENT / SCHEDULE A PRESENTATION — §4.5.4, §4.5.7, §7.3
BUILDING AMENITIES — §4.5.6
MEDIA — §21.9, §11.11
DISCLOSURE BLOCKS — §9.7, §11.6, §21.10
NEIGHBORHOOD CONTENT — §21.12
SCHOOLS — no owning section; the finding
MARKETING CTA — §19.1 and §23.9 own what the resulting inquiry becomes; the eligibility of the CTA itself is the finding
```

**Schools are not a neutral amenity component.** School content on a public page is a steering surface. Mallan does not publish one until §21.12 governs its copy and §21.11's prohibited-proxy and anti-steering rules are applied to it. Until then it does not render.

---

# 24. REQUIREMENT / DOCUMENT GOVERNANCE AND CURRENT-TO-TARGET MAP

## 24.0 Business-completeness ownership matrix

Every durable Mallan capability has one owning section in this Master. "Not mentioned" is not an acceptable way for a requirement to disappear.

| Capability family | Master owner |
|---|---|
| Brokerage mission / capability admission / no parallel systems | §1 |
| Human roles / Brokerage View / My Business | §2 |
| Party / Entity / contact / client lifecycle / disposition | §3 |
| Property / Building / Unit / Listing identity and source rights | §3–§4 |
| Mallan listing source/edit/publication authority | §4 |
| Supplemental/private inventory / Schedule A / source coverage | §4 + §21 |
| Basic / Advanced / Building / Map / Agent / Consumer Search | §5 |
| Saved Search / reverse matching / client-listing memory / alerts | §5 |
| CMA / comps / valuation / property intelligence | §6 |
| Sale + Rental Listing Intake / forms / tools | §7 |
| Backend Listing/Opportunity Workspace | §7 |
| Open Houses / showings / feedback | §7 + §20 |
| Calculators / decision support | §8 |
| Marketing / e-blast / client share | §9 |
| Seller / Landlord live-listing reporting / report infrastructure | §10 |
| Communications / comments / tasks / calendar | §11 |
| Agreements / disclosures / documents / e-sign / Offering Plans | §11 |
| Media / rights / provenance | §11 + §21 |
| Seller operating journey | §12 |
| Landlord / tenancy lifecycle | §13 |
| Buyer operating journey | §14 |
| Tenant operating journey | §15 |
| Investor / 1031 opportunity + signal intelligence | §16 |
| Agent onboarding / professional requirements / policy / profile | §17 |
| Brokerage oversight / exception queues | §18 |
| Leads / assignment / performance / money / commissions / referrals | §19 |
| Offers / applications / transactions / payment readiness | §20 |
| Cotality / REBNY / RLS / source drift / source resilience | §0 + §21 |
| Client / owner / market / agent intelligence + contextual AI | §22 |
| Agent Home / Clients / Deals / Money / Portal / Public Web | §23 |
| Requirement identity / proof state / current-to-target mapping | §24 |
| Dependency-based development program | §25 |
| Definition of Done | §26 |
| GitHub execution control / impact graph / independent proof | §27 |
| Vercel / Neon / environment/database authority | §0 + §27 |

When a new durable capability is approved, update this matrix and its owning section in the same authority change. Do not create a second plan because the matrix is inconvenient.
...
### Public Web brokerage product

```text
CURRENT
the public product is inventory-shaped and identity-shaped; the brokerage service relationships are not exposed as entry paths and durable capture of a non-listing service inquiry is not proven

TARGET
named public entry paths for Buyer, Seller, Landlord/Owner, Tenant/Renter, Investor/1031 and new-development Buyer representation, each stating the service in the governed brokerage identity, composed and published through the §21.10 gate and inside the §4.5 and §23.9 exclusions, and each resolving through §19.1 to a responsible Agent with durable history
```
...
This capability exists to strengthen Mallan's representation of Buyers and Investors. Developer/sponsor representation, project marketing and developer sales operations are outside current scope unless Maya explicitly adds them later.

Developer/sponsor representation, project marketing and developer sales-office operations remain outside scope unless Maya adds them (§1).
...
The §1 capability questions fire earlier than both: they fire on scope, and a capability this matrix finds missing still has to pass them before it is architected.
...
Coverage inside a family is measured against named rows. A row is an index entry — the capability, the section that owns its business truth, and the same attributes above. **A row may not introduce a capability the owning section does not state.** Where no section owns the row, the row is the finding.

```text
Lead intake / Lead lifecycle / Party — CRM
  LEAD SOURCE — §19.1
  LEAD GENERATION — §19.1; the authorized sources are owned, the brokerage capability that produces them is the finding
  LEAD DISTRIBUTION — §19.1, §18; the governed distribution rule is an open Broker decision recorded in §19.1
  LEAD ASSIGNMENT — §19.1
  LEAD REASSIGNMENT — §19.1, §17.6
  LEAD FOLLOW-UP — §19.1
  LEAD CONVERSION — §19.1
  LEAD NURTURE — §19.1

Seller / Landlord / Buyer / Tenant / Investor — 1031
  LEASE LIFECYCLE — §13, §15
  LEASE EXPIRATION — §13, §15; firm-wide exception §18; signals §22.7
  LEASE RENEWAL — §13, §15
  LANDLORD → RE-RENT — §13
  LANDLORD → SELLER CONVERSION — §13, §22.7
  TENANT → NEW RENTAL — §15
  TENANT → BUYER CONVERSION — §15, §22.7
  LANDLORD 6-MONTH REVIEW — §13, §22; the dated review point is the finding
  TENANT 6-MONTH REVIEW — §15, §22; the dated review point is the finding
  LEASE FOLLOW-UP CHECKPOINTS — §13, §15; run on the canonical Tasks / Calendar / Reminders object, not on §17.2's professional-requirement intervals; the dated cadence is the finding

CMA / CMA Intelligence / Calculators
  RENT-v-BUY REPORT — §8, §15, §22.7; also named Cost to Become an Owner. The client deliverable is the finding — §10 owns Seller and Landlord reports only
  HOLD-v-SELL / RE-RENT ANALYSIS — §8, §13

Agent onboarding / Agent professional requirements / Broker and Agent operating documents
  AGENT W-9 / TAX IDENTITY RECORD — §11.12

Commissions / Referrals / Money
  AGENT TAX-YEAR PAYMENT RECONCILIATION — §19.6, §18, §11.12; the Agent-facing annual tax record is the finding

Public Web / SEO, AEO / Client Portals
  PUBLIC BUYER ENTRY — §23.9, §19.1
  PUBLIC SELLER ENTRY — §23.9, §19.1
  PUBLIC LANDLORD / OWNER ENTRY — §23.9, §19.1
  PUBLIC TENANT / RENTER ENTRY — §23.9, §19.1
  PUBLIC INVESTOR / 1031 ENTRY — §23.9, §19.1
  PUBLIC NEW-DEVELOPMENT BUYER-REPRESENTATION ENTRY — §23.9, §9.7, §4.5
```

These rows do not disappear during implementation. A row removed from this list is a decision, not a cleanup.
...
Where a Lead is distributed rather than handed to a named Agent, the distribution is itself a record: the source, the Party resolution result, any Agent relationship already standing on that Party, the rule that was applied, the receiving Agent and the history that follows are retained and reviewable afterward. **A distribution that cannot be explained after the fact is not complete.** §19.1 holds open whether a stated, configurable distribution rule is compatible with its own restraint that assignment stays explicit and that no scoring, ranking or automatic distribution layer is built on it. Until that Broker decision is recorded, this clause governs distribution wherever it already happens and does not authorize building a distribution layer.
...
Not complete link by link either. For a new-development Buyer the standard is the whole path working through itself: Offering Plan and Schedule A → canonical Building and Unit → version and amendment provenance → reconciliation against current Cotality inventory → explicit availability state → professional Agent Search → Buyer match → verified availability → client-safe presentation → Compare, CMA and analysis → coordinated showing → Offer and Buyer workflow. Every link is owned by a standard already stated here — Search for criteria execution, source reconciliation and client-safe output; Backend Listings / Opportunities for Reverify, availability verification, Compare/CMA and showing coordination; Media / Web for the publication, rights and attribution gate — and none of them is restated. **This standard is not met because each link passes on its own. It is met when a Buyer can travel the whole path without the unit changing identity, losing its provenance or acquiring a second version of itself.**

Two failures end it wherever they remain possible: an offering-plan price and a current market ask presented as though they were necessarily the same fact, when §4.5.5 composes them as separate fields and §5.7 and §7.1 require each to carry its own source label; and a separate new-development Search, CMA or CRM universe standing beside the canonical one, which §1 and §5.3 already forbid.
...
Advance visibility is dated. Not complete until the executed lease expiration date drives a first review roughly six months ahead and further checkpoints at ninety, sixty and thirty days where the situation still calls for them; the Landlord side and the Tenant side are each contacted and each response — including no response — is recorded on that side's own relationship; the next follow-up is set by what the response actually was rather than by a blind schedule; and the market, valuation and rent-v-buy or hold-v-sell evidence the decision needs is available at the decision point from §8 and §6 rather than assembled from memory. These checkpoints run on the canonical Tasks / Calendar / Reminders object under §13 — **do not build a lease-only reminder mechanism** — and they are not the professional-requirement reminder intervals in §17.2. Landlord and Tenant remain separate Parties with separate Opportunities: the current Tenant link in §13 is a link, not a copy, and contact recorded on one side does not open the other side's Opportunity.

The lease produces a recorded outcome rather than a lapsed date. Renew, re-rent, relocate and sell are the §13 and §15 branches; a decision to buy another investment, or to sell and reinvest through a 1031 exchange, opens the matching Opportunity under §16 on the same canonical Party; hold and any other owner decision are recorded under §13's OTHER BUSINESS DECISION rather than left as an unhandled state. **This is not a new outcome vocabulary** — §13 and §15 own the branches and §16 owns the investment analysis. Whatever the outcome, the resulting Opportunity carries the same Party, the same Property and tenancy history and the responsible Agent forward.
...
Ownership travels with the work. Not complete until the responsible Agent stays attached from the Lead through assignment, the canonical Party, the role Opportunity, My Business, the Search, listing, CMA and workflow record, the Deal, the post-deal relationship and the future opportunity that follows it, while Brokerage View sees the same records at firm scope over one canonical history rather than a second roster, and until a returning client reconciles to the existing Party under §3.1 rather than becoming another person. Reassignment moves the work and preserves it: the prior Agent, the receiving Agent, the date, the reason and who made the change remain recorded, and the communications and history, the client's stated requirements, Search history, property activity, documents and prior work stay with the record. §17.6 remains the authority for what reassignment may not alter. How much durable lineage the Lead itself carries is held open in §5.8 and §19.1; this standard does not decide that question.

Not complete until the Agent's own recorded payments roll up to a tax year and to the applicable annual tax record for that year, the Agent can see and retrieve the Agent's own record, Maya can see the firm-wide set, and both are retained under §11.8. The identification documents behind it are the §11.12 W-9/tax records; the payment facts are the §19.6 internal commission truth and the §18 accountant-ready annual payment records. **An annual tax figure is a view of recorded payments, never a second payment truth**, and Mallan provides operational accounting and payment records rather than replacing the accountant, as §19.8 already states.
...
- Cotality-shaped work requires live provider proof under §0.
- Vercel-shaped work requires live project/deployment/environment/integration proof under §0.
