...
`lib/idx/auth.ts`), caches it for **~15 min** (`TRESTLE_METADATA_TTL_MS`), and exposes 4 tools:
...
- **2026-07-05 — multi-schema parse fix + cadence + dynamic resources.** The live Cotality
  ...
  moved 24h → 15 min. `trestle_list_fields` now accepts **any live resource** (dynamic,
