...
// Bucket-B dry-run planner:
//   npm run media:backfill:dryrun -- --max-listings 500 --max-requests 300
//   ... --checkpoint .media-dryrun-checkpoint.json --resume
//
// Same strict fail-closed flags as the audit CLI (unknown/misspelled/
// duplicate flags and invalid supplied bounds exit 1; --retries 0 allowed;
// --resume REQUIRES an existing valid checkpoint file). Checkpoints are
// identity-bound and written atomically. scanComplete and coverageComplete
// are reported separately; CONFLICT listings never emit an executable plan.
// NEVER writes — no apply flag exists. Secrets never printed.
...
import {
  ...
  writeCheckpointAtomic, COTALITY_BASE,
...
export async function main(): Promise<void> {
  ...
  if (resume) {
    ...
    validateDryRunCheckpoint(checkpoint, identity); // fail-closed BEFORE any work
  }
  ...
  const deps: DryRunDeps = {
    ...
    checkpoint,
    ...(checkpointPath ? {
      saveCheckpoint: (cp: DryRunCheckpoint) => { writeCheckpointAtomic(checkpointPath, cp); },
    } : {}),
  };
  ...
  try {
    ...
    if (asJson) {
      ...
      console.log(`  scanComplete=${res.scanComplete} coverageComplete=${res.coverageComplete}`);
      ...
      console.log(`  this run: finalized=${res.runCounters.listingsFinalized} probes=${res.runCounters.probesAttempted} attempts=${res.runCounters.requestAttempts} retries=${res.runCounters.retries} unknownRetried=${res.runCounters.unknownRetriesAttempted}`);
    ...
    if (!res.coverageComplete) {
      console.error(`\nNOT COVERAGE-COMPLETE (scanComplete=${res.scanComplete}): ${res.incompleteReasons.join('; ') || 'unresolved listings remain'}`);
      process.exitCode = 2;
    }
  } finally {
