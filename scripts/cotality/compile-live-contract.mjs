...
//
// Ported 2026-09-08 from d19c03cd:scripts/cotality/compile-live-contract.mjs (byte-identical at
// 42c5f241) and verified by execution (--schema-only: 17 resources, 1,456 fields, 31 relationships,
// all catalogs complete). One adaptation: the bundle now carries a FINGERPRINT — sha256 over the
// provider evidence, plus the repo SHA that compiled it — so a Search proof can name the exact
// contract it was verified against, and evidence from another contract or SHA cannot certify it.
//
...
// FINGERPRINT — over the provider-derived content only (never the acquisition timestamps), with
// sorted keys so the same live contract hashes identically regardless of arrival order.
...
const bundle = {
  ...
  summary: {
    ...
    // $metadata is the existence authority for THIS subscription; the Field catalogue is platform-wide
    // (lists resources and fields we cannot select). The two counts are reported side by side so the
    // difference is visible, never silently merged.
    fieldCatalogRows: fieldCatalog.value?.rows?.length ?? null,
