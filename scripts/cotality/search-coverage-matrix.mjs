...
const STATIC_DEFECTS = [
  ...
  { id: 'furnished-ungated', surface: 'Rental Search', claim: "public/legacy paths apply the furnished filter without a rental gate (`?type=sale&furnished=true` narrows the sale universe)", cite: 'lib/search/public-listing-trestle.ts:332; lib/search/public-listing-db.ts:402-405; lib/search/types.ts:183', check: () => ({ status: /Furnished eq 'Furnished'/.test(srcText('lib/search/public-listing-trestle.ts')) ? 'PATTERN PRESENT (gating not mechanically assessed)' : 'NOT REPRODUCED' }) },
