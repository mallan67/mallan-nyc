...
async function run() {
  ...
  const idxViolation = await prisma.listing.count({
    where: {
      status: { in: ['Closed', 'Sold', 'Rented', 'Leased', 'Withdrawn', 'Expired', 'Canceled', 'Cancelled', 'Delete'] }, // = lib/listings/mallan-status.ts TERMINAL_STATUS_FILTER_VALUES (live tokens + legacy spellings)
