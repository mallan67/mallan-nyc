...
async function run() {
  ...
  const filter = `(${ids.map(id => `ResourceRecordID eq '${id}'`).join(' or ')}) and MediaCategory eq 'Photo' and Order le 3`;
  const t = Date.now();
  const r = await fetch(`${apiUrl}/odata/Media?$filter=${encodeURIComponent(filter)}&$select=ResourceRecordID,MediaURL,Order,PreferredPhotoYN&$top=200&$orderby=${encodeURIComponent('ResourceRecordID asc,Order asc')}`, {
    headers: { Authorization: `Bearer ${access_token}`, Accept: 'application/json' },
  });
  const d = await r.json();
  const photoIds = new Set((d.value || []).map(m => m.ResourceRecordID));
  ...
  const filter2 = `(${ids.map(id => `ResourceRecordID eq '${id}'`).join(' or ')}) and MediaCategory eq 'Photo' and Order le 1`;
  const t2 = Date.now();
  const r2 = await fetch(`${apiUrl}/odata/Media?$filter=${encodeURIComponent(filter2)}&$select=ResourceRecordID,MediaURL,Order,PreferredPhotoYN&$top=200&$orderby=${encodeURIComponent('ResourceRecordID asc,Order asc')}`, {
    headers: { Authorization: `Bearer ${access_token}`, Accept: 'application/json' },
  });
  ...
  const filter3 = `(${ids.map(id => `ResourceRecordID eq '${id}'`).join(' or ')}) and PreferredPhotoYN eq true`;
  const t3 = Date.now();
  const r3 = await fetch(`${apiUrl}/odata/Media?$filter=${encodeURIComponent(filter3)}&$select=ResourceRecordID,MediaURL,Order,MediaCategory&$top=200`, {
    headers: { Authorization: `Bearer ${access_token}`, Accept: 'application/json' },
  });
  const d3 = await r3.json();
  const photoIds3 = new Set((d3.value || []).map(m => m.ResourceRecordID));
  ...
  for (const m of (d3.value || []).slice(0, 3)) {
    console.log(`  Preferred: ${m.ResourceRecordID} Cat=${m.MediaCategory} Order=${m.Order}`);
  }
}
