#!/usr/bin/env node
/**
 * scripts/reso/schema-audit.js — Audit our Prisma `Listing` model
 ...
 *   npm run reso:schema-audit -- --json
 */
const fs = require('fs');
...
(async () => {
  ...
  lines.push('');
  lines.push(`> Column-by-column compare of \`prisma/schema.prisma\` Listing model vs REBNY IDX Plus CSV (\`data/rebny-rls-property-fields.csv\`) vs Trestle's \`$metadata\` snapshot (\`artifacts/metadata.xml\`; NOT live — refresh via scripts/get-metadata.js).`);
  lines.push(`> Generated by \`npm run reso:schema-audit\`. Captured: ${new Date().toISOString()}.`);
  lines.push('');
  lines.push('## Stats');
  lines.push('');
