 ...
 */
import { parseRESODateLocal } from "@/app/components/ComingSoonBadge";

describe("parseRESODateLocal", () => {
  it("preserves the calendar day for date-only YYYY-MM-DD input", () => {
    const d = parseRESODateLocal("2026-05-01");
  ...
  it("renders 'May 1' (not 'April 30') in en-US toLocaleDateString in any US timezone", () => {
    ...
    const formatted = parseRESODateLocal("2026-05-01").toLocaleDateString("en-US", {
      month: "long",
      day: "numeric",
      year: "numeric",
    });
  ...
  it("preserves the calendar day across the year boundary", () => {
    const d = parseRESODateLocal("2027-01-01");
  ...
  it("handles whitespace around the date", () => {
    const d = parseRESODateLocal("  2026-12-31  ");
  ...
  it("falls back to native Date for full ISO timestamps with offsets", () => {
    ...
    const d = parseRESODateLocal("2026-05-01T12:00:00-04:00");
