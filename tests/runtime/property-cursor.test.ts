...
describe("advancePropertyCursor", () => {
  ...
  it("incomplete page chain → null regardless of processed content (preserve prior cursor)", () => {
    const processed = [rec("t1", "L1"), rec("t2", "L2"), rec("t3", "L3")]; // all ok...
    expect(advancePropertyCursor(processed, false)).toBeNull(); // ...but chain not drained
  });
  ...
  it(">500 backlog oldest-first advances only to the last processed record (tail re-fetched, not skipped)", () => {
    ...
    const next = advancePropertyCursor(processed, COMPLETE);
  ...
  it("a mid-batch failure freezes the cursor at its predecessor", () => {
    ...
    expect(advancePropertyCursor(processed, COMPLETE)).toEqual({ ts: "t2", key: "L2" });
  });

  it("a failure at the very first record → null (do not advance past unprocessed)", () => {
    const processed = [rec("t1", "L1", false), rec("t2", "L2")];
    expect(advancePropertyCursor(processed, COMPLETE)).toBeNull();
  });

  it("empty run → null (cursor preserved by caller)", () => {
    expect(advancePropertyCursor([], COMPLETE)).toBeNull();
  });

  // Tie-block safety (live-proven for the Media endpoint; defensive default for
  // Property): the cursor must never stop INSIDE a tie block that is not fully
  ...
  it("does not advance into a partially-processed tied-timestamp block; backs off to the last fully-drained ts", () => {
    ...
    expect(advancePropertyCursor(processed, COMPLETE)).toEqual({ ts: "t1", key: "L2" });
  });
  ...
  it("advances to the last record when the final timestamp group is fully processed", () => {
    ...
    expect(advancePropertyCursor(processed, COMPLETE)).toEqual({ ts: "t2", key: "L3" });
  });
